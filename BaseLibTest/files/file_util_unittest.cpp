...
namespace base {
	...
	namespace {
		...
		TEST_F(FileUtilTest, CreateTemporaryFileInDirLongPathTest) {
			...
			path_buffer_length = GetLongPathName(as_wcstr(temp_file.value()), as_writable_wcstr(path_buffer), MAX_PATH);
...
#define MAYBE_FileToFILE FileToFILE
		...
		TEST_F(FileUtilTest, AppendToFile) {
			...
			EXPECT_EQ(static_cast<int>(data.length()), WriteFile(foobar, data.c_str(), data.length()));
		...
		TEST_F(FileUtilTest, ReadFile) {
			...
			ASSERT_EQ(static_cast<int>(kTestData.size()), WriteFile(file_path, kTestData.data(), kTestData.size()));
		...
		TEST_F(FileUtilTest, ReadFileToString) {
			...
			// Create test file.
			ASSERT_EQ(static_cast<int>(strlen(kTestData)), WriteFile(file_path, kTestData, strlen(kTestData)));
...
#define MoreThanBufferSizeChildMainString \
  "WriteToPipeMoreThanBufferSizeChildMain"
		...
		TEST_F(FileUtilTest, ReadFileToStringWithLargeFile) {
			...
			// Create test file.
			ASSERT_EQ(static_cast<int>(kLargeFileSize),	WriteFile(file_path, data.c_str(), kLargeFileSize));
		...
		TEST_F(FileUtilTest, TouchFile) {
			...
			FilePath foobar(data_dir.Append(FILE_PATH_LITERAL("foobar.txt")));
			std::string data("hello");
			ASSERT_EQ(static_cast<int>(data.length()), WriteFile(foobar, data.c_str(), data.length()));
		...
		TEST_F(FileUtilTest, IsDirectoryEmpty) {
			...
			std::string bar("baz");
			ASSERT_EQ(static_cast<int>(bar.length()), WriteFile(foo, bar.c_str(), bar.length()));
