	...
	namespace win {
		...
		namespace {
			...
			TEST_F(RegistryTest, TruncatedCharTest) {
				...
				ASSERT_EQ(ERROR_SUCCESS, key.WriteValue(kName, kData, size(kData), REG_BINARY));
			...
			TEST_F(RegistryTest, OpenSubKey) {
				RegKey key;
				ASSERT_EQ(ERROR_SUCCESS, key.Open(HKEY_CURRENT_USER, kRootKey, KEY_READ | KEY_CREATE_SUB_KEY));
			...
			TEST_F(RegistryTest, ChangeCallback) {
				...
				ASSERT_TRUE(key.StartWatching(BindRepeating(&TestChangeDelegate::OnKeyChanged, Unretained(&delegate))));
