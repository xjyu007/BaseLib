...
namespace base {
	...
	void FeatureList::InitializeFromSharedMemory(
		...
		while ((entry = iter.GetNextOfObject<FeatureEntry>()) != nullptr) {
			...
			const auto trial = FieldTrialList::Find(trial_name.data());
	...
	bool FeatureList::IsFeatureOverriddenFromCommandLine(
		...
		const auto it = overrides_.find(feature_name);
	...
	void FeatureList::AddFeaturesToAllocator(PersistentMemoryAllocator* allocator) {
		...
		for (const auto & override : overrides_) {
			...
			const auto total_size = sizeof(FeatureEntry) + pickle.size();
			const auto entry = allocator->New<FeatureEntry>(total_size);
			if (!entry)
				return;
			...
			const auto dst = reinterpret_cast<char*>(entry) + sizeof(FeatureEntry);
	...
	bool FeatureList::IsFeatureEnabled(const Feature& feature) {
		...
		const auto it = overrides_.find(feature.name);
	...
	void FeatureList::RegisterOverridesFromCommandLine(
		...
		for (const auto& value : SplitFeatureListString(feature_list)) {
			...
			const auto pos = feature_name.find('<');
			if (pos != std::string::npos) {
				feature_name = std::string(value.data(), pos);
				trial = base::FieldTrialList::Find(value.substr(pos + 1).data());
#if !defined(OS_NACL)
	...
	void FeatureList::RegisterOverride(std::string_view feature_name, 
		...
		overrides_.insert(std::make_pair(
			feature_name.data(), OverrideEntry(overridden_state, field_trial)));
	}
	...
	bool FeatureList::CheckFeatureIdentity(const Feature& feature) {
		...
		const auto it = feature_identity_tracker_.find(feature.name);
