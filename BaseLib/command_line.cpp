...
// Unixes don't use slash as a switch.
		...
		bool IsSwitch(const CommandLine::StringType& string,
			...
			switch_string->clear();
			switch_value->clear();
			const auto prefix_length = GetSwitchPrefixLength(string);
			if (prefix_length == 0 || prefix_length == string.length())
				return false;

			const auto equals_position = string.find(kSwitchValueSeparator);
...
#error Unsupported platform
		...
		std::wstring QuoteForCommandLineToArgvW(const std::wstring& arg,
			...
			std::wstring quotable_chars(FILE_PATH_LITERAL(" \\\""));
			...
			for (size_t i = 0; i < arg.size(); ++i) {
				if (arg[i] == '\\') {
					...
					auto backslash_count = end - start;
	...
	CommandLine::CommandLine(int argc, const CommandLine::CharType* const* argv) : argv_(1), begin_args_(1) {
	...
	void CommandLine::set_slash_is_not_a_switch() {
		...
		// Leverage std::wstring_view::operator== to do a deep comparison.
		DCHECK_EQ(*std::rbegin(kSwitchPrefixes),
			std::wstring_view(FILE_PATH_LITERAL("/")).data());
...
#if defined(OS_WIN)
	// static
	CommandLine CommandLine::FromString(std::wstring_view command_line) {
...
#error Unsupported platform
	...
	std::string CommandLine::GetSwitchValueASCII(
		...
		const auto value = GetSwitchValueNative(switch_string);
	...
	CommandLine::StringType CommandLine::GetSwitchValueNative(
		const std::string_view& switch_string) const {
		DCHECK_EQ(ToLowerASCII(switch_string), switch_string);
		const auto result = switches_.find(switch_string);
...
#if defined(OS_WIN)
		const auto switch_key = ToLowerASCII(switch_string);
		auto combined_switch_string(ASCIIToUTF16(switch_key));
#elif defined(OS_POSIX) || defined(OS_FUCHSIA)
		const std::string & switch_key = switch_string;
		StringType combined_switch_string(switch_key);
#endif
		const auto prefix_length = GetSwitchPrefixLength(combined_switch_string);
		auto insertion =
...
#if defined(OS_WIN)
		AppendSwitchNative(switch_string, ASCIIToUTF16(value));
#elif defined(OS_POSIX) || defined(OS_FUCHSIA)
...
#error Unsupported platform
	...
	void CommandLine::RemoveSwitch(std::string_view switch_key_without_prefix) {
#if defined(OS_WIN)
		auto switch_key_native = ASCIIToUTF16(switch_key_without_prefix);
#elif defined(OS_POSIX) || defined(OS_FUCHSIA)
		auto switch_key_native = switch_key_without_prefix.data();
#endif
		...
		const auto erased_from_switches =
			switches_.erase(switch_key_without_prefix.data());
		...
		const auto argv_switches_begin = argv_.begin() + 1;
		const auto argv_switches_end = argv_.begin() + begin_args_;
		DCHECK(argv_switches_begin <= argv_switches_end);
		DCHECK(argv_switches_end <= argv_.end());
		const auto expell = std::remove_if(argv_switches_begin, argv_switches_end,
...
#error Unsupported platform
	...
	void CommandLine::PrependWrapper(const StringType& wrapper) {
		...
		using CommandLineTokenizer =
			StringTokenizerT<StringType, StringType::const_iterator, CharType>;
...
#if defined(OS_WIN)
	void CommandLine::ParseFromString(std::wstring_view command_line) {
...
#endif
		const auto params(GetArgumentsStringInternal(quote_placeholders));
		if (!params.empty()) {
