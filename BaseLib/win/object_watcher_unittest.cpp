...
#include "run_loop.h"
#include "../../BaseLib_Test/scoped_task_environment.h"
...
namespace base {
	namespace win {
		...
		namespace {
			...
			void RunTest_BasicSignal(test::ScopedTaskEnvironment::MainThreadType main_thread_type) {
			...
			void RunTest_BasicCancel(test::ScopedTaskEnvironment::MainThreadType main_thread_type) {
				//test::ScopedTaskEnvironment scoped_task_environment(main_thread_type);
			...
			void RunTest_CancelAfterSet(test::ScopedTaskEnvironment::MainThreadType main_thread_type) {
				//test::ScopedTaskEnvironment scoped_task_environment(main_thread_type);
			...
			void RunTest_SignalBeforeWatch(test::ScopedTaskEnvironment::MainThreadType main_thread_type) {
				//test::ScopedTaskEnvironment scoped_task_environment(main_thread_type);
			...
			void RunTest_OutlivesTaskEnvironment(test::ScopedTaskEnvironment::MainThreadType main_thread_type) {
			...
			public:
				QuitAfterMultipleDelegate(HANDLE event, int iterations)	: event_(event), iterations_(iterations) {
				}
				void OnObjectSignaled(HANDLE object) override {
					if (--iterations_) {
						SetEvent(event_);
			...
			void RunTest_ExecuteMultipleTimes(test::ScopedTaskEnvironment::MainThreadType main_thread_type) {
				//test::ScopedTaskEnvironment scoped_task_environment(main_thread_type);
		...
		TEST(ObjectWatcherTest, SignalBeforeWatch) {
			RunTest_SignalBeforeWatch(test::ScopedTaskEnvironment::MainThreadType::DEFAULT);
		...
		TEST(ObjectWatcherTest, OutlivesTaskEnvironment) {
			RunTest_OutlivesTaskEnvironment(test::ScopedTaskEnvironment::MainThreadType::DEFAULT);
			RunTest_OutlivesTaskEnvironment(test::ScopedTaskEnvironment::MainThreadType::IO);
			RunTest_OutlivesTaskEnvironment(test::ScopedTaskEnvironment::MainThreadType::UI);
		}

		TEST(ObjectWatcherTest, ExecuteMultipleTimes) {
			RunTest_ExecuteMultipleTimes(test::ScopedTaskEnvironment::MainThreadType::DEFAULT);
