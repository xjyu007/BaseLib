...
namespace base {
	namespace internal {
		...
		TimerBase::TimerBase(const Location& posted_from, TimeDelta delay) : TimerBase(posted_from, delay, nullptr) {
		}

		TimerBase::TimerBase(const Location& posted_from, TimeDelta delay, const TickClock* tick_clock)
			: scheduled_task_(nullptr), posted_from_(posted_from), delay_(delay), tick_clock_(tick_clock), is_running_(false) {
		...
		void TimerBase::PostNewScheduledTask(TimeDelta delay) {
			...
			if (delay > TimeDelta::FromMicroseconds(0)) {
				...
				GetTaskRunner()->PostDelayedTask(posted_from_, BindOnce(&BaseTimerTaskInternal::Run, Owned(scheduled_task_)), delay);
				scheduled_run_time_ = desired_run_time_ = Now() + delay;
			}
			else {
				GetTaskRunner()->PostTask(posted_from_, BindOnce(&BaseTimerTaskInternal::Run, Owned(scheduled_task_)));
