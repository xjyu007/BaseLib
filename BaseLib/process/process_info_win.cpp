...
namespace base {
	...
	IntegrityLevel GetCurrentProcessIntegrityLevel() {
		const auto process_token(GetCurrentProcessToken());
		...
		const auto token_label_bytes = std::make_unique<char[]>(token_info_length);
		const auto token_label = reinterpret_cast<TOKEN_MANDATORY_LABEL*>(token_label_bytes.get());
		...
		const auto integrity_level = *::GetSidSubAuthority(token_label->Label.Sid, static_cast<DWORD>(*::GetSidSubAuthorityCount(token_label->Label.Sid) - 1));
