		...
		namespace internal {
			...
			public:
				...
				static std::unique_ptr<SequenceManagerImpl> CreateOnCurrentThread(
					Settings settings = SequenceManager::Settings());
				...
				[[nodiscard]] TimeDomain* GetRealTimeDomain() const override;
				[[nodiscard]] const TickClock* GetTickClock() const override;
				[[nodiscard]] TimeTicks NowTicks() const override;
				...
				[[nodiscard]] const MetricRecordingSettings& GetMetricRecordingSettings() const override;
				[[nodiscard]] size_t GetPendingTaskCountForTesting() const override;
				...
				[[nodiscard]] bool IsBoundToCurrentThread() const;
				[[nodiscard]] MessagePump* GetMessagePump() const;
				[[nodiscard]] bool IsType(MessagePumpType type) const;
				void SetAddQueueTimeToTasks(bool enable);
				void SetTaskExecutionAllowed(bool allowed) const;
				[[nodiscard]] bool IsTaskExecutionAllowed() const;
#if defined(OS_IOS)
				...
				bool HasTasks();
				[[nodiscard]] MessagePumpType GetType() const;

				// Requests that a task to process work is scheduled.
				void ScheduleWork() const;
				...
				void SetNextDelayedDoWork(LazyNow* lazy_now, TimeTicks run_time) const;

				// Returns the currently executing TaskQueue if any. Must be called on the
				// thread this class was created on.
				[[nodiscard]] TaskQueueImpl* currently_executing_task_queue() const;
				...
				[[nodiscard]] const scoped_refptr<AssociatedThreadId>& associated_thread() const {
			...
			private:
				...
				void WillQueueTask(Task* pending_task, const char* task_queue_name) const;
				...
				bool GetAddQueueTimeToTasks() const;
				...
				// native work.
				[[nodiscard]] bool ShouldRunTaskOfPriority(TaskQueue::QueuePriority priority) const;
				...
				[[nodiscard]] const MainThreadOnly& main_thread_only() const {
					DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
					return main_thread_only_;
				}
