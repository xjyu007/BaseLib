...
namespace device {
	...
	namespace {
		...
		bool GetProperty(HDEVINFO dev_info,
			...
			const std::unique_ptr<wchar_t[]> buffer(new wchar_t[buffer_size]);
		...
		base::FilePath FixUpPortName(std::string_view port_name) {
...
#undef min
	...
	std::vector<SerialPortInfo*> SerialDeviceEnumeratorWin::GetDevices() {
		auto devices = GetDevicesNew();
		auto old_devices = GetDevicesOld();
		...
		for (const auto& device : devices) {
			const auto inserted = devices_seen.insert(device->path).second;
			DCHECK(inserted);
		}
	...
	// static
	std::optional<base::FilePath> SerialDeviceEnumeratorWin::GetPath(
	...
	std::vector<SerialPortInfo*>
		...
		for (DWORD i = 0; SetupDiEnumDeviceInfo(dev_info, i, &dev_info_data); i++) {
			...
			auto path = GetPath(friendly_name);
			if (!path)
				continue;

			auto info = new SerialPortInfo;
			info->path = *path;
