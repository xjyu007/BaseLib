...
namespace base::subtle {
	...
	namespace {
		...
		HANDLE CreateFileMappingWithReducedPermissions(SECURITY_ATTRIBUTES* sa, size_t rounded_size, LPCWSTR name) {
			...
			if (!h) {
				LogError(PlatformSharedMemoryRegion::CreateError::CREATE_FILE_MAPPING_FAILURE, GetLastError());
				return nullptr;
			}
			...
			if (!success) {
				LogError(PlatformSharedMemoryRegion::CreateError::REDUCE_PERMISSIONS_FAILURE, GetLastError());
				return nullptr;
			}
	...
	PlatformSharedMemoryRegion
		PlatformSharedMemoryRegion::TakeFromSharedMemoryHandle(
			const SharedMemoryHandle& handle, Mode mode) {
