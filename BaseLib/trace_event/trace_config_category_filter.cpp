...
#include "trace_event/trace_config_category_filter.h"
...
namespace base::trace_event
{
		...
		void TraceConfigCategoryFilter::InitializeFromString(
			...
			for (const std::string_view& category : split) {
				...
				if (category.front() == '-') {
					...
					excluded_categories_.push_back(category.substr(1).data());
				}
				else if (category.substr(0, sizeof(TRACE_DISABLED_BY_DEFAULT(""))).data() == TRACE_DISABLED_BY_DEFAULT("")) {
					//else if (category.starts_with(TRACE_DISABLED_BY_DEFAULT(""))) {
					disabled_categories_.push_back(category.data());
				}
				else {
					included_categories_.push_back(category.data());
				}
			}
		}
		...
		bool TraceConfigCategoryFilter::IsCategoryGroupEnabled(const std::string_view& category_group_name) const {
			bool had_enabled_by_default = false;
			DCHECK(!category_group_name.empty());
			std::string cgstr(category_group_name.data(), category_group_name.size());
			StringTokenizer category_group_tokens(cgstr.begin(), cgstr.end(), ",");
