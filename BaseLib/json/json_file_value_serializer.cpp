...
bool JSONFileValueSerializer::SerializeAndOmitBinaryValues(
	const base::Value& root) const {
	return SerializeInternal(root, true);
}

bool JSONFileValueSerializer::SerializeInternal(const base::Value& root,
	bool omit_binary_values) const {
	std::string json_string;
	JSONStringValueSerializer serializer(&json_string);
	serializer.set_pretty_print(true);
	const auto result = omit_binary_values ?
		serializer.SerializeAndOmitBinaryValues(root) :
		serializer.Serialize(root);
	if (!result)
		return false;

	const auto data_size = static_cast<int>(json_string.size());
	return WriteFile(json_file_path_, json_string.data(), data_size) == data_size;
}
...
std::unique_ptr<base::Value> JSONFileValueDeserializer::Deserialize(
	...
	std::string json_string;
	const auto error = ReadFileToString(&json_string);
