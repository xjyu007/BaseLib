...
#include "test/task_environment.h"
...
#include "test_mock_time_task_runner.h"
...
namespace base::test {
	...
	namespace {
		...
		CreateSequenceManagerForMainThreadType(
			TaskEnvironment::MainThreadType main_thread_type) {
			const auto type = GetMessagePumpTypeForMainThreadType(main_thread_type);
				return CreateSequenceManagerOnCurrentThreadWithPump(
	...
	public:
		...
		std::optional<TimeDelta> DelayTillNextTask(
			...
			const auto run_time = NextScheduledRunTime();
		...
		NextTaskSource FastForwardToNextTaskOrCap(TimeTicks fast_forward_cap) {
			...
			std::optional<TimeTicks> next_main_thread_task_time = NextScheduledRunTime();
	...
	void TaskEnvironment::InitializeThreadPool() {
		...
		// BlockingCall. It also allows enough concurrency to allow TSAN to spot
		// data races.
	...
	void TaskEnvironment::CompleteInitialization() const {
	...
	void TaskEnvironment::DestroyThreadPool() const {
		DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
	...
	scoped_refptr<base::SingleThreadTaskRunner>
		TaskEnvironment::GetMainThreadTaskRunner() const {
		DCHECK(task_runner_);
		return task_runner_;
	}
	...
	void TaskEnvironment::RunUntilIdle() {
		...
		const auto could_run_tasks = task_tracker_->AllowRunTasks();
...
// this call.
	...
	TimeDelta TaskEnvironment::NextMainThreadPendingTaskDelay() const {
		...
		sequence_manager_->ReclaimMemory();
		DCHECK(mock_time_domain_);
		std::optional<TimeTicks> run_time = mock_time_domain_->NextScheduledRunTime();
		if (run_time)
	...
	bool TaskEnvironment::NextTaskIsDelayed() const {
		...
		const auto delay = NextMainThreadPendingTaskDelay();
