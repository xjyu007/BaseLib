...
#include "metrics/dummy_histogram.h"
...
namespace base {
	...
	namespace {
		...
		bool ValidateRangeChecksum(const HistogramBase& histogram,
			...
			if (histogram.GetHistogramType() == DUMMY_HISTOGRAM)
				return true;
			const auto& casted_histogram =
				dynamic_cast<const Histogram&>(histogram);
	...
	protected:
		...
		virtual BucketRanges* CreateRanges() {
			const auto ranges = new BucketRanges(bucket_count_ + 1);
			InitializeBucketRanges(minimum_, maximum_, ranges);
			return ranges;
		}
	...
	void Histogram::InitializeBucketRanges(Sample minimum,
										   Sample maximum,
										   BucketRanges* ranges) {
		const auto log_max = log(static_cast<double>(maximum));
		size_t bucket_index = 1;
		auto current = minimum;
		ranges->set_range(bucket_index, current);
		const auto bucket_count = ranges->bucket_count();
		...
		while (bucket_count > ++bucket_index) {
			...
			const auto log_ratio = (log_max - log_current) / (bucket_count - bucket_index);
			// See where the next bucket would start.
			const auto log_next = log_current + log_ratio;
			const auto next = static_cast<int>(std::round(exp(log_next)));
			if (next > current)
				current = next;
			else
	...
	uint32_t Histogram::FindCorruption(const HistogramSamples& samples) const {
		...
		for (uint32_t index = 0; index < bucket_count(); ++index) {
			const auto new_range = ranges(index);
		...
		const int64_t delta64 = samples.redundant_count() - samples.TotalCount();
		if (delta64 != 0) {
			auto delta = static_cast<int>(delta64);
			if (delta != delta64)
	...
	void Histogram::WriteAsciiImpl(bool graph_it,
		...
		const auto snapshot = SnapshotAllSamples();
		const auto sample_count = snapshot->TotalCount();
		...
		auto largest_non_empty_bucket = bucket_count() - 1;
		...
		for (uint32_t i = 0; i < bucket_count(); ++i) {
			if (snapshot->GetCountAtIndex(i)) {
				const auto width = GetAsciiBucketRange(i).size() + 1;
		...
		for (uint32_t i = 0; i < bucket_count(); ++i) {
			const auto current = snapshot->GetCountAtIndex(i);
			...
			const auto current_size = GetBucketSize(current, i);
			if (graph_it)
	...
	double Histogram::GetPeakBucketSize(const SampleVectorBase& samples) const {
		double max = 0;
		for (uint32_t i = 0; i < bucket_count(); ++i) {
			const auto current_size = GetBucketSize(samples.GetCountAtIndex(i), i);
			if (current_size > max)
				max = current_size;
		}
		return max;
	...
	void Histogram::WriteAsciiHeader(const SampleVectorBase& samples,
		...
		if (sample_count == 0) {
			...
			const double mean = static_cast<float>(samples.sum()) / sample_count;
	...
	void Histogram::WriteAsciiBucketContext(int64_t past,
											Count current,
											int64_t remaining,
											uint32_t i,
											std::string* output) const {
		const auto scaled_sum = (past + current + remaining) / 100.0;
		WriteAsciiBucketValue(current, scaled_sum, output);
		if (0 < i) {
			const auto percentage = past / scaled_sum;
	...
	void Histogram::GetCountAndBucketData(Count* count, 
		...
		*sum = snapshot->sum();
		uint32_t index = 0;
		for (uint32_t i = 0; i < bucket_count(); ++i) {
			const auto count_at_index = snapshot->GetCountAtIndex(i);
			if (count_at_index > 0) {
	...
	protected:
		...
		void FillHistogram(HistogramBase* base_histogram) override {
			...
			if (base_histogram->GetHistogramType() == DUMMY_HISTOGRAM)
				return;
			auto histogram = dynamic_cast<LinearHistogram*>(base_histogram);
			// Set range descriptions.
			if (descriptions_) {
				for (auto i = 0; descriptions_[i].description; ++i) {
	...
	HistogramBase* LinearHistogram::FactoryGetWithRangeDescription(
		...
		const auto valid_arguments = InspectConstructionArguments(
	...
	const std::string LinearHistogram::GetAsciiBucketRange(uint32_t i) const {
		...
		const auto it = bucket_description_.find(range);
	...
	HistogramBase* LinearHistogram::DeserializeInfoImpl(PickleIterator* iter) {
		...
		const auto histogram = FactoryGet(
			histogram_name, declared_min, declared_max, bucket_count, flags);
		if (!histogram)
			return nullptr;
	...
	ScaledLinearHistogram::ScaledLinearHistogram(const char* name,
												 ...
												 int32_t scale,
												 int32_t flags)
		: histogram_(dynamic_cast<LinearHistogram*>(
	...
	void ScaledLinearHistogram::AddScaledCount(Sample value, int count) {
		...
		const auto max_value = static_cast<int32_t>(histogram_->bucket_count() - 1);
		if (value > max_value)
			value = max_value;
		if (value < 0)
			value = 0;

		auto scaled_count = count / scale_;
		auto remainder = count - scaled_count * scale_;
	...
	protected:
		BucketRanges* CreateRanges() override {
			const auto ranges = new BucketRanges(3 + 1);
			InitializeBucketRanges(1, 2, ranges);
			return ranges;
		}
	...
	protected:
		BucketRanges* CreateRanges() override {
			...
			auto ranges = *custom_ranges_;
			ranges.push_back(0);  // Ensure we have a zero value.
			ranges.push_back(HistogramBase::kSampleType_MAX);
			std::sort(ranges.begin(), ranges.end());
			ranges.erase(std::unique(ranges.begin(), ranges.end()), ranges.end());

			auto* bucket_ranges = new BucketRanges(ranges.size());
	...
	// static
	std::vector<Sample> CustomHistogram::ArrayToCustomEnumRanges(span<const Sample> values) {
	...
	HistogramBase* CustomHistogram::DeserializeInfoImpl(PickleIterator* iter) {
		...
		for (auto& sample_range : sample_ranges){
			if (!iter->ReadInt(&sample_range))
				return nullptr;
		}
	...
	bool CustomHistogram::ValidateCustomRanges(const std::vector<Sample>& custom_ranges) {
		auto has_valid_range = false;
		for (auto sample : custom_ranges){
