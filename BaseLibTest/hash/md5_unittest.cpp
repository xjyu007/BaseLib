...
namespace base {
	...
	TEST(MD5, DigestToBase16) {
		...
		const auto actual = MD5DigestToBase16(digest);
	...
	TEST(MD5, MD5SumLongData) {
		const auto length = 10 * 1024 * 1024 + 1;
		const std::unique_ptr<char[]> data(new char[length]);
	...
	TEST(MD5, ContextWithLongData) {
		...
		auto total = 0;
		while (total < length) {
			auto len = 4097;  // intentionally not 2^k.
			if (len > length - total)
				len = length - total;

			MD5Update(&ctx,
				std::string_view(reinterpret_cast<char*>(data.get() + total), len));
			total += len;
		}
