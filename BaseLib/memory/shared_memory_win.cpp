...
namespace base {
	namespace {
		...
		HANDLE CreateFileMappingWithReducedPermissions(SECURITY_ATTRIBUTES* sa, 
													   size_t rounded_size, 
													   LPCWSTR name) {
			const auto h = CreateFileMapping(INVALID_HANDLE_VALUE, sa, PAGE_READWRITE, 0, 
	...
	bool SharedMemory::Create(const SharedMemoryCreateOptions& options) {
		...
		const auto rounded_size = (options.size + kSectionMask) & ~kSectionMask;
