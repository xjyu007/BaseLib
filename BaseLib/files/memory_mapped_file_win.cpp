...
namespace base {
	...
	bool MemoryMappedFile::MapImageToMemory(Access access) {
		...
		const win::PEImage pe_image(data_);
		length_ = pe_image.GetNTHeaders()->OptionalHeader.SizeOfImage;

		return true;
	}

	bool MemoryMappedFile::MapFileRegionToMemory(const Region& region, Access access) {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		...
		file_mapping_.Set(::CreateFileMapping(file_.GetPlatformFile(), nullptr, flags,
		...
		if (region == Region::kWholeFile) {
			...
			const auto file_len = file_.GetLength();
			...
			CalculateVMAlignedBoundaries(region.offset, region.size, &aligned_start,
				&ignored, &data_offset);
			const int64_t full_map_size = data_offset + region.size;
