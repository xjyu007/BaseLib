...
#include "task/thread_pool/worker_thread.h"
...
namespace base::internal {
	...
	namespace {
		...
		public:
			...
			bool IsCurrentThreadSameAsSetThread() const {
		...
		public:
			...
			void WaitForWork(WaitableEvent* wake_up_event) override {
				...
				const auto sleep_time = GetSleepTimeout();
				const auto milliseconds_wait = checked_cast<DWORD>(
					sleep_time.is_max() ? INFINITE : sleep_time.InMilliseconds());
				const auto wake_up_event_handle = wake_up_event->handle();
				MsgWaitForMultipleObjectsEx(1, &wake_up_event_handle, milliseconds_wait,
					QS_ALLINPUT, 0);
			}

		private:
			RegisteredTaskSource GetWorkFromWindowsMessageQueue() const {
				MSG msg;
				if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE) != FALSE) {
					...
					if (task_tracker()->WillPostTask(
						...
						const auto transaction = message_pump_sequence_->BeginTransaction();
						const auto sequence_should_be_queued = transaction.WillPushTask();
	...
	void PooledSingleThreadTaskRunnerManager::Start(
		...
		for (const auto& worker : workers_to_start) {
	...
	void PooledSingleThreadTaskRunnerManager::UnregisterWorkerThread(
		...
		scoped_refptr<WorkerThread> worker_to_destroy;
		{
			...
			const auto worker_iter = std::find(workers_.begin(), workers_.end(), worker);
	...
	void PooledSingleThreadTaskRunnerManager::ReleaseSharedWorkerThreads() {
		...
		for (size_t i = 0; i < size(local_shared_worker_threads); ++i) {
