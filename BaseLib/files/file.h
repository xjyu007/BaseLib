...
namespace base {
  ...
  defined(OS_FUCHSIA) || (defined(OS_ANDROID) && __ANDROID_API__ < 21)
	...
	public:
		...
		//   if (!file.IsValid())
		//     return;
		[[nodiscard]] Error error_details() const { return error_details_; }

		[[nodiscard]] PlatformFile GetPlatformFile() const;
		...
		// an error.
		int Read(int64_t offset, char* data, int size) const;

		// Same as above but without seek.
		int ReadAtCurrentPos(char* data, int size) const;

		// Reads the given number of bytes (or until EOF is reached) starting with the
		// given offset, but does not make any effort to read all data on all
		// platforms. Returns the number of bytes read, or -1 on error.
		int ReadNoBestEffort(int64_t offset, char* data, int size) const;

		// Same as above but without seek.
		int ReadAtCurrentPosNoBestEffort(char* data, int size) const;
		...
		// with FLAG_APPEND.
		int Write(int64_t offset, const char* data, int size) const;

		// Save as above but without seek.
		int WriteAtCurrentPos(const char* data, int size) const;

		// Save as above but does not make any effort to write all data on all
		// platforms. Returns the number of bytes written, or -1 on error.
		int WriteAtCurrentPosNoBestEffort(const char* data, int size) const;
		...
		[[nodiscard]] bool SetLength(int64_t length) const;
		...
		[[nodiscard]] bool SetTimes(Time last_access_time, Time last_modified_time) const;
		...
		[[nodiscard]] Error Lock(LockMode mode = LockMode::kExclusive) const;
