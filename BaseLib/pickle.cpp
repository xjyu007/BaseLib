...
namespace base {
	...
	inline void PickleIterator::Advance(size_t size) {
		const auto aligned_size = bits::Align(size, sizeof(uint32_t));
	...
	inline const char* PickleIterator::GetReadPointerAndAdvance() {
		...
		const auto current_read_ptr = payload_ + read_index_;
		Advance(sizeof(Type));
	...
	bool PickleIterator::ReadFloat(float* result) {
		...
		// into the result.
		const auto read_from = GetReadPointerAndAdvance<float>();
		if (!read_from)
			return false;
		memcpy(result, read_from, sizeof(*result));
		return true;
	...
	bool PickleIterator::ReadDouble(double* result) {
		...
		// cause SIGBUS on some ARM platforms, so force using memcpy to copy the data
		// into the result.
		const auto read_from = GetReadPointerAndAdvance<double>();
		if (!read_from)
			return false;
		memcpy(result, read_from, sizeof(*result));
		return true;
	...
	bool PickleIterator::ReadString(std::string* result) {
		...
		if (!ReadInt(&len))
			return false;
		const auto read_from = GetReadPointerAndAdvance(len);
		if (!read_from)
			return false;
	...
	bool PickleIterator::ReadString16(std::wstring* result) {
		...
		if (!ReadInt(&len))
			return false;
		const auto read_from = GetReadPointerAndAdvance(len, sizeof(wchar_t));
		if (!read_from)
			return false;
	...
	bool PickleIterator::ReadStringPiece16(std::wstring_view* result) {
		int len;
		if (!ReadInt(&len))
			return false;
		const auto read_from = GetReadPointerAndAdvance(len, sizeof(wchar_t));
		...
		*result = std::wstring_view(reinterpret_cast<const wchar_t*>(read_from), len);
		return true;
	}
	...
	bool PickleIterator::ReadBytes(const char** data, int length) {
		const auto read_from = GetReadPointerAndAdvance(length);
		if (!read_from)
			return false;
		*data = read_from;
		return true;
	...
	void Pickle::WriteString(const std::string_view& value) {
		WriteInt(static_cast<int>(value.size()));
		WriteBytes(value.data(), static_cast<int>(value.size()));
	}

	void Pickle::WriteString16(const std::wstring_view& value) {
		WriteInt(static_cast<int>(value.size()));
		WriteBytes(value.data(), static_cast<int>(value.size()) * sizeof(wchar_t));
	}
	...
	void Pickle::Reserve(size_t length) {
		const auto data_len = bits::Align(length, sizeof(uint32_t));
		...
		const auto new_size = write_offset_ + data_len;
	...
	inline void* Pickle::ClaimUninitializedBytesInternal(size_t length) {
		...
		const auto new_size = write_offset_ + data_len;
		if (new_size > capacity_after_header_) {
			auto new_capacity = capacity_after_header_ * 2;
		...
		const auto write = mutable_payload() + write_offset_;
	...
	inline void Pickle::WriteBytesCommon(const void* data, size_t length) {
		...
		const auto write = ClaimUninitializedBytesInternal(length);
