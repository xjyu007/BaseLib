...
#include "pch.h"
#include "test/launcher/test_launcher.h"
...
#include "test/launcher/test_launcher_tracer.h"
#include "test/launcher/test_results_tracker.h"
...
#include "base_paths_fuchsia.h"
#include "fuchsia/default_job.h"
#include "fuchsia/file_utils.h"
#include "fuchsia/fuchsia_logging.h"
...
namespace base {
	...
	namespace {
		...
		void DoLaunchChildTestProcess(
			...
			auto was_timeout = false;
			auto exit_code = LaunchChildTestProcessWithOptions(
	...
	public:
		...
		[[nodiscard]] std::string GetDisabledStrippedName() const;

		// Returns full test name.
		[[nodiscard]] std::string GetFullName() const;

		// Returns test name with PRE_ prefix added, excluding DISABLE_ prefix.
		[[nodiscard]] std::string GetPreName() const;

		// Returns test name excluding DISABLED_ and PRE_ prefixes.
		[[nodiscard]] std::string GetPrefixStrippedName() const;

		[[nodiscard]] const std::string& test_case_name() const { return test_case_name_; }
	...
	bool TestLauncher::TestInfo::pre_test() const {
		return pre_test_;
	}
	...
	void TestLauncher::RunTests() {
		...
		for (const TestInfo& test_info : tests_) {
			...
			bool will_run_test = launcher_delegate_->WillRunTest(test_info.test_case_name(), test_info.test_name());
			if (!will_run_test)
				continue;
			...
			if (Hash(prefix_stripped_name) % total_shards_ != static_cast<uint32_t>(shard_index_)) {
				continue;
			}
