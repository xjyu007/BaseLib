...
#include "pch.h"
#include "containers/unique_ptr_adapters.h"
#include <memory>
#include <vector>
...
namespace base {
	namespace {
		...
		TEST(UniquePtrMatcherTest, Deleter) {
			...
			{
				auto iter = std::find_if(v.begin(), v.end(), UniquePtrMatcher<Foo, TestDeleter>(foo1));
				ASSERT_TRUE(iter != v.end());
				EXPECT_EQ(foo1, iter->get());
			}

			{
				auto iter = std::find_if(v.begin(), v.end(), UniquePtrMatcher<Foo, TestDeleter>(foo2));
				ASSERT_TRUE(iter != v.end());
				EXPECT_EQ(foo2, iter->get());
			}

			{
				auto iter = std::find_if(v.begin(), v.end(), MatchesUniquePtr<Foo, TestDeleter>(foo2));
				ASSERT_TRUE(iter != v.end());
				EXPECT_EQ(foo2, iter->get());
			}
		}
