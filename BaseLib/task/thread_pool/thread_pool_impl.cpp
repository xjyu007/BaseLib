...
#include "task/thread_pool/service_thread.h"
...
#undef min
		...
		const auto max_best_effort_tasks =
		...
		if (FeatureList::IsEnabled(kUseNativeThreadPool)) {
			auto pool = std::move(foreground_thread_group_);
	...
	bool ThreadPoolImpl::PostDelayedTask(const Location& from_here,
		...
		const auto new_traits = SetUserBlockingPriorityIfNeeded(traits);
	...
	bool ThreadPoolImpl::PostTaskWithSequenceNow(Task task,
		...
		if (task_source) {
			const auto traits = transaction.traits();
