...
#include "trace_event/memory_dump_scheduler.h"
...
namespace base::trace_event {
	...
	void MemoryDumpManager::RegisterDumpProviderInternal(
		...
		const auto whitelisted_for_background_mode = IsMemoryDumpProviderWhitelisted(name);

		const scoped_refptr<MemoryDumpProviderInfo> mdpinfo =
			new MemoryDumpProviderInfo(mdp, name, std::move(task_runner), options,
				whitelisted_for_background_mode);

		{
			AutoLock lock(lock_);
			const auto already_registered = !dump_providers_.insert(mdpinfo).second;
	...
	scoped_refptr<SequencedTaskRunner>
		...
		const auto started = dump_thread_->Start();
		CHECK(started);
	...
	void MemoryDumpManager::ContinueAsyncProcessDump(
		...
		while (!pmd_async_state->pending_dump_providers.empty()) {
			...
			const auto did_post_task = task_runner->PostTask(
				FROM_HERE,
