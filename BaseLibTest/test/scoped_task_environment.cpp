...
namespace base::test {
	...
	public:
		...
		std::optional<TimeDelta> DelayTillNextTask(
			...
			std::optional<TimeTicks> run_time = NextScheduledRunTime();
	...
	void ScopedTaskEnvironment::CompleteInitialization() const {
	...
	void ScopedTaskEnvironment::DestroyThreadPool() const {
		DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
	...
	scoped_refptr<base::SingleThreadTaskRunner>
		ScopedTaskEnvironment::GetMainThreadTaskRunner() const {
		DCHECK(task_runner_);
		return task_runner_;
	}
	...
	void ScopedTaskEnvironment::RunUntilIdle() const {
		DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
