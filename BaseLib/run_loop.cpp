...
namespace base {
	...
	void RunLoop::Run() {
		...
		const auto run_timeout = ScopedRunTimeoutForTestTLS().Get();
		if (run_timeout) {
		...
		const auto application_tasks_allowed =
	...
	bool RunLoop::IsRunningOnCurrentThread() {
		const auto delegate = GetTlsDelegate().Get();
	...
	void RunLoop::AddNestingObserverOnCurrentThread(NestingObserver* observer) {
		auto delegate = GetTlsDelegate().Get();
		DCHECK(delegate);
