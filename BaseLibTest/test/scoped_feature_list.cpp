...
namespace base::test
{
		...
		namespace {
			...
			std::vector<std::string_view> GetFeatureVector(const std::vector<Feature>& features) {
			...
			std::vector<std::string_view> GetFeatureVectorFromFeaturesAndParams(
			...
			std::string GetFeatureName(std::string_view feature) {
				std::string feature_name(feature.data());
				...
				const auto index = feature_name.find("<");
			...
			struct Features {
				std::vector<std::string_view> enabled_feature_list;
				std::vector<std::string_view> disabled_feature_list;
			};
			...
			void OverrideFeatures(const std::string& features, FeatureList::OverrideState override_state, Features* merged_features) {
				auto features_list = SplitStringPiece(features, ",", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);

				for (auto feature : features_list) {
					auto feature_name = GetFeatureName(feature);
			...
			std::string HexDecodeString(const std::string& input) {
				...
				const auto result = HexStringToBytes(input, &bytes);
				DCHECK(result);
		...
		void ScopedFeatureList::InitWithFeaturesImpl(
			...
			//field_trial_list_ = std::make_unique<FieldTrialList>(std::make_unique<MockEntropyProvider>());
			...
			for (const auto& enabled_feature : enabled_features_and_params) {
				...
				const std::string trial_name = "scoped_feature_list_trial_for_" + feature_name;
				...
				field_trial_param_associator->ClearParamsForTesting(trial_name, kTrialGroup);
