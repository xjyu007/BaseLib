...
namespace base::win
{
		...
		TEST_F(ShortcutTest, CreateAndResolveShortcutProperties) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(file_1, link_properties_, SHORTCUT_CREATE_ALWAYS));
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(file_2, only_target_properties, SHORTCUT_CREATE_ALWAYS));
		...
		TEST_F(ShortcutTest, CreateAndResolveShortcut) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, only_target_properties, SHORTCUT_CREATE_ALWAYS));
		...
		TEST_F(ShortcutTest, ResolveShortcutWithArgs) {
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, link_properties_, SHORTCUT_CREATE_ALWAYS));
		...
		TEST_F(ShortcutTest, CreateShortcutWithOnlySomeProperties) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, target_and_args_properties,SHORTCUT_CREATE_ALWAYS));
		...
		TEST_F(ShortcutTest, UpdateShortcutVerifyProperties) {
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, link_properties_, SHORTCUT_CREATE_ALWAYS));
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, link_properties_2_, SHORTCUT_UPDATE_EXISTING));
		...
		TEST_F(ShortcutTest, UpdateShortcutUpdateOnlyTargetAndResolve) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, update_only_target_properties, SHORTCUT_UPDATE_EXISTING));
		...
		TEST_F(ShortcutTest, UpdateShortcutMakeDualMode) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, make_dual_mode_properties, SHORTCUT_UPDATE_EXISTING));
		...
		TEST_F(ShortcutTest, UpdateShortcutRemoveDualMode) {
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, link_properties_2_, SHORTCUT_CREATE_ALWAYS));

			ShortcutProperties remove_dual_mode_properties;
			remove_dual_mode_properties.set_dual_mode(false);

			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, remove_dual_mode_properties, SHORTCUT_UPDATE_EXISTING));
		...
		TEST_F(ShortcutTest, UpdateShortcutClearArguments) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, clear_arguments_properties, SHORTCUT_UPDATE_EXISTING));
		...
		TEST_F(ShortcutTest, FailUpdateShortcutThatDoesNotExist) {
			ASSERT_FALSE(CreateOrUpdateShortcutLink(link_file_, link_properties_, SHORTCUT_UPDATE_EXISTING));
			ASSERT_FALSE(PathExists(link_file_));
		}

		TEST_F(ShortcutTest, ReplaceShortcutAllProperties) {
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, link_properties_, SHORTCUT_CREATE_ALWAYS));
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, link_properties_2_, SHORTCUT_REPLACE_EXISTING));
			ValidateShortcut(link_file_, link_properties_2_);
		}
		...
		TEST_F(ShortcutTest, ReplaceShortcutSomeProperties) {
			...
			ASSERT_TRUE(CreateOrUpdateShortcutLink(link_file_, new_properties, SHORTCUT_REPLACE_EXISTING));
		...
		TEST_F(ShortcutTest, FailReplaceShortcutThatDoesNotExist) {
			ASSERT_FALSE(CreateOrUpdateShortcutLink(link_file_, link_properties_, SHORTCUT_REPLACE_EXISTING));
			ASSERT_FALSE(PathExists(link_file_));
		}
