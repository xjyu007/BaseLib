...
namespace device {
	...
	public:
		...
		virtual void Open(const SerialConnectionOptions& options,
		...
		void CancelRead(SerialReceiveError reason);

		// Attempts to cancel a pending write operation.
		void CancelWrite(SerialSendError reason);
		...
		// successfully read.
		virtual SerialPortControlSignals* GetControlSignals() const = 0;
		...
		virtual bool SetControlSignals(
			const SerialHostControlSignals& control_signals) = 0;

		// Performs platform-specific port configuration. Returns |true| iff
		// configuration was successful.
		bool ConfigurePort(const SerialConnectionOptions& options);
		...
		virtual SerialConnectionInfo* GetPortInfo() const = 0;
	...
	protected:
		...
		void ReadCompleted(int bytes_read, SerialReceiveError error);
		...
		// if the associated I/O operation was the only thing keeping it alive.
		void WriteCompleted(int bytes_written, SerialSendError error);
		...
		void QueueReadCompleted(int bytes_read, SerialReceiveError error);
		...
		// without being reentrant.
		void QueueWriteCompleted(int bytes_written, SerialSendError error);
		...
		SerialReceiveError read_cancel_reason() const {
		...
		SerialSendError write_cancel_reason() const {
			return write_cancel_reason_;
		}

		bool write_canceled() const { return write_canceled_; }

		const SerialConnectionOptions& options() const { return options_; }
	...
	private:
		...
		void MergeConnectionOptions(const SerialConnectionOptions& options);
		...
		SerialConnectionOptions options_;

		std::unique_ptr<WritableBuffer> pending_read_buffer_;
		SerialReceiveError read_cancel_reason_;
		bool read_canceled_;

		std::unique_ptr<ReadOnlyBuffer> pending_write_buffer_;
		SerialSendError write_cancel_reason_;
