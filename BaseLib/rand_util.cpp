...
namespace base {
	...
	int RandInt(int min, int max) {
		...
		const auto result = static_cast<int>(min + static_cast<int64_t>(base::RandGenerator(range)));
	...
	double BitsToOpenEndedUnitInterval(uint64_t bits) {
		...
		static const auto kBits = std::numeric_limits<double>::digits;
		const auto random_bits = bits & ((UINT64_C(1) << kBits) - 1);
		const auto result = ldexp(static_cast<double>(random_bits), -1 * kBits);
	...
	uint64_t RandGenerator(uint64_t range) {
		...
		const auto max_acceptable_value = (std::numeric_limits<uint64_t>::max() / range) * range - 1;
