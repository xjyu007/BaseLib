...
#include "hash/md5.h"
#include "../third_party/boringssl/md5.h"
...
namespace base {
	...
	void MD5Init(MD5Context* context) {
		auto ctx = reinterpret_cast<struct Context*>(context);
	...
	void MD5Update(MD5Context* context, const std::string_view& data) {
		...
		if (t) {
			const auto p = static_cast<uint8_t*>(ctx->in + t);
	...
	void MD5Sum(const void* data, size_t length, MD5Digest* digest) {
		MD5Context ctx;
		MD5Init(&ctx);
		MD5Update(&ctx, std::string_view(reinterpret_cast<const char*>(data), length));
		MD5Final(digest, &ctx);
	}
