...
namespace base {
	...
	namespace debug {
		...
		struct OwningProcess {
			...
			static bool GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp);
		...
		public:
			...
			public:
				...
				// Dereferencing the returned std::string_view when the memory is not accessible
				// will cause the program to SEGV!
				std::string_view GetReference() const;
				std::string_view GetStringReference() const;
			...
			private:
				...
				std::string_view ref_value_;   // Used to hold reference to external data.
			};
			...
			// recording depth.
			void Set(std::string_view name, const void* memory, size_t size) {
			...
			void SetString(std::string_view name, std::wstring_view value) {
				SetString(name, WideToUTF8(value));
			}
			void SetBool(std::string_view name, bool value) {
				char cvalue = value ? 1 : 0;
				Set(name, BOOL_VALUE, &cvalue, sizeof(cvalue));
			}
			void SetChar(std::string_view name, char value) {
			...
			void SetUint(std::string_view name, uint64_t value) {
			...
			// of information.
			void SetReference(std::string_view name, const void* memory, size_t size) {
				SetReference(name, RAW_VALUE_REFERENCE, memory, size);
			}
			void SetStringReference(std::string_view name, std::string_view value) {
			...
			// Explicitly sets the process ID.
			void SetOwningProcessIdForTesting(int64_t pid, int64_t stamp) const;
		...
		protected:
			virtual void Set(std::string_view name, ValueType type, const void* memory, size_t size);
		...
		private:
			...
			struct ValueInfo {
				...
				std::string_view name;                 // The "key" of the record.
			...
			void SetReference(std::string_view name, ValueType type, const void* memory, size_t size);
			...
			mutable std::map<std::string_view, ValueInfo> values_{};
		...
		public:
			...
			public:
				ScopedActivity(ThreadActivityTracker* tracker, const void* program_counter, const void* origin, Activity::Type type,
			...
			ActivityId PushActivity(const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data) const;
			...
			void ChangeActivity(ActivityId id, Activity::Type type, const ActivityData& data) const;

			// Indicates that an activity has completed.
			void PopActivity(ActivityId id) const;

			// Sets the user-data information for an activity.
			std::unique_ptr<ActivityUserData> GetUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator) const;
			...
			bool HasUserData(ActivityId id) const;

			// Release the user-data information for an activity.
			void ReleaseUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator) const;

			// Save an exception. |origin| is the location of the exception.
			void RecordExceptionActivity(const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data) const;
			...
			uint32_t GetDataVersionForTesting() const;
		...
		private:
			...
			std::unique_ptr<ActivityUserData> CreateUserDataForActivity(Activity* activity, ActivityTrackerMemoryAllocator* allocator) const;
  ...
  // asserts on running in the same sequence each time.
		...
		public:
			...
			using ProcessExitCallback =
				RepeatingCallback<void(int64_t process_id, int64_t exit_stamp, int exit_code, ProcessPhase exit_phase,
			...
			// purposes.
			static void CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator, int stack_depth, int64_t process_id);
			...
			static bool CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, std::string_view name, int stack_depth);
			...
			static bool CreateWithLocalMemory(size_t size, uint64_t id, std::string_view name, int stack_depth, int64_t process_id);
			...
			static bool CreateWithSharedMemory(base::WritableSharedMemoryMapping mapping, uint64_t id, std::string_view name, int stack_depth);

			// Gets the global activity-tracker or null if none exists.
			static GlobalActivityTracker* Get() {
				return reinterpret_cast<GlobalActivityTracker*>(subtle::Acquire_Load(&g_tracker_));
			}
			...
			void RecordLogMessage(std::string_view message) const;
			static void RecordLogMessageIfEnabled(std::string_view message) {
				GlobalActivityTracker* tracker = Get();
				if (tracker)
			...
			void RecordFieldTrial(const std::string& trial_name, std::string_view group_name);
			static void RecordFieldTrialIfEnabled(const std::string& trial_name, std::string_view group_name) {
				GlobalActivityTracker* tracker = Get();
				if (tracker)
		...
		private:
			...
			private:
				void Set(std::string_view name, ValueType type, const void* memory, size_t size) override;
			...
			void CleanupAfterProcess(int64_t process_id, int64_t exit_stamp, int exit_code, std::string&& command_line);
		...
		public:
			...
			//     ...
			//   }
			ALWAYS_INLINE ScopedActivity(uint8_t action, uint32_t id, int32_t info)
		...
		public:
			ALWAYS_INLINE explicit ScopedTaskRunActivity(const PendingTask& task)
		...
		public:
			ALWAYS_INLINE explicit ScopedLockAcquireActivity(const base::internal::LockImpl* lock)
		...
		public:
			ALWAYS_INLINE explicit ScopedEventWaitActivity(const WaitableEvent* event)
		...
		public:
			ALWAYS_INLINE explicit ScopedThreadJoinActivity(const PlatformThreadHandle* thread)
		...
		public:
			ALWAYS_INLINE explicit ScopedProcessWaitActivity(const Process* process) 
