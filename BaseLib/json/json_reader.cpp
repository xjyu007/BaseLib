...
namespace base {
	...
	std::unique_ptr<Value> JSONReader::ReadAndReturnErrorDeprecated(
		...
		auto ret = ReadAndReturnValueWithError(json, options);
		if (ret.value)
	...
	std::optional<Value> JSONReader::ReadToValue(std::string_view json) const {
		return parser_->Parse(json);
	}

	std::unique_ptr<Value> JSONReader::ReadToValueDeprecated(std::string_view json) const {
		auto value = parser_->Parse(json);
