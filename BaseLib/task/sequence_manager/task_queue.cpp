...
namespace base::sequence_manager {
	...
	namespace {
		...
		// TODO(kraynov): Move NullTaskRunner from //test to //base.
	...
	void TaskQueue::RemoveQueueEnabledVoter(bool voter_is_enabled) {
		...
		const auto was_enabled = AreAllQueueEnabledVotersEnabled();
		if (voter_is_enabled) {
			--enabled_voter_count_;
		...
		const auto is_enabled = AreAllQueueEnabledVotersEnabled();
	...
	std::optional<TimeTicks> TaskQueue::GetNextScheduledWakeUp() const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return std::nullopt;
	...
	void TaskQueue::SetQueuePriority(TaskQueue::QueuePriority priority) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::AddTaskObserver(TaskObserver* task_observer) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::RemoveTaskObserver(TaskObserver* task_observer) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::SetTimeDomain(TimeDomain* time_domain) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::SetBlameContext(trace_event::BlameContext* blame_context) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::InsertFence(InsertFencePosition position) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::InsertFenceAt(TimeTicks time) const {
		impl_->InsertFenceAt(time);
	}

	void TaskQueue::RemoveFence() const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
		impl_->RemoveFence();
	...
	bool TaskQueue::HasActiveFence() const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return false;
	...
	void TaskQueue::SetObserver(Observer* observer) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (!impl_)
			return;
	...
	void TaskQueue::SetShouldReportPostedTasksWhenDisabled(bool should_report) const {
