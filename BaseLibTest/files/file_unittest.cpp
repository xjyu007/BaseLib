...
TEST(FileTest, SelfSwap) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("create_file_1");
...
TEST(FileTest, Async) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("create_file");

	{
		const File file(file_path, base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_ASYNC);
		EXPECT_TRUE(file.IsValid());
		EXPECT_TRUE(file.async());
	}

	{
		const File file(file_path, base::File::FLAG_OPEN_ALWAYS);
		EXPECT_TRUE(file.IsValid());
...
TEST(FileTest, DeleteOpenFile) {
	...
	// Create a file.
	File file(file_path, base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_READ | base::File::FLAG_SHARE_DELETE);
	EXPECT_TRUE(file.IsValid());
	EXPECT_TRUE(file.created());
	EXPECT_EQ(base::File::FILE_OK, file.error_details());
...
TEST(FileTest, ReadWrite) {
	...
	for (auto i = 0; i < kTestDataSize; i++)
		EXPECT_EQ(data_to_write[i], data_read_2[i]);
	for (auto i = kTestDataSize; i < kOffsetBeyondEndOfFile; i++)
		EXPECT_EQ(0, data_read_2[i]);
	for (auto i = kOffsetBeyondEndOfFile; i >= file_size; i++)
...
TEST(FileTest, GetLastFileError) {
	...
	const auto nonexistent_path(temp_dir.GetPath().AppendASCII("nonexistent"));
	const File file(nonexistent_path, File::FLAG_OPEN | File::FLAG_READ);
	const auto last_error = File::GetLastFileError();
	EXPECT_FALSE(file.IsValid());
...
TEST(FileTest, Append) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("append_file");
	File file(file_path, base::File::FLAG_CREATE | base::File::FLAG_APPEND);
	ASSERT_TRUE(file.IsValid());

	char data_to_write[] = "test";
	const auto kTestDataSize = 4;

	// Write 0 bytes to the file.
	auto bytes_written = file.Write(0, data_to_write, 0);
	EXPECT_EQ(0, bytes_written);
...
TEST(FileTest, Length) {
	...
	// Extend the file.
	const auto kExtendedFileLength = 10;
	int64_t file_size = 0;
	...
	// Make sure the file was zero-padded.
	char data_read[32];
	auto bytes_read = file.Read(0, data_read, static_cast<int>(file_size));
	EXPECT_EQ(file_size, bytes_read);
	for (auto i = 0; i < kTestDataSize; i++)
		EXPECT_EQ(data_to_write[i], data_read[i]);
	for (auto i = kTestDataSize; i < file_size; i++)
		EXPECT_EQ(0, data_read[i]);

	// Truncate the file.
	const auto kTruncatedFileLength = 2;
...
TEST(FileTest, DISABLED_TouchGetInfo) {
	...
	auto now = base::Time::Now() + base::TimeDelta::FromSeconds(2);
	...
	char data[] = "test";
	const auto kTestDataSize = 4;
	auto bytes_written = file.Write(0, data, kTestDataSize);
	EXPECT_EQ(kTestDataSize, bytes_written);
	...
	auto new_last_accessed = info.last_accessed + base::TimeDelta::FromSeconds(234);
	auto new_last_modified = info.last_modified + base::TimeDelta::FromMinutes(567);
...
TEST(FileTest, ReadAtCurrentPosition) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("read_at_current_position");
	const File file(file_path, base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE);
	EXPECT_TRUE(file.IsValid());
...
TEST(FileTest, WriteAtCurrentPosition) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("write_at_current_position");
	const File file(file_path, File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE);
	EXPECT_TRUE(file.IsValid());
...
TEST(FileTest, Seek) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("seek_file");
	const File file(file_path, File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE);
	ASSERT_TRUE(file.IsValid());
...
TEST(FileTest, Duplicate) {
	base::ScopedTempDir temp_dir;
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("file");
	File file(file_path, (base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE));
	ASSERT_TRUE(file.IsValid());

	auto file2(file.Duplicate());
	ASSERT_TRUE(file2.IsValid());
...
TEST(FileTest, DuplicateDeleteOnClose) {
	...
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("file");
	File file(file_path, (File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_DELETE_ON_CLOSE));
	ASSERT_TRUE(file.IsValid());
	auto file2(file.Duplicate());
	ASSERT_TRUE(file2.IsValid());
	file.Close();
...
TEST(FileTest, MAYBE_WriteDataToLargeOffset) {
	...
	ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
	const auto file_path = temp_dir.GetPath().AppendASCII("file");
	const File file(file_path, (File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_DELETE_ON_CLOSE));
	ASSERT_TRUE(file.IsValid());
...
TEST(FileTest, DeleteNoop) {
	...
	File file(file_path, (File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_CAN_DELETE_ON_CLOSE));
	ASSERT_TRUE(file.IsValid());
	file.Close();
...
TEST(FileTest, IrrevokableDeleteOnClose) {
	...
	File file(file_path, (File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_DELETE_ON_CLOSE |
			File::FLAG_SHARE_DELETE | File::FLAG_CAN_DELETE_ON_CLOSE));
	ASSERT_TRUE(file.IsValid());
...
TEST(FileTest, IrrevokableDeleteOnCloseOther) {
	...
	File file2(file_path, (File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WRITE |
			File::FLAG_SHARE_DELETE | File::FLAG_CAN_DELETE_ON_CLOSE));
	ASSERT_TRUE(file2.IsValid());
...
TEST(FileTest, UnsharedDeleteOnClose) {
	...
	ASSERT_TRUE(file.IsValid());
	const File file2(file_path, (File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_DELETE_ON_CLOSE | 
