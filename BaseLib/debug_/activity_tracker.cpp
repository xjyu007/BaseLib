...
#undef max
	...
	void OwningProcess::Release_Initialize(int64_t pid) {
		const auto old_id = data_id.load(std::memory_order_acquire);
		DCHECK_EQ(0U, old_id);
	...
	// static
	bool OwningProcess::GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp) {
		const auto info = reinterpret_cast<const OwningProcess*>(memory);
		const auto id = info->data_id.load(std::memory_order_acquire);
		if (id == 0)
			return false;
	...
	ActivityTrackerMemoryAllocator::Reference ActivityTrackerMemoryAllocator::GetObjectReference() {
		...
		while (cache_used_ > 0) {
			const auto cached = cache_values_[--cache_used_];
		...
		// anything.
		const auto last = iterator_.GetLast();
		while (true) {
			uint32_t type;
			const auto found = iterator_.GetNext(&type);
		...
		const auto allocated = allocator_->Allocate(object_size_, object_type_);
	...
	void ActivityTrackerMemoryAllocator::ReleaseObjectReference(Reference ref) {
		// Mark object as free.
		const auto success = allocator_->ChangeType(ref, object_free_type_, object_type_,	/*clear=*/true);
		DCHECK(success);
	...
	void Activity::FillFrom(Activity* activity, const void* program_counter, const void* origin, Type type, const ActivityData& data) {
		...
		size_t stack_depth;
		const auto stack_addrs = stack_trace.Addresses(&stack_depth);
	...
	std::string_view ActivityUserData::TypedValue::Get() const {
		DCHECK_EQ(RAW_VALUE, type_);
		return long_value_;
	}

	std::string_view ActivityUserData::TypedValue::GetString() const {
	...
	std::string_view ActivityUserData::TypedValue::GetReference() const {
		DCHECK_EQ(RAW_VALUE_REFERENCE, type_);
		return ref_value_;
	}

	std::string_view ActivityUserData::TypedValue::GetStringReference() const {
	...
	bool ActivityUserData::CreateSnapshot(Snapshot* output_snapshot) const {
		...
		for (const auto& entry : values_) {
			...
			{
				const auto ref = reinterpret_cast<ReferenceRecord*>(entry.second.memory);
				value.ref_value_ = std::string_view(
			...
			const auto inserted = output_snapshot->insert(
				std::make_pair(entry.second.name.data(), std::move(value)));
	...
	void ActivityUserData::SetOwningProcessIdForTesting(int64_t pid, int64_t stamp) const {
		if (!header_)
			return;
	...
	// static
	bool ActivityUserData::GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp) {
		const auto* header = reinterpret_cast<const MemoryHeader*>(memory);
		return OwningProcess::GetOwningProcessId(&header->owner, out_id, out_stamp);
	}
#undef min
	void ActivityUserData::Set(std::string_view name, ValueType type, const void* memory, size_t size) {
		...
		if (name.length() > kMaxUserDataNameLength)
			name = std::string(name.data(), kMaxUserDataNameLength);
		...
		if (existing != values_.end()) {
			...
			const auto name_size = name.length();
			auto name_extent = RoundUpToAlignment(sizeof(FieldHeader) + name_size, kMemoryAlignment) - sizeof(FieldHeader);
			const auto value_extent = RoundUpToAlignment(size, kMemoryAlignment);

			// The "base size" is the size of the header and (padded) string key. Stop
			// now if there's not room enough for even this.
			auto base_size = sizeof(FieldHeader) + name_extent;
			if (base_size > available_)
				return;

			// The "full size" is the size for storing the entire value.
			auto full_size = std::min(base_size + value_extent, available_);
			...
			auto header = reinterpret_cast<FieldHeader*>(memory_);
			memory_ += full_size;
			...
			std::string_view persistent_name(name_memory, name_size);
		...
		info->size_ptr->store(static_cast<unsigned short>(size), std::memory_order_release);
	}

	void ActivityUserData::SetReference(std::string_view name, ValueType type, const void* memory, size_t size) {
		ReferenceRecord rec{};
	...
	void ActivityUserData::ImportExistingData() const {
		...
		while (available_ > sizeof(FieldHeader)) {
			auto header = reinterpret_cast<FieldHeader*>(memory_);
			const auto type = static_cast<ValueType>(header->type.load(std::memory_order_acquire));
			if (type == END_OF_VALUES)
				return;
			if (header->record_size > available_)
				return;

			auto value_offset = RoundUpToAlignment(sizeof(FieldHeader) + header->name_size, kMemoryAlignment);
			...
			ValueInfo info;
			info.name = std::string_view(memory_ + sizeof(FieldHeader), header->name_size);
			info.type = type;
			info.memory = memory_ + value_offset;
			info.size_ptr = &header->value_size;
			info.extent = header->record_size - value_offset;

			std::string_view key(info.name);
	...
	ThreadActivityTracker::ActivityId ThreadActivityTracker::PushActivity(
		...
		const auto depth = header_->current_depth.load(std::memory_order_relaxed);
	...
	void ThreadActivityTracker::ChangeActivity(ActivityId id, Activity::Type type, const ActivityData& data) const {
	...
	void ThreadActivityTracker::PopActivity(ActivityId id) const {
		...
		const auto depth = header_->current_depth.fetch_sub(1, std::memory_order_relaxed) - 1;
	...
	std::unique_ptr<ActivityUserData> ThreadActivityTracker::GetUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator) const {
	...
	bool ThreadActivityTracker::HasUserData(ActivityId id) const {
		// User-data is only stored for activities actually held in the stack.
		return (id < stack_slots_ && stack_[id].user_data_ref);
	}

	void ThreadActivityTracker::ReleaseUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator) const {
		// User-data is only stored for activities actually held in the stack.
	...
	bool ThreadActivityTracker::CreateSnapshot(Snapshot* output_snapshot) const {
		...
		for (auto attempt = 0; attempt < kMaxAttempts; ++attempt) {
			...
			const auto starting_id = header_->owner.data_id.load(std::memory_order_acquire);
			const auto starting_create_stamp = header_->owner.create_stamp;
			const auto starting_process_id = header_->owner.process_id;
			const auto starting_thread_id = header_->thread_ref.as_id;
			...
			const auto pre_version = header_->data_version.load(std::memory_order_seq_cst);
			...
			const auto count = std::min(depth, stack_slots_);
			...
			const auto start_time = Time::FromInternalValue(header_->start_time);
			const auto start_ticks = header_->start_ticks;
	...
	const void* ThreadActivityTracker::GetBaseAddress() const {
		return header_;
	}

	uint32_t ThreadActivityTracker::GetDataVersionForTesting() const {
		return header_->data_version.load(std::memory_order_relaxed);
	}

	void ThreadActivityTracker::SetOwningProcessIdForTesting(int64_t pid, int64_t stamp) const {
		header_->owner.SetOwningProcessIdForTesting(pid, stamp);
	}

	// static
	bool ThreadActivityTracker::GetOwningProcessId(const void* memory, int64_t* out_id, int64_t* out_stamp) {
		const auto header = reinterpret_cast<const Header*>(memory);
		return OwningProcess::GetOwningProcessId(&header->owner, out_id, out_stamp);
	}
	...
	bool ThreadActivityTracker::CalledOnValidThread() const {
#if DCHECK_IS_ON()
	...
	std::unique_ptr<ActivityUserData>
		ThreadActivityTracker::CreateUserDataForActivity(Activity* activity, ActivityTrackerMemoryAllocator* allocator) const {
		DCHECK_EQ(0U, activity->user_data_ref);

		const auto ref = allocator->GetObjectReference();
		void* memory = allocator->GetAsArray<char>(ref, kUserDataSize);
		if (memory) {
			auto user_data = std::make_unique<ActivityUserData>(memory, kUserDataSize);
	...
	bool GlobalActivityTracker::ModuleInfoRecord::DecodeTo(GlobalActivityTracker::ModuleInfo* info, size_t record_size) const {
		...
		const auto current_changes = changes.load(std::memory_order_acquire);
		...
		if (offsetof(ModuleInfoRecord, pickle) + pickle_size > record_size)
			return false;
		const Pickle pickler(pickle, pickle_size);
		PickleIterator iter(pickler);
		return iter.ReadString(&info->file) && iter.ReadString(&info->debug_file);
	}

	GlobalActivityTracker::ModuleInfoRecord* GlobalActivityTracker::ModuleInfoRecord::CreateFrom(const GlobalActivityTracker::ModuleInfo& info, PersistentMemoryAllocator* allocator) {
		Pickle pickler;
		pickler.WriteString(info.file);
		pickler.WriteString(info.debug_file);
		const auto required_size = offsetof(ModuleInfoRecord, pickle) + pickler.size();
		auto* record = allocator->New<ModuleInfoRecord>(required_size);
		if (!record)
			return nullptr;
		...
		const auto success = record->UpdateFrom(info);
		DCHECK(success);
		return record;
	}
	...
	bool GlobalActivityTracker::ModuleInfoRecord::UpdateFrom(const GlobalActivityTracker::ModuleInfo& info) {
		...
		const auto success = changes.compare_exchange_strong(new_changes, old_changes + 1,
	...
	GlobalActivityTracker::ScopedThreadActivity::ScopedThreadActivity(
		...
		const ActivityData& data,
		bool lock_allowed)
		: ScopedActivity(GetOrCreateTracker(lock_allowed),
			program_counter,
			origin,
			type,
			data) {
	...
	void GlobalActivityTracker::ThreadSafeUserData::Set(std::string_view name,
		ValueType type,
		const void* memory,
		size_t size) {
	...
	bool GlobalActivityTracker::CreateWithFile(const FilePath& file_path,
		...
		const auto success = mapped_file->Initialize(
	...
	ThreadActivityTracker* GlobalActivityTracker::CreateTrackerForCurrentThread() {
		...
		void* mem_base = allocator_->GetAsObject<ThreadActivityTracker::Header>(mem_reference);
	...
	void GlobalActivityTracker::CleanupAfterProcess(int64_t process_id,
		...
		if (process_exit_callback) {
			...
			const auto phase = process_data_snapshot.find(kProcessPhaseDataKey);
	...
	void GlobalActivityTracker::RecordLogMessage(std::string_view message) const {
		...
		const auto ref = allocator_->Allocate(message.size() + 1, kTypeIdGlobalLogMessage);
		const auto memory = allocator_->GetAsArray<char>(ref, kTypeIdGlobalLogMessage, message.size() + 1);
		if (memory) {
	...
	void GlobalActivityTracker::RecordModuleInfo(const ModuleInfo& info) {
		...
		const auto found = modules_.find(info.file);
		...
		auto record = ModuleInfoRecord::CreateFrom(info, allocator_.get());
		if (!record)
			return;
	...
	void GlobalActivityTracker::RecordFieldTrial(const std::string& trial_name, std::string_view group_name) {
		const auto key = std::string("FieldTrial.") + trial_name;
	...
	void GlobalActivityTracker::MarkDeleted() const {
		allocator_->SetMemoryState(PersistentMemoryAllocator::MEMORY_DELETED);
	}

	GlobalActivityTracker::GlobalActivityTracker(std::unique_ptr<PersistentMemoryAllocator> allocator, int stack_depth, int64_t process_id)
	...
	void GlobalActivityTracker::ReturnTrackerMemory(ManagedActivityTracker* tracker) {
		const auto mem_reference = tracker->mem_reference_;
		const auto mem_base = tracker->mem_base_;
		DCHECK(mem_reference);
		DCHECK(mem_base);
		...
		AutoLock autolock(thread_tracker_allocator_lock_);
	...
	void GlobalActivityTracker::RecordExceptionImpl(const void* pc, const void* origin, uint32_t code) {
		...
		// problem.
		const auto tracker = GetTrackerForCurrentThread();
		if (!tracker)
			return;
