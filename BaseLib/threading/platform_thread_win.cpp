...
#include "threading/platform_thread_win.h"
...
namespace base {
	...
	// thread mode is enabled on Windows 7.
		...
		DWORD __stdcall ThreadFunc(void* params) {
			const auto thread_params = static_cast<ThreadParams*>(params);
			auto delegate = thread_params->delegate;
			...
			const auto did_dup = DuplicateHandle(GetCurrentProcess(),
				GetCurrentThread(),
				GetCurrentProcess(),
				&platform_handle,
				0,
	...
	void PlatformThread::Sleep(TimeDelta duration) {
		...
		// real-time.
		const auto end = subtle::TimeTicksNowIgnoringOverride() + duration;
		for (auto now = subtle::TimeTicksNowIgnoringOverride(); now < end; 
	...
	void PlatformThread::Join(PlatformThreadHandle thread_handle) {
		...
		debug::ScopedThreadJoinActivity thread_activity(&thread_handle);
	...
	void PlatformThread::SetCurrentThreadPriorityImpl(ThreadPriority priority) {
		...
		const auto use_thread_mode_background = 
		...
		auto desired_priority = THREAD_PRIORITY_ERROR_RETURN;
		switch (priority) {
