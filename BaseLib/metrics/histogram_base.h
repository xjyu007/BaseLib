...
namespace base {
	...
	public:
		...
		[[nodiscard]] const char* histogram_name() const { return histogram_name_; }
		...
		virtual void CheckName(const std::string_view& name) const;

		// Get a unique ID for this histogram's samples.
		[[nodiscard]] virtual uint64_t name_hash() const = 0;

		// Operations with Flags enum.
		[[nodiscard]] int32_t flags() const { return subtle::NoBarrier_Load(&flags_); }
		void SetFlags(int32_t flags);
		void ClearFlags(int32_t flags);

		[[nodiscard]] virtual HistogramType GetHistogramType() const = 0;

		// Whether the histogram has construction arguments as parameters specified.
		// For histograms that don't have the concept of minimum, maximum or
		// bucket_count, this function always returns false.
		[[nodiscard]] virtual bool HasConstructionArguments(
		...
		[[nodiscard]] virtual uint32_t FindCorruption(const HistogramSamples& samples) const;
		...
		[[nodiscard]] virtual std::unique_ptr<HistogramSamples> SnapshotSamples() const = 0;
		...
		// read-only memory.
		[[nodiscard]] virtual std::unique_ptr<HistogramSamples> SnapshotFinalDelta() const = 0;
	...
	protected:
		...
		// to |sum|.
		virtual void GetCountAndBucketData(Count* count, int64_t* sum, ListValue* buckets) const = 0;

		//// Produce actual graph (set of blank vs non blank char's) for a bucket.
		void WriteAsciiBucketGraph(double current_size, double max_size, std::string* output) const;

		// Return a string description of what goes in a given bucket.
		[[nodiscard]] std::string GetSimpleAsciiBucketRange(Sample sample) const;
