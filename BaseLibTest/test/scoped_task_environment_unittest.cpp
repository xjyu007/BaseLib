...
#include "scoped_task_environment.h"
...
#include "threading/sequence_local_storage_slot.h"
#include "threading/thread_task_runner_handle.h"
#include "time/clock.h"
#include "time/default_clock.h"
...
namespace base::test
{
		...
		namespace {
			...
			class ScopedTaskEnvironmentTest : public testing::TestWithParam<ScopedTaskEnvironment::MainThreadType> {
			};
			...
			void RunUntilIdleTest(
				...
				ScopedTaskEnvironment scoped_task_environment(main_thread_type, execution_control_mode);
		...
		TEST_P(ScopedTaskEnvironmentTest, AsyncTasksRunAsTheyArePosted) {
			ScopedTaskEnvironment scoped_task_environment(GetParam(), ScopedTaskEnvironment::ExecutionMode::ASYNC);
			WaitableEvent task_ran(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
			PostTask(FROM_HERE,	BindOnce([](WaitableEvent* task_ran) { task_ran->Signal(); }, Unretained(&task_ran)));
			task_ran.Wait();
		}
		...
		// RunUntilIdle().
		TEST_P(ScopedTaskEnvironmentTest, AsyncTasksRunAsTheyArePostedAfterRunUntilIdle) {
			ScopedTaskEnvironment scoped_task_environment(GetParam(), ScopedTaskEnvironment::ExecutionMode::ASYNC);
			scoped_task_environment.RunUntilIdle();
			WaitableEvent task_ran(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
			PostTask(FROM_HERE, BindOnce([](WaitableEvent* task_ran) { task_ran->Signal(); }, Unretained(&task_ran)));
		...
		TEST_P(ScopedTaskEnvironmentTest, DelayedTasks) {
			...
			ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](subtle::Atomic32* counter) {
				subtle::NoBarrier_AtomicIncrement(counter, 4); }, Unretained(&counter)), kShortTaskDelay);
			// TODO(gab): This currently doesn't run because the ThreadPool's clock
			// isn't mocked but it should be.
			PostDelayedTask(FROM_HERE, BindOnce([](subtle::Atomic32* counter) {	subtle::NoBarrier_AtomicIncrement(counter, 128); },
			...
			ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](subtle::Atomic32* counter) {
				subtle::NoBarrier_AtomicIncrement(counter, 8); }, Unretained(&counter)), TimeDelta::FromDays(5));
			ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](subtle::Atomic32* counter) {
				subtle::NoBarrier_AtomicIncrement(counter, 16);	}, Unretained(&counter)), kLongTaskDelay);

			ThreadTaskRunnerHandle::Get()->PostTask(
				FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 1); }, Unretained(&counter)));
			PostTask(FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 2);	}, Unretained(&counter)));
		...
		TEST_F(ScopedTaskEnvironmentTest, FastForwardAdvanceMockClock) {
			...
			ScopedTaskEnvironment scoped_task_environment(ScopedTaskEnvironment::MainThreadType::MOCK_TIME);
		...
		TEST_F(ScopedTaskEnvironmentTest, MockTimeDomain_MaybeFastForwardToNextTask) {
			...
			ScopedTaskEnvironmentForTest scoped_task_environment(
				...
				ScopedTaskEnvironment::NowSource::MAIN_THREAD_MOCK_TIME);
			const TimeTicks start_time = base::TimeTicks::Now();
			EXPECT_FALSE(scoped_task_environment.GetTimeDomain()->MaybeFastForwardToNextTask(false));
			EXPECT_EQ(start_time, base::TimeTicks::Now());

			ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::DoNothing(), kDelay);
			EXPECT_TRUE(scoped_task_environment.GetTimeDomain()->MaybeFastForwardToNextTask(false));
			EXPECT_EQ(start_time + kDelay, base::TimeTicks::Now());
		}
		...
		TEST_F(ScopedTaskEnvironmentTest, ThreadPoolPoolAllowsMTA) {
			ScopedTaskEnvironment scoped_task_environment;
			PostTask(FROM_HERE, BindOnce(&win::AssertComApartmentType, win::ComApartmentType::MTA));
			scoped_task_environment.RunUntilIdle();
		}
#endif  // defined(OS_WIN)

		TEST_F(ScopedTaskEnvironmentTest, SetsDefaultRunTimeout) {
			const RunLoop::ScopedRunTimeoutForTest* old_run_timeout = RunLoop::ScopedRunTimeoutForTest::Current();

			{
				...
				// calling test.
				const RunLoop::ScopedRunTimeoutForTest* run_timeout = RunLoop::ScopedRunTimeoutForTest::Current();
				ASSERT_NE(run_timeout, old_run_timeout);
				EXPECT_EQ(run_timeout->timeout(), TestTimeouts::action_max_timeout());
				EXPECT_NONFATAL_FAILURE({ run_timeout->on_timeout().Run(); }, "Run() timed out");
			}
