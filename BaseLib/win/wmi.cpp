...
namespace base::win
{
		...
		void WmiComputerSystemInfo::PopulateModelAndManufacturer(
			...
			static constexpr std::wstring_view query_computer_system = L"SELECT Manufacturer,Model FROM Win32_ComputerSystem";
			...
			if (SUCCEEDED(hr) && manufacturer.type() == VT_BSTR) {
				manufacturer_ = V_BSTR(manufacturer.ptr());
			}
			ScopedVariant model;
			hr = class_object->Get(L"Model", 0, model.Receive(), 0, 0);
			if (SUCCEEDED(hr) && model.type() == VT_BSTR) {
				model_ = V_BSTR(model.ptr());
			}
		}

		void WmiComputerSystemInfo::PopulateSerialNumber(
			const ComPtr<IWbemServices>& services) {
			static constexpr std::wstring_view query_bios = L"SELECT SerialNumber FROM Win32_Bios";
			...
			if (SUCCEEDED(hr) && serial_number.type() == VT_BSTR) {
				serial_number_ = V_BSTR(serial_number.ptr());
			}
		}
} // namespace base
