...
namespace base {
	namespace {
		...
		class CopyCounter {
		public:
			CopyCounter(int* copies, int* assigns) : counter_(copies, assigns, nullptr, nullptr) {
			}
		...
		class MoveCounter {
		public:
			MoveCounter(int* move_constructs, int* move_assigns) : counter_(nullptr, nullptr, move_constructs, move_assigns) {
			}
			...
			explicit MoveCounter(DerivedCopyMoveCounter&& other) : counter_(std::move(other)) {
			}
