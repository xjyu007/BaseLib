...
#include "trace_event/trace_log.h"
...
#include "time/time.h"
#include "trace_event/event_name_filter.h"
#include "trace_event/heap_profiler.h"
#include "trace_event/heap_profiler_event_filter.h"
...
#include "trace_event/thread_instruction_count.h"
#include "trace_event/trace_buffer.h"
...
#undef max
		...
		void ForEachCategoryFilter(const unsigned char* category_group_enabled,
			...
			auto filter_bitmap = category->enabled_filters();
			for (auto index = 0; filter_bitmap != 0; filter_bitmap >>= 1, index++) {
	...
	class TraceLog::ThreadLocalEventBuffer final
	...
	public:
		...
		TraceEvent* GetEventByHandle(TraceEventHandle handle) const	{
	...
	bool TraceLog::ThreadLocalEventBuffer::OnMemoryDump(const MemoryDumpArgs& args, 
		...
		if (!chunk_)
			return true;
		const auto dump_base_name = StringPrintf(
	...
	const unsigned char* TraceLog::GetCategoryGroupEnabled(
		...
		if (!category) {
			...
			// at the same time.
			const auto category_initializer = [](TraceCategory* category) {
				GetInstance()->UpdateCategoryState(category);
			};
	...
	TraceLog::InternalTraceOptions TraceLog::GetInternalOptionsFromTraceConfig(
		...
		const auto ret = config.IsArgumentFilterEnabled()
	...
	int TraceLog::GetNumTracesRecorded() const {
		AutoLock lock(lock_);
	...
	void TraceLog::FlushInternal(const OutputCallback& cb,
		...
		if (IsEnabled()) {
			...
			const scoped_refptr<RefCountedString> empty_result = new RefCountedString;
			if (!cb.is_null())
	...
	void TraceLog::ConvertTraceEventsToTraceFormat(
		...
		while (const auto chunk = logged_events->NextChunk()) {
			for (size_t j = 0; j < chunk->size(); ++j) {
				const auto size = json_events_str_ptr->size();
	...
	void TraceLog::FlushCurrentThread(int generation, bool discard_events) {
		...
		const auto on_flush_override = on_flush_override_.load(std::memory_order_relaxed);
		...
		if (thread_id == static_cast<int>(PlatformThread::CurrentId())) {
			...
			if (new_name != current_thread_name->Get() && new_name && *new_name) {
				...
				if (existing_name == thread_names_.end()) {
					...
					// new name.
					const auto existing_names = SplitStringPiece(
						existing_name->second, ",", KEEP_WHITESPACE,
						SPLIT_WANT_NONEMPTY);
	...
	std::string TraceLog::EventToConsoleMessage(unsigned char phase,
		...
		size_t depth = 0;
		const auto it = thread_event_start_times_.find(thread_id);
	...
	void TraceLog::UpdateTraceEventDuration(
		...
		const char* name,
		TraceEventHandle handle) {
		const char category_group_enabled_local = *category_group_enabled;
	...
	uint64_t TraceLog::MangleEventId(uint64_t id) const {
	...
	TraceEvent* TraceLog::GetEventByHandle(TraceEventHandle handle) const {
	...
	void TraceLog::SetProcessID(int process_id) {
		...
		const auto kOffsetBasis = 14695981039346656037ull;
		const auto kFnvPrime = 1099511628211ull;
		const auto pid = static_cast<unsigned long long>(process_id_);
	...
	TraceBuffer* TraceLog::CreateTraceBuffer() const{
		HEAP_PROFILER_SCOPED_IGNORE;
		const auto options = trace_options();
