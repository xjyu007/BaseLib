...
namespace base {
	...
	bool TestLauncherTracer::Dump(const FilePath& path) {
		...
		for (const Event& event : events_) {
			...
			json_event->SetInteger("ts", static_cast<int>((event.timestamp - trace_start_time_).InMicroseconds()));
			json_event->SetIntKey("dur", static_cast<int>(event.duration.InMicroseconds()));
