...
#undef BITRATE_TO_SPEED_CASE
		...
		int DataBitsEnumToConstant(SerialDataBits data_bits) {
			switch (data_bits) {
		...
		int ParityBitEnumToConstant(SerialParityBit parity_bit) {
			switch (parity_bit) {
		...
		int StopBitsEnumToConstant(SerialStopBits stop_bits) {
			switch (stop_bits) {
...
#undef SPEED_TO_BITRATE_CASE
		...
		SerialDataBits DataBitsConstantToEnum(int data_bits) {
			switch (data_bits) {
			case 7:
				return SerialDataBits::SEVEN;
			case 8:
			default:
				return SerialDataBits::EIGHT;
			}
		}

		SerialParityBit ParityBitConstantToEnum(int parity_bit) {
			switch (parity_bit) {
			case EVENPARITY:
				return SerialParityBit::EVEN;
			case ODDPARITY:
				return SerialParityBit::ODD;
			case NOPARITY:
			default:
				return SerialParityBit::NO_PARITY;
			}
		}

		SerialStopBits StopBitsConstantToEnum(int stop_bits) {
			switch (stop_bits) {
			case TWOSTOPBITS:
	...
	void SerialIoHandlerWin::OnDeviceRemoved(const std::string& device_path) {
		...
		if (port() == *path)
			CancelRead(SerialReceiveError::DEVICE_LOST);
	}
	...
	bool SerialIoHandlerWin::PostOpen() {
		...
		const auto io_thread_task_runner =
	...
	void SerialIoHandlerWin::ReadImpl() {
		...
		event_mask_ = 0;
		const auto ok = ::WaitCommEvent(file().GetPlatformFile(), &event_mask_,
			&comm_context_->overlapped);
		if (!ok && GetLastError() != ERROR_IO_PENDING) {
			VPLOG(1) << "Failed to receive serial event";
			QueueReadCompleted(0, SerialReceiveError::SYSTEM_ERROR);
		}
	...
	void SerialIoHandlerWin::WriteImpl() {
		...
		const auto ok = ::WriteFile(file().GetPlatformFile(), pending_write_buffer(),
			pending_write_buffer_len(), NULL,
			&write_context_->overlapped);
		if (!ok && GetLastError() != ERROR_IO_PENDING) {
			VPLOG(1) << "Write failed";
			QueueWriteCompleted(0, SerialSendError::SYSTEM_ERROR);
		}
	}
	...
	bool SerialIoHandlerWin::ConfigurePortImpl() {
		...
		DCHECK(options().data_bits != SerialDataBits::NONE);
		config.ByteSize = DataBitsEnumToConstant(options().data_bits);

		DCHECK(options().parity_bit != SerialParityBit::NONE);
		config.Parity = ParityBitEnumToConstant(options().parity_bit);

		DCHECK(options().stop_bits != SerialStopBits::NONE);
	...
	void SerialIoHandlerWin::OnIOCompleted(
		...
		if (context == comm_context_.get()) {
			...
			if (!ClearCommError(file().GetPlatformFile(), &errors, &status) ||
				errors != 0) {
				if (errors & CE_BREAK) {
					ReadCompleted(0, SerialReceiveError::BREAK);
				} else if (errors & CE_FRAME) {
					ReadCompleted(0, SerialReceiveError::FRAME_ERROR);
				} else if (errors & CE_OVERRUN) {
					ReadCompleted(0, SerialReceiveError::OVERRUN);
				} else if (errors & CE_RXOVER) {
					ReadCompleted(0, SerialReceiveError::BUFFER_OVERFLOW);
				} else if (errors & CE_RXPARITY) {
					ReadCompleted(0, SerialReceiveError::PARITY_ERROR);
				} else {
					ReadCompleted(0, SerialReceiveError::SYSTEM_ERROR);
				}
				return;
			}
			...
			if (read_canceled()) {
				...
				const auto ok = ::ReadFile(file().GetPlatformFile(), pending_read_buffer(),
			...
			if (write_canceled()) {
				...
				WriteCompleted(0, SerialSendError::SYSTEM_ERROR);
				if (error == ERROR_GEN_FAILURE && IsReadPending()) {
					...
					// SerialOnReceiveError which will notify the app about the
					// disconnection.
					CancelRead(SerialReceiveError::SYSTEM_ERROR);
				}
			} else {
	...
	SerialPortControlSignals* SerialIoHandlerWin::GetControlSignals()
		const {
		DWORD status;
		...
		const auto signals = new SerialPortControlSignals;
	...
	bool SerialIoHandlerWin::SetControlSignals(
		const SerialHostControlSignals& signals) {
		if (signals.has_dtr) {
	...
	SerialConnectionInfo* SerialIoHandlerWin::GetPortInfo() const {
		DCB config = { 0 };
		config.DCBlength = sizeof(config);
		if (!GetCommState(file().GetPlatformFile(), &config)) {
			VPLOG(1) << "Failed to get serial port info";
		...
		auto info = new SerialConnectionInfo;
