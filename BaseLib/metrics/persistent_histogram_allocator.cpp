...
namespace base
{
		...
		FilePath* out_spare_path)
	{
		...
		std::string tmp(name.data());
		if (out_active_path) {
			*out_active_path = ConstructFilePath(dir, tmp.append("-active"));
		}

		if (out_spare_path) {
			*out_spare_path = ConstructFilePath(dir, tmp.append("-spare"));
		}
	}
