...
namespace base {
	...
	TEST(StringUtilTest, TruncateUTF8ToByteSize) {
		...
		EXPECT_TRUE(Truncated("a\xc2\x81\xe1\x80\xbf\xf1""a""\x80\xa0",	10, &output));
	...
	TEST(StringUtilTest, as_u16cstr) {
		wchar_t rw_buffer[10] = {};
		static_assert(std::is_same<wchar_t*, decltype(as_writable_u16cstr(rw_buffer))>::value, "");
		EXPECT_EQ(static_cast<void*>(rw_buffer), as_writable_u16cstr(rw_buffer));

		std::wstring rw_str(10, '\0');
		static_assert(std::is_same<wchar_t*, decltype(as_writable_u16cstr(rw_str))>::value, "");
		EXPECT_EQ(static_cast<const void*>(rw_str.data()), as_writable_u16cstr(rw_str));

		const wchar_t ro_buffer[10] = {};
		static_assert(std::is_same<const wchar_t*, decltype(as_u16cstr(ro_buffer))>::value, "");
		EXPECT_EQ(static_cast<const void*>(ro_buffer), as_u16cstr(ro_buffer));

		const std::wstring ro_str(10, '\0');
		static_assert(std::is_same<const wchar_t*, decltype(as_u16cstr(ro_str))>::value, "");
		EXPECT_EQ(static_cast<const void*>(ro_str.data()), as_u16cstr(ro_str));

		//WStringPiece piece = ro_buffer;
		//static_assert(std::is_same<const wchar_t*, decltype(as_u16cstr(piece))>::value,	"");
	...
	TEST(StringUtilTest, TrimWhitespace) {
		...
		for (const auto& value : trim_cases) {
			EXPECT_EQ(value.return_value, TrimWhitespace(value.input, value.positions, &output));
	...
	TEST(StringUtilTest, IsStringASCII) {
		...
		{
			const size_t string_length = base::size(wchar_t_ascii) - 1;
			for (size_t offset = 0; offset < 4; ++offset) {
				for (size_t len = 0, max_len = string_length - offset; len < max_len;
					++len) {
					EXPECT_TRUE(IsStringASCII(std::wstring_view(wchar_t_ascii + offset, len)));
					for (size_t char_pos = offset; char_pos < len; ++char_pos) {
						wchar_t_ascii[char_pos] |= 0x80;
						EXPECT_FALSE(
							IsStringASCII(std::wstring_view(wchar_t_ascii + offset, len)));
						wchar_t_ascii[char_pos] &= ~0x80;
						// Also test when the upper half is non-zero.
						wchar_t_ascii[char_pos] |= 0x100;
						EXPECT_FALSE(
							IsStringASCII(std::wstring_view(wchar_t_ascii + offset, len)));
						wchar_t_ascii[char_pos] &= ~0x100;
					}
	...
	TEST(StringUtilTest, ConvertASCII) {
		...
		for (size_t i = 0; i < base::size(char_cases); ++i) {
			...
			std::wstring utf16 = ASCIIToUTF16(char_cases[i]);
			EXPECT_EQ(wchar_cases[i], utf16);

			std::string ascii = UTF16ToASCII(wchar_cases[i]);
		...
		std::wstring string16_with_nul = ASCIIToUTF16(string_with_nul);
		EXPECT_EQ(static_cast<std::wstring::size_type>(length_with_nul), string16_with_nul.length());
	...
	TEST(StringUtilTest, ToLowerASCII) {
		...
		EXPECT_EQ(static_cast<wchar_t>('c'), ToLowerASCII(static_cast<wchar_t>('C')));
		EXPECT_EQ(static_cast<wchar_t>('c'), ToLowerASCII(static_cast<wchar_t>('c')));
		EXPECT_EQ(static_cast<wchar_t>('2'), ToLowerASCII(static_cast<wchar_t>('2')));
	...
	TEST(StringUtilTest, ToUpperASCII) {
		...
		EXPECT_EQ(static_cast<wchar_t>('C'), ToUpperASCII(static_cast<wchar_t>('C')));
		EXPECT_EQ(static_cast<wchar_t>('C'), ToUpperASCII(static_cast<wchar_t>('c')));
		EXPECT_EQ(static_cast<wchar_t>('2'), ToUpperASCII(static_cast<wchar_t>('2')));
	...
	TEST(StringUtilTest, ReplaceSubstringsAfterOffset) {
		...
		// base::std::wstring variant
		for (const auto& scenario : cases) {
			std::wstring str = ASCIIToUTF16(scenario.str);
		...
		for (const auto& scenario : cases) {
			std::string str = std::string(scenario.str.data(), scenario.str.size());
	...
	TEST(StringUtilTest, ReplaceFirstSubstringAfterOffset) {
		...
		for (const auto& i : cases) {
			std::wstring str = ASCIIToUTF16(i.str);
	...
	TEST(StringUtilTest, Joinstring16) {
		std::wstring separator = ASCIIToUTF16(", ");
		std::vector<std::wstring> parts;
		EXPECT_EQ(std::wstring(), JoinString(parts, separator));

		parts.push_back(std::wstring());
		EXPECT_EQ(std::wstring(), JoinString(parts, separator));
		parts.clear();
	...
	TEST(StringUtilTest, JoinStringPiece16) {
		...
		// Test empty first part (https://crbug.com/698073).
		parts.push_back(std::wstring_view());
		EXPECT_EQ(std::wstring(), JoinString(parts, separator));
		parts.clear();

		const std::wstring kA = ASCIIToUTF16("a");
		parts.push_back(kA);
		EXPECT_EQ(ASCIIToUTF16("a"), JoinString(parts, separator));

		const std::wstring kB = ASCIIToUTF16("b");
		parts.push_back(kB);
		const std::wstring kC = ASCIIToUTF16("c");
		parts.push_back(kC);
		EXPECT_EQ(ASCIIToUTF16("a, b, c"), JoinString(parts, separator));

		parts.push_back(std::wstring_view());
		EXPECT_EQ(ASCIIToUTF16("a, b, c, "), JoinString(parts, separator));
		const std::wstring kSpace = ASCIIToUTF16(" ");
	...
	TEST(StringUtilTest, JoinStringInitializerList16) {
		std::wstring separator = ASCIIToUTF16(", ");
		EXPECT_EQ(std::wstring(), JoinString({}, separator));

		// Test empty first part (https://crbug.com/698073).
		EXPECT_EQ(std::wstring(), JoinString({ std::wstring_view() }, separator));
		...
		EXPECT_EQ(ASCIIToUTF16("a, b, c, "),JoinString({ kA, kB, kC, std::wstring_view() }, separator));
		const std::wstring kSpace = ASCIIToUTF16(" ");
		EXPECT_EQ(
			ASCIIToUTF16("a|b|c|| "),
			JoinString({ kA, kB, kC, std::wstring_view(), kSpace }, ASCIIToUTF16("|")));

		// With std::wstring_views.
		const std::wstring_view kPieceA = kA;
		const std::wstring_view kPieceB = kB;
	...
	TEST(StringUtilTest, StartsWith) {
		...
		EXPECT_FALSE(StartsWith(ASCIIToUTF16("java"), ASCIIToUTF16("javascript"),
			base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_FALSE(StartsWith(std::wstring(), ASCIIToUTF16("javascript"),
			base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_FALSE(StartsWith(std::wstring(), ASCIIToUTF16("javascript"),
			base::CompareCase::SENSITIVE));
		EXPECT_TRUE(StartsWith(ASCIIToUTF16("java"), std::wstring(),
			base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_TRUE(StartsWith(ASCIIToUTF16("java"), std::wstring(),
			base::CompareCase::SENSITIVE));
	}
	...
	TEST(StringUtilTest, EndsWith) {
		...
		EXPECT_FALSE(EndsWith(ASCIIToUTF16("Foo.plugin Bar"), ASCIIToUTF16(".plugin"),
			base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_FALSE(EndsWith(std::wstring(), ASCIIToUTF16(".plugin"),
			base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_FALSE(EndsWith(std::wstring(), ASCIIToUTF16(".plugin"),
			base::CompareCase::SENSITIVE));
		EXPECT_TRUE(EndsWith(ASCIIToUTF16("Foo.plugin"), std::wstring(),
			base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_TRUE(EndsWith(ASCIIToUTF16("Foo.plugin"), std::wstring(),
			base::CompareCase::SENSITIVE));
		...
		EXPECT_TRUE(EndsWith(ASCIIToUTF16(".plugin"), ASCIIToUTF16(".plugin"),
			base::CompareCase::SENSITIVE));
		EXPECT_TRUE(
			EndsWith(std::wstring(), std::wstring(), base::CompareCase::INSENSITIVE_ASCII));
		EXPECT_TRUE(EndsWith(std::wstring(), std::wstring(), base::CompareCase::SENSITIVE));
	}
	...
	TEST(StringUtilTest, GetStringFWithOffsets) {
		...
		ReplaceStringPlaceholders(ASCIIToUTF16("Hello, $1. Your number is $2."), subst, &offsets);
		EXPECT_EQ(2U, offsets.size());
		EXPECT_EQ(7U, offsets[0]);
		EXPECT_EQ(25U, offsets[1]);
		offsets.clear();

		ReplaceStringPlaceholders(ASCIIToUTF16("Hello, $2. Your number is $1."), subst, &offsets);
		EXPECT_EQ(2U, offsets.size());
	...
	TEST(StringUtilTest, ReplaceStringPlaceholders) {
		...
		std::wstring formatted = ReplaceStringPlaceholders(ASCIIToUTF16("$1a,$2b,$3c,$4d,$5e,$6f,$7g,$8h,$9i"), subst, nullptr);
	...
	TEST(StringUtilTest, ReplaceStringPlaceholdersNetExpansionWithContraction) {
		...
		std::wstring original = ASCIIToUTF16("$1a,$2b,$3c,$4d,$5e,$6f,$7g,$8h,$9i");
	...
	TEST(StringUtilTest, ReplaceStringPlaceholdersNetContractionWithExpansion) {
		...
		std::wstring formatted = ReplaceStringPlaceholders(ASCIIToUTF16("$3_$4$2$1$5"), subst, nullptr);
	...
	TEST(StringUtilTest, ReplaceStringPlaceholdersOneDigit) {
		...
		std::wstring formatted = ReplaceStringPlaceholders(ASCIIToUTF16(" $16 "), subst, nullptr);
	...
	TEST(StringUtilTest, ReplaceStringPlaceholdersInvalidPlaceholder) {
		std::vector<std::wstring> subst;
		subst.push_back(ASCIIToUTF16("1a"));
		std::wstring formatted = ReplaceStringPlaceholders(ASCIIToUTF16("+$-+$A+$1+"), subst, nullptr);
	...
	TEST(StringUtilTest, ContainsOnlyChars) {
		...
		EXPECT_TRUE(ContainsOnlyChars(std::wstring(), kWhitespaceUTF16));
	...
	protected:
		static void WritesCorrectly(size_t num_chars) {
			...
			strncpy_s(WriteInto(&buffer, num_chars + 1), buffer.size(), kOriginal, num_chars);
	...
	TEST_F(WriteIntoTest, WriteInto) {
		...
		strncpy_s(WriteInto(&dead, 5), dead.size(), kDead, 4);
