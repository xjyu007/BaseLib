...
#include "task/common/operations_controller.h"
#include "task/sequence_manager/associated_thread_id.h"
#include "task/sequence_manager/atomic_flag_set.h"
#include "task/sequence_manager/enqueue_order.h"
#include "task/sequence_manager/lazily_deallocated_deque.h"
	...
	namespace sequence_manager {
		...
		namespace internal {
			...
			public:
				...
				std::optional<TimeTicks> GetNextScheduledWakeUp();
				std::optional<DelayedWakeUp> GetNextScheduledWakeUpImpl();
				...
				WeakPtr<SequenceManagerImpl> GetSequenceManagerWeakPtr() const;
			...
			protected:
				void SetDelayedWakeUpForTesting(std::optional<DelayedWakeUp> wake_up);
			...
			private:
				...
				private:
					...
					[[nodiscard]] bool PostTask(PostedTask task) const;
				...
				public:
					...
					[[nodiscard]] const Task& top() const { return queue_.top(); }
					void swap(DelayedIncomingQueue* rhs);

					[[nodiscard]] bool has_pending_high_resolution_tasks() const {
				...
				struct MainThreadOnly {
					...
					// excessive calls.
					std::optional<DelayedWakeUp> scheduled_wake_up;
