...
namespace base::experimental {
	...
	JobDelegate::~JobDelegate() {
		...
		auto expected_max_concurrency = recorded_max_concurrency_;
...
#if DCHECK_IS_ON()
		// Case 1:
		const auto max_concurrency = task_source_->GetMaxConcurrency();
		if (max_concurrency <= expected_max_concurrency)
    		return;

		// Case 2a:
		const auto actual_version = task_source_->GetConcurrencyIncreaseVersion();
		DCHECK_LE(recorded_increase_version_, actual_version);
		if (recorded_increase_version_ < actual_version)
			return;

		// Case 2b:
		const auto updated = task_source_->WaitForConcurrencyIncreaseUpdate(
	...
	void JobHandle::UpdatePriority(TaskPriority new_priority) const	{
		task_source_->delegate()->UpdatePriority(task_source_, new_priority);
	}

	void JobHandle::NotifyConcurrencyIncrease() const {
		task_source_->NotifyConcurrencyIncrease();
	}
