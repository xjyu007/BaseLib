...
namespace base {
	...
	void MessageLoopCurrent::AddDestructionObserver(
	    DestructionObserver* destruction_observer) const {
	...
	bool MessageLoopCurrent::IsIdleForTesting() const {
		DCHECK(current_->IsBoundToCurrentThread());
		return current_->IsIdleForTesting();
	}

	void MessageLoopCurrent::AddTaskObserver(TaskObserver* task_observer) const {
		DCHECK(current_->IsBoundToCurrentThread());
		current_->AddTaskObserver(task_observer);
	}

	void MessageLoopCurrent::RemoveTaskObserver(TaskObserver* task_observer) const {
		DCHECK(current_->IsBoundToCurrentThread());
		current_->RemoveTaskObserver(task_observer);
	}

	void MessageLoopCurrent::SetAddQueueTimeToTasks(bool enable) const {
		DCHECK(current_->IsBoundToCurrentThread());
		current_->SetAddQueueTimeToTasks(enable);
	}

	void MessageLoopCurrent::SetNestableTasksAllowed(bool allowed) const {
		DCHECK(current_->IsBoundToCurrentThread());
	...
	MessagePumpForIO* MessageLoopCurrentForIO::GetMessagePumpForIO() const {
		return dynamic_cast<MessagePumpForIO*>(current_->GetMessagePump());
	}
	...
	bool MessageLoopCurrentForIO::WaitForIOCompletion(
	    DWORD timeout,
	    MessagePumpForIO::IOHandler* filter) const {
		DCHECK(current_->IsBoundToCurrentThread());
