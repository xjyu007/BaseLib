...
#include "macros.h"
//#include "sanitizer_buildflags.h"
#include "../BaseLib_Test/scoped_feature_list.h"
...
namespace logging {
	...
	namespace {
		...
		void LogSink(const char* file, int line, const std::string_view message, const std::string_view stack_trace) {
...
/*
#if !defined(ADDRESS_SANITIZER) && !defined(MEMORY_SANITIZER) && !BUILDFLAG(IS_HWASAN)
		...
		TEST_F(LoggingTest, LogMessageMarkersOnStack) {
			...
			ScopedLogAssertHandler assert_handler(base::BindRepeating(
				...
				const std::string_view stack_view(start_char_marker,
					end_char_marker - start_char_marker);
				ASSERT_FALSE(stack_view.find(message) == std::string_view::npos);
			},
				&stack_start));
