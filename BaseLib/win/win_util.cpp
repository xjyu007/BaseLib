...
#include "win/win_client_metrics.h"
...
namespace base::win {
	...
	namespace {
		...
		bool SetProcessDpiAwarenessWrapper(PROCESS_DPI_AWARENESS value) {
			...
			if (set_process_dpi_awareness_func) {
				const auto hr = set_process_dpi_awareness_func(value);
				if (SUCCEEDED(hr))
					return true;
		...
		bool* GetRegisteredWithManagementStateStorage() {
			static bool state = []() {
				...
				const HRESULT hr = is_device_registered_with_management_function(&is_managed, 0, nullptr);
	...
	bool IsWindows10TabletMode(HWND hwnd) {
		...
		const auto view_settings_guid = ScopedHString::Create(
			RuntimeClass_Windows_UI_ViewManagement_UIViewSettings);
		Microsoft::WRL::ComPtr<IUIViewSettingsInterop> view_settings_interop;
		auto hr = base::win::RoGetActivationFactory(
		...
		auto mode = ABI::Windows::UI::ViewManagement::UserInteractionMode_Mouse;
	...
	bool IsKeyboardPresentOnSlate(std::string* reason, HWND hwnd) {
		...
		const auto device_info = SetupDiGetClassDevs(&KEYBOARD_CLASS_GUID, nullptr, NULL, DIGCF_PRESENT);
	...
	bool IsDeviceUsedAsATablet(std::string* reason) {
		...
		const auto role = GetPlatformRole();
	...
	bool GetLoadedModulesSnapshot(HANDLE process, std::vector<HMODULE>* snapshot) {
		...
		do {
			...
			const size_t num_modules = bytes_required / sizeof(HMODULE);
	...
	bool IsProcessPerMonitorDpiAware() {
		...
		if (per_monitor_dpi_aware == PerMonitorDpiAware::UNKNOWN) {
			...
			const auto shcore_dll = ::LoadLibrary(L"shcore.dll");
			if (shcore_dll) {
				const auto get_process_dpi_awareness_func = reinterpret_cast<decltype(::GetProcessDpiAwareness)*>(
	...
	std::wstring String16FromGUID(REFGUID rguid) {
		...
		CHECK(SUCCEEDED(StringCchPrintfW(
			...
			rguid.Data4[7])));
		return std::wstring(as_u16cstr(guid_string), kGuidStringCharacters - 1);
	}
	...
	bool IsRunningUnderDesktopName(std::wstring_view desktop_name) {
		const auto thread_desktop = ::GetThreadDesktop(::GetCurrentThreadId());
		if (!thread_desktop)
			return false;

		const auto current_desktop_name = GetWindowObjectName(thread_desktop);
	...
	bool IsCurrentSessionRemote() {
		...
		const RegKey key(HKEY_LOCAL_MACHINE, kRdpSettingsKeyName, KEY_READ);
		if (!key.Valid())
			return false;
