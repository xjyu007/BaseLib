...
namespace base {
	...
	void SampleVectorBase::Accumulate(Sample value, Count count) {
		...
		const auto old_value = new_value - count;
	...
	Count SampleVectorBase::TotalCount() const {
		// Handle the single-sample case.
		const auto sample = single_sample().Load();
		if (sample.count != 0)
			return sample.count;
		...
		if (counts() || MountExistingCountsStorage()) {
			Count count = 0;
			const auto size = counts_size();
			const auto counts_array = counts();
	...
	size_t SampleVectorBase::GetBucketIndex(Sample value) const {
		const auto bucket_count = bucket_ranges_->bucket_count();
	...
	void SampleVectorBase::MoveSingleSampleToCounts() {
		...
		const auto sample = single_sample().Extract(/*disable=*/true);
	...
	PersistentSampleVector::PersistentSampleVector(
		...
		if (single_sample().IsDisabled()) {
			const auto success = PersistentSampleVector::MountExistingCountsStorage();
			DCHECK(success);
		}
	}
