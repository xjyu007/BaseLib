...
namespace base::internal {
	...
	namespace {
		...
		HistogramBase* GetLatencyHistogram(std::string_view histogram_name,
		...
		// "ThreadPool.{histogram_name}.{histogram_label}.{task_type_suffix}".
		HistogramBase* GetCountHistogram(std::string_view histogram_name,
										 std::string_view histogram_label,
										 std::string_view task_type_suffix) {
			DCHECK(!histogram_name.empty());
			DCHECK(!histogram_label.empty());
	...
	void TaskTracker::RecordLatencyHistogram(
		...
		const auto task_latency = TimeTicks::Now() - posted_time;
