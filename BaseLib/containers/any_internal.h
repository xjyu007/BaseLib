...
namespace base {
	...
	namespace internal {
		...
		public:
			...
			AnyInternal(AnyInternal&& other) noexcept : type_ops_(other.type_ops_), union_({}) {
				if (other.type_ops_)
			...
			constexpr explicit AnyInternal(const TypeOps* type_ops) : type_ops_(type_ops), union_({}) {
			}
			...
			struct InlineStorageHelper {
				static constexpr bool kUseInlineStorage = (sizeof(T) <= sizeof(InlineAlloc));
			...
			constexpr T* GetStorage() {
				return static_cast<T*>(
					GetStorageHelper<InlineStorageHelper<T>::kUseInlineStorage>::GetStorage(*this));
			}
