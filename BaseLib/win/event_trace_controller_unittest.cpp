...
namespace base {
	namespace win {
		...
		namespace {
			...
			DEFINE_GUID(kGuidNull, 0x0000000, 0x0000, 0x0000, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0);

			const ULONG kTestProviderFlags = 0xCAFEBABE;

			class TestingProvider : public EtwTraceProvider {
			public:
				explicit TestingProvider(const GUID& provider_name)	: EtwTraceProvider(provider_name) {
		...
		namespace {
			...
			public:
				EtwTraceControllerTest() : session_name_(StringPrintf(L"TestSession-%d", GetCurrentProcId())) {
				}
		...
		TEST_F(EtwTraceControllerTest, StartRealTimeSession) {
			...
			HRESULT hr = controller.StartRealtimeSession(session_name_.c_str(), 100 * 1024);
