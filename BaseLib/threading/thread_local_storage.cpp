...
namespace {
	...
	TlsVectorEntry* ConstructTlsVector() {
		...
		const auto tls_data = new TlsVectorEntry[kThreadLocalStorageSize];
	...
	void OnThreadExitInternal(TlsVectorEntry* tls_data) {
		...
		auto remaining_attempts = kMaxDestructorIterations;
		auto need_to_scan_destructors = true;
...
namespace base {
	...
	namespace internal {
		...
		void PlatformThreadLocalStorage::OnThreadExit() {
			...
			const auto state = GetTlsVectorStateAndValue(key, &tls_vector);
...
// static
	...
	void ThreadLocalStorage::Slot::Set(void* value) const {
		TlsVectorEntry* tls_data = nullptr;
		const auto state = GetTlsVectorStateAndValue(
			subtle::NoBarrier_Load(&g_native_tls_key), &tls_data);
		DCHECK_NE(state, TlsVectorState::kDestroyed);
		if (!tls_data) {
			if (!value)
