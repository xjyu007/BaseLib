...
#include "containers/unique_any.h"
#include "location.h"
#include "memory/ref_counted.h"
#include "task/common/checked_lock.h"
#include "task/promise/dependent_list.h"
#include "task/promise/promise_executor.h"
...
namespace base {
	...
	namespace internal {
		...
		public:
			...
			[[nodiscard]] const Location& from_here() const { return from_here_; }

			[[nodiscard]] bool IsSettled() const { return dependents_.IsSettled(); }
			[[nodiscard]] bool IsCanceled() const;

			// It's an error (result will be racy) to call these if unsettled.
			[[nodiscard]] bool IsRejected() const { return dependents_.IsRejected(); }
			[[nodiscard]] bool IsResolved() const { return dependents_.IsResolved(); }

			[[nodiscard]] bool IsRejectedForTesting() const {
				return dependents_.IsRejectedForTesting();
			}

			[[nodiscard]] bool IsResolvedForTesting() const {
				return dependents_.IsResolvedForTesting();
			}

			[[nodiscard]] bool IsResolvedWithPromise() const {
				return value_.type() == TypeId::From<scoped_refptr<AbstractPromise>>();
			}

			[[nodiscard]] const unique_any& value() const { return FindNonCurriedAncestor()->value_; }

			class ValueHandle {
			public:
				[[nodiscard]] unique_any& value() const { return value_; }
			...
			[[nodiscard]] const AbstractPromise* FindNonCurriedAncestor() const;
			...
			[[nodiscard]] const AbstractPromise* GetCurriedPromise() const;
			...
			[[nodiscard]] const std::vector<DependentList::Node>* prerequisite_list() const {
				if (!prerequisites_)
					return nullptr;
			...
			[[nodiscard]] AbstractPromise* GetOnlyPrerequisite() const {
			...
			// rerequsite if any.
			[[nodiscard]] AbstractPromise* GetFirstSettledPrerequisite() const;
		...
		private:
			...
			[[nodiscard]] const PromiseExecutor* GetExecutor() const;
...
#if DCHECK_IS_ON()
			void MaybeInheritChecks(AbstractPromise* prerequisite)
			...
			public:
				...
				void CheckForDoubleMoveErrors(
					const Location& new_dependent_location,
