...
namespace base {
	namespace sequence_manager {
		...
		public:
			...
			[[nodiscard]] virtual LazyNow CreateLazyNow() const = 0;

			// Evaluates TimeDomain's time.
			// Can be called from any thread.
			// TODO(alexclarke): Make this main thread only.
			[[nodiscard]] virtual TimeTicks Now() const = 0;
			...
			virtual std::optional<TimeDelta> DelayTillNextTask(LazyNow* lazy_now) = 0;

			void AsValueInto(trace_event::TracedValue* state) const;
			[[nodiscard]] bool HasPendingHighResolutionTasks() const;
		...
		protected:
			...
			[[nodiscard]] SequenceManager* sequence_manager() const;

			// Returns the earliest scheduled wake up in the TimeDomain's time.
			[[nodiscard]] virtual std::optional<TimeTicks> NextScheduledRunTime() const;

			[[nodiscard]] size_t NumberOfScheduledWakeUps() const {
		...
		private:
			...
			void SetNextWakeUpForQueue(internal::TaskQueueImpl* queue,
									   std::optional<internal::DelayedWakeUp> wake_up,
			...
			// ScheduleWork.
			void MoveReadyDelayedTasksToWorkQueues(LazyNow* lazy_now) const;
			...
			struct ScheduledDelayedWakeUp {
				...
				void SetHeapHandle(base::internal::HeapHandle handle) const {
