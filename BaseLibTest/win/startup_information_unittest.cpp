...
MULTIPROCESS_TEST_MAIN(FireInheritedEvents) {
	...
	HANDLE* events = reinterpret_cast<HANDLE*>(::MapViewOfFile(section, PAGE_READWRITE, 0, 0, kSectionSize));
...
TEST_F(StartupInformationTest, InheritStdOut) {
	...
	HANDLE section = ::CreateFileMappingW(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, kSectionSize, kSectionName);
	ASSERT_TRUE(section);

	HANDLE* events = reinterpret_cast<HANDLE*>(::MapViewOfFile(section, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, kSectionSize));
	...
	ASSERT_TRUE(startup_info.UpdateProcThreadAttribute(PROC_THREAD_ATTRIBUTE_HANDLE_LIST, &events[0], sizeof(events[0])));

	std::wstring cmd_line = MakeCmdLine("FireInheritedEvents").GetCommandLineString();

	PROCESS_INFORMATION temp_process_info = {};
	ASSERT_TRUE(
		::CreateProcess(NULL, base::as_writable_wcstr(cmd_line), NULL, NULL, true, EXTENDED_STARTUPINFO_PRESENT, NULL,
