...
namespace base {
	...
	namespace {
		...
		bool GetAppOutputInternal(const std::wstring_view& cl,
			bool include_stderr,
			std::string* output,
			int* exit_code) {
	...
	Process LaunchProcess(const std::wstring& cmdline,
		const LaunchOptions& options) {
		...
		if (options.as_user) {
			...
			if (!launched) {
				DPLOG(ERROR) << "Command line:" << std::endl << WideToUTF8(cmdline)
					<< std::endl;
				return Process();
			}
		}
	...
	bool GetAppOutput(const std::wstring_view& cl, std::string* output) {
		int exit_code;
