...
namespace base::win {
	...
	PIMAGE_NT_HEADERS PEImage::GetNTHeaders() const {
		const auto dos_header = GetDosHeader();
	...
	DWORD PEImage::GetImageDirectoryEntrySize(UINT directory) const {
		const auto entry = GetDataDirectory(directory);
	...
	PIMAGE_SECTION_HEADER PEImage::GetImageSectionFromAddr(PVOID address) const {
		const auto target = reinterpret_cast<PBYTE>(address);
		PIMAGE_SECTION_HEADER section;

		for (UINT i = 0; NULL != (section = GetSectionHeader(i)); i++) {
			// Don't use the virtual RVAToAddr.
			const auto start = reinterpret_cast<PBYTE>(
	...
	PIMAGE_SECTION_HEADER PEImage::GetImageSectionHeaderByName(
		...
		for (int i = 0; i < num_sections; i++) {
			auto section = GetSectionHeader(i);
	...
	bool PEImage::GetDebugId(LPGUID guid,
		...
		const auto debug_directory_size =
		...
		const size_t directory_count = debug_directory_size / sizeof(IMAGE_DEBUG_DIRECTORY);
	...
	PDWORD PEImage::GetExportEntry(LPCSTR name) const {
		const auto exports = GetExportDirectory();
		...
		const auto functions = reinterpret_cast<PDWORD>(
	...
	FARPROC PEImage::GetProcAddress(LPCSTR function_name) const {
		const auto export_entry = GetExportEntry(function_name);
		if (NULL == export_entry)
			return NULL;

		const auto function = reinterpret_cast<PBYTE>(RVAToAddr(*export_entry));

		const auto exports = reinterpret_cast<PBYTE>(
			GetImageDirectoryEntryAddr(IMAGE_DIRECTORY_ENTRY_EXPORT));
		const auto size = GetImageDirectoryEntrySize(IMAGE_DIRECTORY_ENTRY_EXPORT);
		if (!exports || !size)
			return NULL;
	...
	bool PEImage::GetProcOrdinal(LPCSTR function_name, WORD* ordinal) const {
		...
		if (IsOrdinal(function_name)) {
			*ordinal = ToOrdinal(function_name);
			...
			const auto names = reinterpret_cast<PDWORD>(RVAToAddr(exports->AddressOfNames));
			auto lower = names;
			auto upper = names + exports->NumberOfNames;
			auto cmp = -1;

			// Binary Search for the name.
			while (lower != upper) {
				const auto middle = lower + (upper - lower) / 2;
				const auto name = reinterpret_cast<LPCSTR>(RVAToAddr(*middle));
			...
			const auto ordinals = reinterpret_cast<PWORD>(
	...
	bool PEImage::EnumSections(EnumSectionsFunction callback, PVOID cookie) const {
		...
		const UINT num_sections = nt_headers->FileHeader.NumberOfSections;
		auto section = GetSectionHeader(0);

		for (UINT i = 0; i < num_sections; i++, section++) {
			const auto section_start = RVAToAddr(section->VirtualAddress);
			const auto size = section->Misc.VirtualSize;
	...
	bool PEImage::EnumExports(EnumExportsFunction callback, PVOID cookie) const {
		const auto directory = GetImageDirectoryEntryAddr(IMAGE_DIRECTORY_ENTRY_EXPORT);
		const auto size = GetImageDirectoryEntrySize(IMAGE_DIRECTORY_ENTRY_EXPORT);
		...
		const auto exports = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(
			directory);
		const UINT ordinal_base = exports->Base;
		const UINT num_funcs = exports->NumberOfFunctions;
		const UINT num_names = exports->NumberOfNames;
		const auto functions = reinterpret_cast<PDWORD>(RVAToAddr(
			exports->AddressOfFunctions));
		const auto names = reinterpret_cast<PDWORD>(RVAToAddr(exports->AddressOfNames));
		const auto ordinals = reinterpret_cast<PWORD>(RVAToAddr(
	...
	bool PEImage::EnumRelocs(EnumRelocsFunction callback, PVOID cookie) const {
		const auto directory = GetImageDirectoryEntryAddr(IMAGE_DIRECTORY_ENTRY_BASERELOC);
		auto size = GetImageDirectoryEntrySize(IMAGE_DIRECTORY_ENTRY_BASERELOC);
		...
		while (size >= sizeof(IMAGE_BASE_RELOCATION) && base->SizeOfBlock &&
			...
			auto reloc = reinterpret_cast<PWORD>(base + 1);
			UINT num_relocs = (base->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) /
				sizeof(WORD);

			for (UINT i = 0; i < num_relocs; i++, reloc++) {
				const WORD type = *reloc >> 12;
				const auto address = RVAToAddr(base->VirtualAddress + (*reloc & 0x0FFF));
	...
	bool PEImage::EnumImportChunks(EnumImportChunksFunction callback,
		...
		for (; import->FirstThunk; import++) {
			const auto module_name = reinterpret_cast<LPCSTR>(RVAToAddr(import->Name));
			const auto name_table = reinterpret_cast<PIMAGE_THUNK_DATA>(
				RVAToAddr(import->OriginalFirstThunk));
			const auto iat = reinterpret_cast<PIMAGE_THUNK_DATA>(
	...
	bool PEImage::EnumDelayImportChunks(EnumDelayImportChunksFunction callback,
	                                    PVOID cookie,
	                                    LPCSTR target_module_name) const {
		const auto directory = GetImageDirectoryEntryAddr(
			IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT);
		const auto size = GetImageDirectoryEntrySize(IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT);
	...
	bool PEImage::ImageRVAToOnDiskOffset(uintptr_t rva,
		...
		const auto address = RVAToAddr(rva);
	...
	bool PEImage::ImageAddrToOnDiskOffset(LPVOID address,
		...
		const auto section_header = GetImageSectionFromAddr(address);
		if (NULL == section_header)
			return false;

		// Don't follow the virtual RVAToAddr, use the one on the base.
		const auto offset_within_section =
	...
	PVOID PEImageAsData::RVAToAddr(uintptr_t rva) const {
		...
		const auto in_memory = PEImage::RVAToAddr(rva);
		DWORD disk_offset;
