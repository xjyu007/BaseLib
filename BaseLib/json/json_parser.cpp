...
namespace base::internal {
	...
	std::optional<std::string_view> JSONParser::PeekChars(size_t count) const {
	...
	std::optional<char> JSONParser::PeekChar() const {
		auto chars = PeekChars(1);
		if (chars)
			return (*chars)[0];
		return std::nullopt;
	}
	...
	std::optional<std::string_view> JSONParser::ConsumeChars(size_t count) {
		const auto chars = PeekChars(count);
		if (chars)
			index_ += count;
		return chars;
	}
	...
	std::optional<char> JSONParser::ConsumeChar() {
		auto chars = ConsumeChars(1);
		if (chars)
			return (*chars)[0];
		return std::nullopt;
	}

	const char* JSONParser::pos() const {
	...
	bool JSONParser::EatComment() {
		const auto comment_start = ConsumeChars(2);
		if (!comment_start)
			return false;
	...
	std::optional<Value> JSONParser::ConsumeDictionary() {
		...
		const StackMarker depth_check(max_depth_, &stack_depth_);
	...
	bool JSONParser::ConsumeStringRaw(StringBuilder* out) {
		...
		while (PeekChar()) {
			...
			if (next_char != '\\') {
				...
				// or with a UTF conversion), so using std::string_view isn't possible -- force
				// a conversion.
				string.Convert();

				// Read past the escape '\' and ensure there's a character following.
				auto escape_sequence = ConsumeChars(2);
				if (!escape_sequence) {
	...
	std::optional<Value> JSONParser::ConsumeNumber() {
		...
		const std::string_view num_string(num_start, end_index - start_index);
		...
		double num_double;
		if (StringToDouble(num_string.data(), &num_double) &&
