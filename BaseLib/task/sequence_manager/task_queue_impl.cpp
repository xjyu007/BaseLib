...
#include "task/common/scoped_defer_task_posting.h"
...
#include "trace_event/common/trace_event_common.h"
...
namespace base::sequence_manager {
	...
	namespace internal {
		...
		bool TaskQueueImpl::GuardedTaskPoster::PostTask(PostedTask task) {
			...
			const auto token = operations_controller_.TryBeginOperation();
			if (!token)
				return false;
		...
		void TaskQueueImpl::MaybeLogPostTask(PostedTask* task) const {
#if DCHECK_IS_ON()
		...
		void TaskQueueImpl::PostImmediateTaskImpl(PostedTask task,
			...
			bool should_schedule_work = false;
			{
				...
				const auto add_queue_time_to_tasks = sequence_manager_->GetAddQueueTimeToTasks();
		...
		void TaskQueueImpl::PostDelayedTaskImpl(PostedTask task,
			...
			if (current_thread == CurrentThread::kMainThread) {
				...
				const auto time_domain_now = main_thread_only().time_domain->Now();
				const auto time_domain_delayed_run_time = time_domain_now + task.delay;
		...
		std::optional<DelayedWakeUp> TaskQueueImpl::GetNextScheduledWakeUpImpl() {
		...
		std::optional<TimeTicks> TaskQueueImpl::GetNextScheduledWakeUp() {
			std::optional<DelayedWakeUp> wake_up = GetNextScheduledWakeUpImpl();
			if (!wake_up)
				return std::nullopt;
			return wake_up->time;
		}
		...
		void TaskQueueImpl::AsValueInto(TimeTicks now,
			...
			if (main_thread_only().current_fence)
				state->SetInteger("current_fence", static_cast<int>(main_thread_only().current_fence));
			if (main_thread_only().delayed_fence) {
				state->SetDouble(
		...
		void TaskQueueImpl::SetTimeDomain(TimeDomain* time_domain) {
			...
			// domains.
			main_thread_only().scheduled_wake_up = std::nullopt;
			UpdateDelayedWakeUp(&lazy_now);
		}
		...
		void TaskQueueImpl::InsertFence(TaskQueue::InsertFencePosition position) {
			...
			const auto previous_fence = main_thread_only().current_fence;
			const auto current_fence = position == TaskQueue::InsertFencePosition::kNow
		...
		void TaskQueueImpl::TaskAsValueInto(const Task& task,
			...
			if (task.enqueue_order_set())
				state->SetInteger("enqueue_order", static_cast<int>(task.enqueue_order()));
		...
		WeakPtr<SequenceManagerImpl> TaskQueueImpl::GetSequenceManagerWeakPtr() const {
