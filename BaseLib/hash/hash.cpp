...
namespace base {
	...
	uint32_t Hash(const std::wstring& str) {
		return PersistentHash(str.data(), str.size() * sizeof(wchar_t));
	}
	...
	size_t HashInts32(uint32_t value1, uint32_t value2) {
		const uint64_t value1_64 = value1;
		uint64_t hash64 = (value1_64 << 32) | value2;

		if constexpr (sizeof(size_t) >= sizeof(uint64_t))
			return static_cast<size_t>(hash64);

		const uint64_t odd_random = 481046412LL << 32 | 1025306955LL;
		const auto shift_random = 10121U << 16;
	...
	size_t HashInts64(uint64_t value1, uint64_t value2) {
		...
		const auto product1 = static_cast<uint64_t>(value1a) * short_random1;
		const auto product2 = static_cast<uint64_t>(value1b) * short_random2;
		const auto product3 = static_cast<uint64_t>(value2a) * short_random3;
		const auto product4 = static_cast<uint64_t>(value2b) * short_random4;

		auto hash64 = product1 + product2 + product3 + product4;
