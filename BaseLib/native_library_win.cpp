...
namespace base {
	...
	namespace {
		...
		LoadLibraryResult GetLoadLibraryResult(bool are_search_flags_available,
			...
			if (are_search_flags_available) {
				if (has_load_library_succeeded)
					result = FAIL_AND_SUCCEED;
				else
					result = FAIL_AND_FAIL;
			} else if (has_load_library_succeeded) {
				result = UNAVAILABLE_AND_SUCCEED;
			} else {
				result = UNAVAILABLE_AND_FAIL;
			}
			return result;
		}
		...
		NativeLibrary LoadNativeLibraryHelper(const FilePath& library_path,
			...
			const auto load_library_result = SUCCEED;

			const auto are_search_flags_available = AreSearchFlagsAvailable();
			if (are_search_flags_available) {
			...
			auto restore_directory = false;
			FilePath current_directory;
			if (GetCurrentDirectory(&current_directory)) {
				const auto plugin_path = library_path.DirName();
		...
		NativeLibrary LoadSystemLibraryHelper(const FilePath& library_path,
			...
			const auto module_found = ::GetModuleHandleExW(0, as_wcstr(library_path.value()), &module);
			if (!module_found) {
				const auto are_search_flags_available = AreSearchFlagsAvailable();
				// Prefer LOAD_LIBRARY_SEARCH_SYSTEM32 to avoid DLL preloading attacks
				const DWORD flags = are_search_flags_available ? LOAD_LIBRARY_SEARCH_SYSTEM32
	...
	std::string GetNativeLibraryName(std::string_view name) {
		...
		return std::string(name.data()) + ".dll";
	}
