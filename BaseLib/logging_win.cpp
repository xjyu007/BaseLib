...
namespace logging {
	...
	bool LogEventProvider::LogMessage(LogSeverity severity, const char* file, int line, size_t message_start, const std::string& message) {
		...
		if (provider == nullptr || level > provider->enable_level())
			return false;
		...
		if (provider->enable_flags() & ENABLE_LOG_MESSAGE_ONLY) {
			...
			if (provider->enable_flags() & ENABLE_STACK_TRACE_CAPTURE)
				depth = CaptureStackBackTrace(2, kMaxBacktraceDepth, backtrace, nullptr);
		...
		return severity < provider->old_log_level_;
	}
