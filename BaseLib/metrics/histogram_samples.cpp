...
namespace base {
	...
	bool HistogramSamples::AtomicSingleSample::Accumulate(
		...
		const auto count_is_negative = count < 0;
		const auto count16 = static_cast<uint16_t>(count_is_negative ? -count : count);
		const auto bucket16 = static_cast<uint16_t>(bucket);
		...
		bool sample_updated;
		do {
			const auto original = subtle::Acquire_Load(&as_atomic);
			...
			const auto existing = subtle::Release_CompareAndSwap(
	...
	void HistogramSamples::Add(const HistogramSamples& other) {
		...
		const auto success = AddSubtractImpl(it.get(), ADD);
		DCHECK(success);
	}
	...
	void HistogramSamples::Subtract(const HistogramSamples& other) {
		IncreaseSumAndCount(-other.sum(), -other.redundant_count());
		const auto it = other.Iterator();
		const auto success = AddSubtractImpl(it.get(), SUBTRACT);
		DCHECK(success);
	}
	...
	void HistogramSamples::RecordNegativeSample(NegativeSampleReason reason, 
												HistogramBase::Count increment) const {
