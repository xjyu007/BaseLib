...
#include "macros.h"
#include "memory/read_only_shared_memory_region.h"
...
namespace base {
	...
	public:
		...
		public:
			...
			[[nodiscard]] virtual double GetEntropyForTrial(const std::string& trial_name,
		...
		// FieldTrial object. Does not use std::string_view to avoid conversions back to
		// std::string.
		...
		private:
			...
			[[nodiscard]] PickleIterator GetPickleIterator() const;
		...
		[[nodiscard]] const std::string& trial_name() const { return trial_name_; }
	...
	private:
		...
		[[nodiscard]] std::string default_group_name() const { return default_group_name_; }
		...
		[[nodiscard]] std::string group_name_internal() const { return group_name_; }
