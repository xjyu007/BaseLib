...
#define PRINT_INNER_FIELD_INFO(x, y)                           \
  LOG(INFO) << #y " type=" #x " offset=" << offsetof(Value, y) \
	...
	TEST(ValuesTest, ConstructStringFromConstChar16Ptr) {
		std::wstring str = ASCIIToUTF16("foobar");
	...
	TEST(ValuesTest, ConstructStringFromStringPiece16) {
		std::wstring str = ASCIIToUTF16("foobar");
		Value value{ std::wstring_view(str) };
		EXPECT_EQ(Value::Type::STRING, value.type());
		EXPECT_EQ("foobar", value.GetString());
	}
	...
	TEST(ValuesTest, FindBoolKey) {
		...
		const Value dict(std::move(storage));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("null"));
		EXPECT_NE(std::nullopt, dict.FindBoolKey("bool"));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("int"));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("double"));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("string"));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("blob"));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("list"));
		EXPECT_EQ(std::nullopt, dict.FindBoolKey("dict"));
	}
	...
	TEST(ValuesTest, FindIntKey) {
		...
		const Value dict(std::move(storage));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("null"));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("bool"));
		EXPECT_NE(std::nullopt, dict.FindIntKey("int"));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("double"));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("string"));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("blob"));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("list"));
		EXPECT_EQ(std::nullopt, dict.FindIntKey("dict"));
	}
	...
	TEST(ValuesTest, FindDoubleKey) {
		...
		const Value dict(std::move(storage));
		EXPECT_EQ(std::nullopt, dict.FindDoubleKey("null"));
		EXPECT_EQ(std::nullopt, dict.FindDoubleKey("bool"));
		EXPECT_NE(std::nullopt, dict.FindDoubleKey("int"));
		EXPECT_NE(std::nullopt, dict.FindDoubleKey("double"));
		EXPECT_EQ(std::nullopt, dict.FindDoubleKey("string"));
		EXPECT_EQ(std::nullopt, dict.FindDoubleKey("blob"));
		EXPECT_EQ(std::nullopt, dict.FindDoubleKey("list"));
		EXPECT_EQ(std::nullopt, dict.FindDoubleKey("dict"));
	}
	...
	TEST(ValuesTest, SetKey) {
		...
		dict.SetKey(std::string_view("null"), Value(Value::Type::NONE));
		dict.SetKey(std::string_view("bool"), Value(Value::Type::BOOLEAN));
	...
	TEST(ValuesTest, SetBoolKey) {
		...
		auto value = dict.FindBoolKey("true_key");
		ASSERT_TRUE(value);
		ASSERT_TRUE(*value);
	...
	TEST(ValuesTest, SetIntKey) {
		...
		auto value = dict.FindIntKey("one_key");
		ASSERT_TRUE(value);
		ASSERT_EQ(1, *value);
	...
	TEST(ValuesTest, SetPath) {
		...
		auto inserted = root.SetPath("one.two", Value(123));
		auto found = root.FindPathOfType("one.two", Value::Type::INTEGER);
		ASSERT_TRUE(found);
	...
	TEST(ValuesTest, SetBoolPath) {
		...
		auto inserted = root.SetBoolPath("foo.bar", true);
		auto found = root.FindPath("foo.bar");
		ASSERT_TRUE(found);
		EXPECT_EQ(inserted, found);
	...
	TEST(ValuesTest, SetIntPath) {
		DictionaryValue root;
		auto inserted = root.SetIntPath("foo.bar", 123);
		auto found = root.FindPath("foo.bar");
		ASSERT_TRUE(found);
		EXPECT_EQ(inserted, found);
	...
	TEST(ValuesTest, SetDoublePath) {
		DictionaryValue root;
		auto inserted = root.SetDoublePath("foo.bar", 1.23);
		auto found = root.FindPath("foo.bar");
		ASSERT_TRUE(found);
		EXPECT_EQ(inserted, found);
	...
	TEST(ValuesTest, SetStringPath) {
		DictionaryValue root;
		auto inserted = root.SetStringPath("foo.bar", "hello world");
		auto found = root.FindPath("foo.bar");
		ASSERT_TRUE(found);
		EXPECT_EQ(inserted, found);
		...
		ASSERT_TRUE(root.SetStringPath("foo.bar", std::string_view("rah rah")));
		ASSERT_TRUE(root.SetStringPath("foo.bar", std::string("temp string")));
		ASSERT_TRUE(root.SetStringPath("foo.bar", UTF8ToWide("temp string")));
	...
	TEST(ValuesTest, ExtractKey) {
		...
		EXPECT_EQ(std::nullopt, root.ExtractKey("two"));
		...
		EXPECT_EQ(std::nullopt, root.ExtractKey("one"));
	}
	...
	TEST(ValuesTest, ExtractPath) {
		...
		// Extraction of missing key should fail.
		EXPECT_EQ(std::nullopt, root.ExtractPath("one.two.four"));

		// Extraction of existing key should succeed.
		EXPECT_EQ(Value(123), root.ExtractPath("one.two.three"));

		// Second extraction of previously existing key should fail.
		EXPECT_EQ(std::nullopt, root.ExtractPath("one.two.three"));
	...
	TEST(ValuesTest, Basic) {
		...
		ASSERT_TRUE(settings.FindKey("global"));
		auto homepage = settings.FindStringPath("global.homepage");
		...
		auto toolbar_bookmarks = settings.FindListPath("global.toolbar.bookmarks");
		...
		auto bookmark_list = settings.FindPath("global.toolbar.bookmarks");
		ASSERT_TRUE(bookmark_list);
		ASSERT_EQ(1U, bookmark_list->GetList().size());
		auto bookmark = &bookmark_list->GetList()[0];
		ASSERT_TRUE(bookmark);
		ASSERT_TRUE(bookmark->is_dict());
		auto bookmark_name = bookmark->FindStringKey("name");
		ASSERT_TRUE(bookmark_name);
		ASSERT_EQ(std::string("Froogle"), *bookmark_name);
		auto bookmark_url = bookmark->FindStringKey("url");
	...
	TEST(ValuesTest, StringValue) {
		...
		auto utf16 = ASCIIToUTF16("http://google.com");
		...
		ASSERT_TRUE(narrow_value->GetAsString(static_cast<std::wstring*>(nullptr)));
	...
	TEST(ValuesTest, ListRemoval) {
...

		{
			...
			auto original_value = value->Clone();
	...
	TEST(ValuesTest, DictionarySetReturnsPointer) {
		{
			DictionaryValue dict;
			auto blank_ptr = dict.Set("foo.bar", std::make_unique<base::Value>());
			EXPECT_EQ(Value::Type::NONE, blank_ptr->type());
		}

		{
			DictionaryValue dict;
			auto blank_ptr = dict.SetWithoutPathExpansion(
		...
		{
			DictionaryValue dict;
			auto int_ptr = dict.SetInteger("foo.bar", 42);
		...
		{
			DictionaryValue dict;
			auto double_ptr = dict.SetDouble("foo.bar", 3.142);
		...
		{
			DictionaryValue dict;
			auto string_ptr = dict.SetString("foo.bar", "foo");
		...
		{
			DictionaryValue dict;
			auto string16_ptr = dict.SetString("foo.bar", ASCIIToUTF16("baz"));
		...
		{
			DictionaryValue dict;
			auto dict_ptr = dict.SetDictionary(
	...
	TEST(ValuesTest, DeepCopy) {
		...
		auto null_weak = original_dict.Set("null", std::make_unique<Value>());
		auto bool_weak = original_dict.Set("bool", std::make_unique<Value>(true));
		auto int_weak = original_dict.Set("int", std::make_unique<Value>(42));
		auto double_weak =
			original_dict.Set("double", std::make_unique<Value>(3.14));
		auto string_weak =
			original_dict.Set("string", std::make_unique<Value>("hello"));
		auto string16_weak = original_dict.Set(
			"std::wstring", std::make_unique<Value>(ASCIIToUTF16("hello16")));

		auto binary_weak = original_dict.Set(
		...
		auto list_element_0_weak = &list_weak->GetList()[0];
		auto list_element_1_weak = &list_weak->GetList()[1];

		auto dict_weak = original_dict.SetDictionary(
		...
		std::wstring copy_string16_value;
		...
		ASSERT_TRUE(copy_dict->Get("std::wstring", &copy_string16));
	...
	TEST(ValuesTest, DeepCopyCovariantReturnTypes) {
		...
		Value* string16_weak =
			original_dict.SetKey("std::wstring", Value(ASCIIToUTF16("hello16")));
		Value* binary_weak =
	...
	TEST(ValuesTest, GetWithNullOutValue) {
		...
		EXPECT_FALSE(main_dict.GetString("bool", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetString("int", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetString("double", static_cast<std::wstring*>(nullptr)));
		EXPECT_TRUE(main_dict.GetString("string", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetString("binary", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetString("dict", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetString("list", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetString("DNE", static_cast<std::wstring*>(nullptr)));
		...
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"bool", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"int", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"double", static_cast<std::wstring*>(nullptr)));
		EXPECT_TRUE(main_dict.GetStringWithoutPathExpansion(
			"string", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"binary", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"dict", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"list", static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_dict.GetStringWithoutPathExpansion(
			"DNE", static_cast<std::wstring*>(nullptr)));
		...
		EXPECT_FALSE(main_list.GetString(0, static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_list.GetString(1, static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_list.GetString(2, static_cast<std::wstring*>(nullptr)));
		EXPECT_TRUE(main_list.GetString(3, static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_list.GetString(4, static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_list.GetString(5, static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_list.GetString(6, static_cast<std::wstring*>(nullptr)));
		EXPECT_FALSE(main_list.GetString(7, static_cast<std::wstring*>(nullptr)));
