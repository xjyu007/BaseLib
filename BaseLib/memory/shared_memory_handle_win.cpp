...
namespace base {
	...
	SharedMemoryHandle::SharedMemoryHandle(HANDLE h, size_t size, const base::UnguessableToken& guid)
	...
	SharedMemoryHandle SharedMemoryHandle::Duplicate() const {
		...
		const auto success = ::DuplicateHandle(process, handle_, process, &duped_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
		if (!success)
