...
namespace base {
	...
	public:
		...
		void ExpectUniqueSample(const std::string& name, HistogramBase::Sample sample, HistogramBase::Count expected_count) const;
		template <typename T>
		void ExpectUniqueSample(const std::string& name, T sample, HistogramBase::Count expected_count) const {
			ExpectUniqueSample(name, static_cast<HistogramBase::Sample>(sample), expected_count);
		}
		...
		// object was constructed.
		void ExpectBucketCount(const std::string& name, HistogramBase::Sample sample, HistogramBase::Count expected_count) const;
		template <typename T>
		void ExpectBucketCount(const std::string& name, T sample, HistogramBase::Count expected_count) const {
		...
		// constructed.
		void ExpectTotalCount(const std::string& name, HistogramBase::Count count) const;

		// We know exact number of samples for buckets corresponding to a time
		// interval. Other intervals may have samples too.
		void ExpectTimeBucketCount(const std::string& name, TimeDelta sample, HistogramBase::Count count) const;
		...
		HistogramBase::Count GetBucketCount(const std::string& name, HistogramBase::Sample sample) const;
		...
		std::unique_ptr<HistogramSamples> GetHistogramSamplesSinceCreation(const std::string& histogram_name) const;
	...
	private:
		...
		void CheckBucketCount(const std::string& name, HistogramBase::Sample sample, Histogram::Count expected_count, const HistogramSamples& samples) const;
		...
		void CheckTotalCount(const std::string& name, Histogram::Count expected_count, const HistogramSamples& samples) const;
		...
		void GetBucketCountForSamples(const std::string& name, HistogramBase::Sample sample, const HistogramSamples& samples, HistogramBase::Count* count) const;
