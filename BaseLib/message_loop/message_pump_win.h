...
namespace base {
	...
	private:
		...
		void WaitForWork(Delegate::NextWorkInfo next_work_info) const;
	...
	public:
		...
		HRESULT RegisterIOHandler(HANDLE file_handle, IOHandler* handler) const;
		...
		bool RegisterJobObject(HANDLE job_handle, IOHandler* handler) const;
	...
	private:
		...
		bool GetIOItem(DWORD timeout, IOItem* item) const;
