...
namespace base {
	namespace {
		...
		public:
			explicit LegacyMockTickClock(scoped_refptr<const TestMockTimeTaskRunner> task_runner)
		...
		public:
			explicit LegacyMockClock(scoped_refptr<const TestMockTimeTaskRunner> task_runner)
				: task_runner_(std::move(task_runner)) {
			}
	...
	public:
		explicit NonOwningProxyTaskRunner(SingleThreadTaskRunner* target) : target_(target) {
			DCHECK(target_);
		}
	...
	TestMockTimeTaskRunner::TestOrderedPendingTask::TestOrderedPendingTask() : ordinal(0) {
	}
	...
	TestMockTimeTaskRunner::TestMockTimeTaskRunner(Type type) : TestMockTimeTaskRunner(Time::UnixEpoch(), TimeTicks(), type) {
	}

	TestMockTimeTaskRunner::TestMockTimeTaskRunner(Time start_time, TimeTicks start_ticks, Type type)
		: now_(start_time),	now_ticks_(start_ticks), tasks_lock_cv_(&tasks_lock_), proxy_task_runner_(MakeRefCounted<NonOwningProxyTaskRunner>(this)),
		mock_clock_(this) {
