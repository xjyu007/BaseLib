...
namespace base {
	...
	namespace trace_event {
		...
		public:
			...
			[[nodiscard]] bool IsFull() const { return next_free_ == kTraceBufferChunkSize; }

			[[nodiscard]] uint32_t seq() const { return seq_; }
			[[nodiscard]] static size_t capacity() { return kTraceBufferChunkSize; }
			[[nodiscard]] size_t size() const { return next_free_; }
			...
			[[nodiscard]] const TraceEvent* GetEventAt(size_t index) const {
				DCHECK(index < size());
				return &chunk_[index];
			}
		...
		public:
			...
			struct BASE_EXPORT SimpleOutput {
				...
				void Append(const std::string& json_trace_output);
