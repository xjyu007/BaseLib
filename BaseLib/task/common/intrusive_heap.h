...
#include "containers/intrusive_heap.h"
...
namespace base {
	namespace internal {
		...
		public:
			...
			[[nodiscard]] bool empty() const { return IntrusiveHeapImplType::empty(); }
			[[nodiscard]] size_t size() const { return IntrusiveHeapImplType::size(); }
			...
			[[nodiscard]] const T& Min() const { return IntrusiveHeapImplType::top(); }
			...
			[[nodiscard]] const T& at(HeapHandle handle) const {
				return IntrusiveHeapImplType::at(handle);
			}

			// Caution, mutating the heap invalidates iterators!
			[[nodiscard]] const T* begin() const { return IntrusiveHeapImplType::data(); }
			[[nodiscard]] const T* end() const { return IntrusiveHeapImplType::data() + size(); }
		};
