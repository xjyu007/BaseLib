...
#include "metrics/persistent_sample_map.h"
#include "metrics/sample_map.h"
...
namespace base {
	...
	bool SparseHistogram::HasConstructionArguments(Sample expected_minimum, Sample expected_maximum, uint32_t expected_bucket_count) const {
	...
	SparseHistogram::SparseHistogram(const char* name) : HistogramBase(name), unlogged_samples_(new SampleMap(HashMetricName(name))),
	...
	void SparseHistogram::GetCountAndBucketData(Count* count, int64_t* sum, ListValue* buckets) const {
		// TODO(kaiwang): Implement. (See HistogramBase::WriteJSON.)
	}

	void SparseHistogram::WriteAsciiImpl(bool graph_it, const std::string& newline, std::string* output) const {
	...
	void SparseHistogram::WriteAsciiHeader(const Count total_count, std::string* output) const {
		StringAppendF(output, "Histogram: %s recorded %d samples", histogram_name(), total_count);
		if (flags())
