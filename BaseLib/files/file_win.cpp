...
namespace base {
	...
	int64_t File::Seek(Whence whence, int64_t offset) const	{
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		...
		const auto move_method = static_cast<DWORD>(whence);
		if (!SetFilePointerEx(file_.Get(), distance, &res, move_method))
			return -1;
		return res.QuadPart;
	}

	int File::Read(int64_t offset, char* data, int size) const {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		DCHECK(!async_);
		if (size < 0)
	...
	int File::ReadAtCurrentPos(char* data, int size) const {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		DCHECK(!async_);
		if (size < 0)
	...
	int File::ReadNoBestEffort(int64_t offset, char* data, int size) const {
		// TODO(dbeam): trace this separately?
		return Read(offset, data, size);
	}

	int File::ReadAtCurrentPosNoBestEffort(char* data, int size) const {
		// TODO(dbeam): trace this separately?
		return ReadAtCurrentPos(data, size);
	}

	int File::Write(int64_t offset, const char* data, int size) const {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		DCHECK(!async_);
	...
	int File::WriteAtCurrentPos(const char* data, int size) const {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		DCHECK(!async_);
		if (size < 0)
	...
	int File::WriteAtCurrentPosNoBestEffort(const char* data, int size) const {
	...
	bool File::SetLength(int64_t length) const {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		...
		return ((::SetEndOfFile(file_.Get()) != FALSE) &&
			(::SetFilePointerEx(file_.Get(), file_pointer, nullptr, FILE_BEGIN) != FALSE));
	}

	bool File::SetTimes(Time last_access_time, Time last_modified_time) const {
		ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
		DCHECK(IsValid());
		...
		return (::SetFileTime(file_.Get(), nullptr, &last_access_filetime, &last_modified_filetime) != FALSE);
	}
	...
	File::Error File::Lock(File::LockMode mode) const {
		DCHECK(IsValid());
	...
	File::Error File::Unlock() const {
		DCHECK(IsValid());

		SCOPED_FILE_TRACE("Unlock");

		OVERLAPPED overlapped = {};
		const auto result = UnlockFileEx(file_.Get(), /*dwReserved=*/0,
			/*nNumberOfBytesToLockLow=*/MAXDWORD,
			/*nNumberOfBytesToLockHigh=*/MAXDWORD, &overlapped);
		if (!result)
			return GetLastFileError();
	...
	bool File::DeleteOnClose(bool delete_on_close) const {
	...
	void File::DoInitialize(const FilePath& path, uint32_t flags) {
		...
		file_.Set(CreateFile(as_wcstr(path.value()), access, sharing, nullptr,
