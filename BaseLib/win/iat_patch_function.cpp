...
namespace base::win {
	...
	namespace {
		...
		DWORD InterceptImportedFunction(HMODULE module_handle,
			...
			const PEImage target_image(module_handle);
	...
	DWORD IATPatchFunction::Patch(const wchar_t* module,
		...
		const char* function_name,
		void* new_function) {
		const auto module_handle = LoadLibraryW(module);
		if (!module_handle) {
			NOTREACHED();
			return GetLastError();
		}

		const auto error = PatchFromModule(module_handle, imported_from_module,
	...
	DWORD IATPatchFunction::PatchFromModule(HMODULE module,
		...
		const auto error = InterceptImportedFunction(module,
			imported_from_module,
			function_name,
			new_function,
			&original_function_,
	...
	DWORD IATPatchFunction::Unpatch() {
		const auto error = RestoreImportedFunction(intercept_function_,
			original_function_,
			iat_thunk_);
		DCHECK_EQ(static_cast<DWORD>(NO_ERROR), error);
