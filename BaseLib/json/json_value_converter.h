...
namespace base {
	...
	namespace internal {
		...
		template <>
		class BASE_EXPORT BasicValueConverter<std::wstring>
			: public ValueConverter<std::wstring> {
		public:
			BasicValueConverter() = default;

			bool Convert(const base::Value& value, std::wstring* field) const override;
	...
	public:
		...
		void RegisterStringField(const std::string& field_name,
			std::wstring StructType::* field) {
			fields_.push_back(
				std::make_unique<internal::FieldConverter<StructType, std::wstring>>(
					field_name, field, new internal::BasicValueConverter<std::wstring>));
		}
		...
		void RegisterRepeatedString(
			const std::string& field_name,
			std::vector<std::unique_ptr<std::wstring>> StructType::* field) {
			fields_.push_back(std::make_unique<internal::FieldConverter<
				StructType, std::vector<std::unique_ptr<std::wstring>>>>(
					field_name, field, new internal::RepeatedValueConverter<std::wstring>));
		}
