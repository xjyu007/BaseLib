...
#define STATIC_LATENCY_HISTOGRAM_POINTER_GROUP(histogram_name, priority, \
  ...
  STATIC_HISTOGRAM_POINTER_GROUP(                                        \
      ...
      Histogram::FactoryMicrosecondsTimeGet(                             \
		...
		TaskTracker::TaskTracker(std::string_view histogram_label)
		...
		void TaskTracker::FlushForTesting() const {
			CheckedAutoLock auto_lock(flush_lock_);
		...
		RegisteredTaskSource TaskTracker::RegisterTaskSource(
			...
			const auto shutdown_behavior = task_source->shutdown_behavior();
		...
		bool TaskTracker::CanRunPriority(TaskPriority priority) const {
			const auto can_run_policy = can_run_policy_.load();
		...
		void TaskTracker::RecordLatencyHistogram(TaskPriority priority,
			...
			const auto get_latency_histogram_name = [this, priority]() {
		...
		void TaskTracker::RecordHeartbeatLatencyAndTasksRunWhileQueuingHistograms(
			...
			const auto get_heartbeat_latency_histogram_name = [this, priority]() {
			...
			const auto get_num_tasks_run_while_queuing_histogram_name = [this, priority]() {
		...
		void TaskTracker::RunTask(Task task,
			...
			const auto previous_singleton_allowed =
				ThreadRestrictions::SetSingletonAllowed(
					traits.shutdown_behavior() !=
					TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN);
			const auto previous_io_allowed =
				ThreadRestrictions::SetIOAllowed(traits.may_block());
			const auto previous_wait_allowed =
...

			{
				...
				std::optional<SequencedTaskRunnerHandle> sequenced_task_runner_handle;
				std::optional<ThreadTaskRunnerHandle> single_thread_task_runner_handle;
				std::optional<EphemeralTaskExecutor> ephemiral_task_executor;
				...
				case TaskSourceExecutionMode::kSequenced:
					...
					sequenced_task_runner_handle.emplace(
						dynamic_cast<SequencedTaskRunner*>(task_source->task_runner()));
				...
				case TaskSourceExecutionMode::kSingleThread:
					...
					single_thread_task_runner_handle.emplace(
						dynamic_cast<SingleThreadTaskRunner*>(task_source->task_runner()));
					ephemiral_task_executor.emplace(
						static_cast<SequencedTaskRunner*>(task_source->task_runner()),
		...
		void TaskTracker::DecrementNumItemsBlockingShutdown() const {
			const auto shutdown_started_and_no_items_block_shutdown =
