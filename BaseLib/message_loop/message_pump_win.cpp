...
namespace base {
	namespace {
		...
		DWORD GetSleepTimeoutMs(TimeTicks next_task_time, 
			...
			const auto now = recent_now.is_null() ? TimeTicks::Now() : recent_now;
			const auto timeout_ms = (next_task_time - now).InMillisecondsRoundedUp();
	...
	MessagePumpForUI::MessagePumpForUI() {
		const auto succeeded = message_window_.Create(
	...
	void MessagePumpForUI::ScheduleWork() {
		...
		auto not_scheduled = false;
		if (!work_scheduled_.compare_exchange_strong(not_scheduled, true))
			return;  // Someone else continued the pumping.

		  // Make sure the MessagePump does some work for us.
		const auto ret = PostMessage(message_window_.hwnd(), kMsgHaveWork, 0, 0);
		if (ret)
	...
	void MessagePumpForUI::WaitForWork(Delegate::NextWorkInfo next_work_info) const	{
		DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
		...
		for (DWORD delay = GetSleepTimeoutMs(next_work_info.delayed_run_time, 
			...
			if (WAIT_OBJECT_0 == result) {
				...
				// current thread.
				MSG msg;
				const auto has_pending_sent_message = 
	...
	void MessagePumpForUI::HandleWorkMessage() {
		...
		const auto next_work_info = state_->delegate->DoSomeWork();
	...
	void MessagePumpForUI::ScheduleNativeTimer(
		...
		auto delay_msec = strict_cast<UINT>(GetSleepTimeoutMs(
	...
	bool MessagePumpForUI::ProcessNextWindowsMessage() {
		...
		auto sent_messages_in_queue = false;
		const auto queue_status = ::GetQueueStatus(QS_SENDMESSAGE);
	...
	bool MessagePumpForUI::ProcessMessageHelper(const MSG& msg) {
		...
		//TRACE_EVENT1("base,toplevel", "MessagePumpForUI::ProcessMessageHelper", "message", msg.message);
	...
	void MessagePumpForIO::ScheduleWork() {
		...
		if (!work_scheduled_.compare_exchange_strong(not_scheduled, true))
		  ...
		  // Make sure the MessagePump does some work for us.
		const auto ret = ::PostQueuedCompletionStatus(port_.Get(), 0,
	...
	HRESULT MessagePumpForIO::RegisterIOHandler(HANDLE file_handle,	
												IOHandler* handler) const {
		DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);

		const auto port = ::CreateIoCompletionPort(
	...
	void MessagePumpForIO::DoRunLoop() {
		...
		for (;;) {
			...
			auto next_work_info = state_->delegate->DoSomeWork();
			auto more_work_is_plausible = next_work_info.is_immediate();
			if (state_->should_quit)
				break;
	...
	void MessagePumpForIO::WaitForWork(Delegate::NextWorkInfo next_work_info) {
		...
		auto timeout = GetSleepTimeoutMs(next_work_info.delayed_run_time, 
										 next_work_info.recent_now);
	...
	bool MessagePumpForIO::GetIOItem(DWORD timeout, IOItem* item) const {
		DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);

		memset(item, 0, sizeof(*item));
		auto key = reinterpret_cast<ULONG_PTR>(nullptr);
	...
	bool MessagePumpForIO::MatchCompletedIOItem(IOHandler* filter, IOItem* item) {
		...
		for (auto it = completed_io_.begin(); 
