...
namespace base {
	...
	TEST(StringNumberConversionsTest, NumberToString) {
		...
		for (const auto& test : int_tests) {
			...
			EXPECT_EQ(NumberToString16(test.num), UTF8ToWide(test.sexpected));
			EXPECT_EQ(NumberToString(static_cast<unsigned>(test.num)), test.uexpected);
			EXPECT_EQ(NumberToString16(static_cast<unsigned>(test.num)), UTF8ToWide(test.uexpected));
		}
		for (const auto& test : int64_tests) {
			EXPECT_EQ(NumberToString(test.num), test.sexpected);
			EXPECT_EQ(NumberToString16(test.num), UTF8ToWide(test.sexpected));
			EXPECT_EQ(NumberToString(static_cast<uint64_t>(test.num)), test.uexpected);
			EXPECT_EQ(NumberToString16(static_cast<uint64_t>(test.num)), UTF8ToWide(test.uexpected));
		}
	}
	...
	TEST(StringNumberConversionsTest, StringToInt) {
		...
		for (const auto& i : cases) {
			...
			std::wstring utf16_input = UTF8ToWide(i.input);
		...
		std::wstring utf16_input = UTF8ToWide(input_string);
		output = 0;
		EXPECT_FALSE(StringToInt(utf16_input, &output));
		EXPECT_EQ(6, output);

		output = 0;
		const wchar_t negative_wide_input[] = { 0xFF4D, '4', '2', 0 };
		EXPECT_FALSE(StringToInt(std::wstring(negative_wide_input), &output));
		EXPECT_EQ(0, output);
	}
	...
	TEST(StringNumberConversionsTest, StringToUint) {
		...
		output = 0;
		const wchar_t negative_wide_input[] = { 0xFF4D, '4', '2', 0 };
		EXPECT_FALSE(StringToUint(std::wstring(negative_wide_input), &output));
		EXPECT_EQ(0U, output);
	}
	...
	TEST(StringNumberConversionsTest, DoubleToString) {
		...
		for (const auto& i : cases) {
			...
			EXPECT_EQ(i.expected, WideToUTF8(NumberToString16(i.input)));
		}
