...
namespace base {
	...
	namespace {
		...
		void UmaHistogramExactLinearWithSuffix(const char* histogram_name,
			...
			static_assert(std::is_convertible<SampleType, int>::value, "SampleType should be convertible to int");
		...
		void UmaHistogramTimesWithSuffix(const char* histogram_name, std::string_view histogram_suffix, TimeDelta sample) {
			DCHECK(histogram_name);
			std::string histogram_full_name(histogram_name);
			if (!histogram_suffix.empty()) {
				histogram_full_name.append(".");
		...
		void LogFailure(const FilePath& path, std::string_view histogram_suffix, TempFileFailure failure_code, std::string_view message) {
			UmaHistogramExactLinearWithSuffix("ImportantFile.TempFileFailures", histogram_suffix, failure_code, TEMP_FILE_FAILURE_MAX);
		...
		void DeleteTmpFile(const FilePath& tmp_file_path, std::string_view histogram_suffix) {
	...
	bool ImportantFileWriter::WriteFileAtomically(const FilePath& path, std::string_view data, std::string_view histogram_suffix) {
		...
		if (!tmp_file.IsValid()) {
			...
			LogFailure(path, histogram_suffix, FAILED_OPENING, "could not open temporary file");
			DeleteFile(tmp_file_path, false);
			return false;
		}

		// If this fails in the wild, something really bad is going on.
		const auto data_length = checked_cast<int32_t>(data.length());
		const auto bytes_written = tmp_file.Write(0, data.data(), data_length);
		if (bytes_written < data_length) {
			UmaHistogramExactLinearWithSuffix(
				"ImportantFile.FileWriteError", histogram_suffix,
				-base::File::GetLastFileError(), -base::File::FILE_ERROR_MAX);
		}
		const auto flush_success = tmp_file.Flush();
		tmp_file.Close();

		if (bytes_written < data_length) {
			LogFailure(path, histogram_suffix, FAILED_WRITING, "error writing, bytes_written=" + NumberToString(bytes_written));
		...
		if (!ReplaceFile(tmp_file_path, path, &replace_file_error)) {
			UmaHistogramExactLinearWithSuffix("ImportantFile.FileRenameError", histogram_suffix, -replace_file_error, -base::File::FILE_ERROR_MAX);
