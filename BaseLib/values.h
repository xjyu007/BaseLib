...
namespace base {
	...
	public:
		...
		// Value(const char*) and Value(const wchar_t*) are required despite
		// Value(std::string_view) and Value(std::wstring_view) because otherwise the
		...
		explicit Value(const wchar_t* in_string16);
		explicit Value(std::wstring_view in_string16);
		...
		[[nodiscard]] bool is_int() const { return type() == Type::INTEGER; }
		[[nodiscard]] bool is_double() const { return type() == Type::DOUBLE; }
		[[nodiscard]] bool is_string() const { return type() == Type::STRING; }
		[[nodiscard]] bool is_blob() const { return type() == Type::BINARY; }
		[[nodiscard]] bool is_dict() const { return type() == Type::DICTIONARY; }
		[[nodiscard]] bool is_list() const { return type() == Type::LIST; }

		// These will all CHECK that the type matches.
		[[nodiscard]] bool GetBool() const;
		[[nodiscard]] int GetInt() const;
		[[nodiscard]] double GetDouble() const;  // Implicitly converts from int if necessary.
		...
		[[nodiscard]] const Value* FindKey(std::string_view key) const;
		...
		[[nodiscard]] std::optional<int> FindIntKey(std::string_view key) const;
		// Note FindDoubleKey() will auto-convert INTEGER keys to their double
		// value, for consistency with GetDouble().
		[[nodiscard]] std::optional<double> FindDoubleKey(std::string_view key) const;

		// |FindStringKey| returns |nullptr| if value is not found or not a string.
		[[nodiscard]] const std::string* FindStringKey(std::string_view key) const;
		std::string* FindStringKey(std::string_view key);

		// Returns nullptr is value is not found or not a binary.
		[[nodiscard]] const BlobStorage* FindBlobKey(std::string_view key) const;

		// Returns nullptr if value is not found or not a dictionary.
		[[nodiscard]] const Value* FindDictKey(std::string_view key) const;
		Value* FindDictKey(std::string_view key);

		// Returns nullptr if value is not found or not a list.
		[[nodiscard]] const Value* FindListKey(std::string_view key) const;
		Value* FindListKey(std::string_view key);
		...
		Value* SetBoolKey(std::string_view key, bool val);
		Value* SetIntKey(std::string_view key, int val);
		Value* SetDoubleKey(std::string_view key, double val);
		Value* SetStringKey(std::string_view key, std::string_view val);
		// NOTE: These two overloads are provided as performance / code generation
		// optimizations.
		Value* SetStringKey(std::string_view key, const char* val);
		Value* SetStringKey(std::string_view key, std::string&& val);
		Value* SetStringKey(std::string_view key, std::wstring_view val);
		...
		std::optional<Value> ExtractKey(std::string_view key);
		...
		//
		// This version takes a std::string_view for the path, using dots as separators.
		// Example:
		//    auto* found = FindPath("foo.bar");
		Value* FindPath(std::string_view path);
		[[nodiscard]] const Value* FindPath(std::string_view path) const;

		// There are also deprecated versions that take the path parameter
		// as either a std::initializer_list<std::string_view> or a
		// span<const std::string_view>. The latter is useful to use a
		...
		[[nodiscard]] const Value* FindPath(std::initializer_list<std::string_view> path) const;
		[[nodiscard]] const Value* FindPath(span<const std::string_view> path) const;
		...
		Value* FindPathOfType(std::string_view path, Type type);
		[[nodiscard]] const Value* FindPathOfType(std::string_view path, Type type) const;
		...
		[[nodiscard]] std::optional<bool> FindBoolPath(std::string_view path) const;
		[[nodiscard]] std::optional<int> FindIntPath(std::string_view path) const;
		[[nodiscard]] std::optional<double> FindDoublePath(std::string_view path) const;
		[[nodiscard]] const std::string* FindStringPath(std::string_view path) const;
		std::string* FindStringPath(std::string_view path);
		[[nodiscard]] const BlobStorage* FindBlobPath(std::string_view path) const;
		Value* FindDictPath(std::string_view path);
		[[nodiscard]] const Value* FindDictPath(std::string_view path) const;
		Value* FindListPath(std::string_view path);
		[[nodiscard]] const Value* FindListPath(std::string_view path) const;

		// The following forms are deprecated too, use the ones that take the path
		// as a single std::string_view instead.
		Value* FindPathOfType(std::initializer_list<std::string_view> path, Type type);
		Value* FindPathOfType(span<const std::string_view> path, Type type);
		[[nodiscard]] const Value* FindPathOfType(std::initializer_list<std::string_view> path, 
												  Type type) const;
		[[nodiscard]] const Value* FindPathOfType(span<const std::string_view> path, Type type) const;
		...
		Value* SetPath(std::string_view path, Value&& value);

		// These setters are more convenient and efficient than the corresponding
		// SetPath(...) call.
		Value* SetBoolPath(std::string_view path, bool value);
		Value* SetIntPath(std::string_view path, int value);
		Value* SetDoublePath(std::string_view path, double value);
		Value* SetStringPath(std::string_view path, std::string_view value);
		Value* SetStringPath(std::string_view path, const char* value);
		Value* SetStringPath(std::string_view path, std::string&& value);
		Value* SetStringPath(std::string_view path, std::wstring_view value);

		// Deprecated: use the ones that take a std::string_view path parameter instead.
		...
		std::optional<Value> ExtractPath(std::string_view path);
		...
		[[nodiscard]] const_dict_iterator_proxy DictItems() const;

		// Returns the size of the dictionary, and if the dictionary is empty.
		// Note: These CHECK that type() is Type::DICTIONARY.
		[[nodiscard]] size_t DictSize() const;
		[[nodiscard]] bool DictEmpty() const;
		...
		// TODO(crbug.com/646113): Delete this and migrate callsites.
		[[nodiscard]] Value* DeepCopy() const;
		// DEPRECATED, use Value::Clone() instead.
		// TODO(crbug.com/646113): Delete this and migrate callsites.
		[[nodiscard]] std::unique_ptr<Value> CreateDeepCopy() const;
	...
	private:
		...
		[[nodiscard]] double AsDoubleInternal() const;
		...
		Value* SetKeyInternal(std::string_view key, std::unique_ptr<Value>&& val_ptr);
		Value* SetPathInternal(std::string_view path, std::unique_ptr<Value>&& value_ptr);
	...
	public:
		...
		[[nodiscard]] size_t size() const { return dict_.size(); }

		// Returns whether the dictionary is empty.
		[[nodiscard]] bool empty() const { return dict_.empty(); }
		...
		Value* SetDouble(std::string_view path, double in_value);
		// DEPRECATED, use Value::SetStringPath().
		Value* SetString(std::string_view path, std::string_view in_value);
		// DEPRECATED, use Value::SetStringPath().
		Value* SetString(std::string_view path, const std::wstring& in_value);
		...
		ListValue* SetList(std::string_view path, std::unique_ptr<ListValue> in_value);
		...
		// DEPRECATED, use Value::FindPath(path) instead.
		bool Get(std::string_view path, Value** out_value) const;
		...
		bool GetBoolean(std::string_view path, bool* bool_value) const;
		...
		// DEPRECATED, use Value::FindStringPath(path) instead.
		bool GetString(std::string_view path, std::wstring* out_value) const;
		...
		// DEPRECATED, use Value::FindBlobPath(path) instead.
		bool GetBinary(std::string_view path, Value** out_value) const;
		// DEPRECATED, use Value::FindPath(path) and Value's Dictionary API instead.
		bool GetDictionary(std::string_view path, 
						   const DictionaryValue** out_value) const;
		// DEPRECATED, use Value::FindPath(path) and Value's Dictionary API instead.
		bool GetDictionary(std::string_view path, DictionaryValue** out_value) const;
		...
		// DEPRECATED, use Value::FindPath(path) and Value::GetList() instead.
		bool GetList(std::string_view path, ListValue** out_value) const;

		// Like Get(), but without special treatment of '.'.  This allows e.g. URLs to
		// be used as paths.
		// DEPRECATED, use Value::FindKey(key) instead.
		bool GetWithoutPathExpansion(std::string_view key, const Value** out_value) const;
		// DEPRECATED, use Value::FindKey(key) instead.
		bool GetWithoutPathExpansion(std::string_view key, Value** out_value) const;
		// DEPRECATED, use Value::FindBoolKey(key) instead.
		bool GetBooleanWithoutPathExpansion(std::string_view key, bool* out_value) const;
		// DEPRECATED, use Value::FindIntKey(key) instead.
		bool GetIntegerWithoutPathExpansion(std::string_view key, int* out_value) const;
		// DEPRECATED, use Value::FindDoubleKey(key) instead.
		bool GetDoubleWithoutPathExpansion(std::string_view key, double* out_value) const;
		...
		// DEPRECATED, use Value::FindListKey(key) instead.
		bool GetListWithoutPathExpansion(std::string_view key, ListValue** out_value) const;
		...
		[[nodiscard]] std::unique_ptr<DictionaryValue> DeepCopyWithoutEmptyChildren() const;
		...
		public:
			...
			[[nodiscard]] bool IsAtEnd() const { return it_ == target_.dict_.end(); }
			void Advance() { ++it_; }

			[[nodiscard]] const std::string& key() const { return it_->first; }
			[[nodiscard]] const Value& value() const { return *it_->second; }
		...
		// DEPRECATED, use Value::DictItems() instead.
		[[nodiscard]] const_iterator begin() const { return dict_.begin(); }
		[[nodiscard]] const_iterator end() const { return dict_.end(); }

		// DEPRECATED, use Value::Clone() instead.
		// TODO(crbug.com/646113): Delete this and migrate callsites.
		[[nodiscard]] DictionaryValue* DeepCopy() const;
		// DEPRECATED, use Value::Clone() instead.
		// TODO(crbug.com/646113): Delete this and migrate callsites.
		[[nodiscard]] std::unique_ptr<DictionaryValue> CreateDeepCopy() const;
	};
	...
	public:
		...
		[[nodiscard]] size_t GetSize() const { return list_.size(); }
		...
		// DEPRECATED, use Value::Clone() instead.
		// TODO(crbug.com/646113): Delete this and migrate callsites.
		[[nodiscard]] ListValue* DeepCopy() const;
		// DEPRECATED, use Value::Clone() instead.
		// TODO(crbug.com/646113): Delete this and migrate callsites.
		[[nodiscard]] std::unique_ptr<ListValue> CreateDeepCopy() const;
	};
