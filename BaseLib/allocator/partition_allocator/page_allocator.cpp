...
#include "allocator/partition_allocator/address_space_randomization.h"
#include "allocator/partition_allocator/page_allocator_internal.h"
...
#include <windows.h>
#include "allocator/partition_allocator/page_allocator_internals_win.h"
...
namespace base {
	...
	namespace {
		...
		void* AllocPagesIncludingReserved(void* address, size_t length, PageAccessibilityConfiguration accessibility, PageTag page_tag,
			bool commit) {
			void* ret = SystemAllocPages(address, length, accessibility, page_tag, commit);
			if (ret == nullptr) {
		...
		void* TrimMapping(void* base, size_t base_length, size_t trim_length, uintptr_t alignment, PageAccessibilityConfiguration accessibility,
			bool commit) {
			...
			return TrimMappingInternal(base, base_length, trim_length, accessibility, commit, pre_slack, post_slack);
		}
	...
	void* SystemAllocPages(void* hint, size_t length, PageAccessibilityConfiguration accessibility, PageTag page_tag, bool commit) {
		...
		DCHECK(!(reinterpret_cast<uintptr_t>(hint) & kPageAllocationGranularityOffsetMask));
		DCHECK(commit || accessibility == PageInaccessible);
		return SystemAllocPagesInternal(hint, length, accessibility, page_tag, commit);
	}

	void* AllocPages(void* address, size_t length, size_t align, PageAccessibilityConfiguration accessibility, PageTag page_tag, bool commit) {
		...
		DCHECK(!(reinterpret_cast<uintptr_t>(address) & kPageAllocationGranularityOffsetMask));
		...
		if (address == nullptr) {
			...
			address = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(address) & align_base_mask);
		}
  ...
  // On 64 bit systems, try 3 random aligned addresses.
		...
		for (int i = 0; i < kExactSizeTries; ++i) {
			void* ret = AllocPagesIncludingReserved(address, length, accessibility, page_tag, commit);
			if (ret != nullptr) {
			...
			address = reinterpret_cast<void*>((reinterpret_cast<uintptr_t>(ret) + align_offset_mask) & align_base_mask);
		...
		do {
			...
			ret = AllocPagesIncludingReserved(address, try_length, accessibility, page_tag, commit);
			// The retries are for Windows, where a race can steal our mapping on
			// resize.
		} while (ret != nullptr && (ret = TrimMapping(ret, try_length, length, align, accessibility, commit)) == nullptr);
	...
	bool RecommitSystemPages(void* address, size_t length, PageAccessibilityConfiguration accessibility) {
		DCHECK_EQ(0UL, length & kSystemPageOffsetMask);
	...
	bool ReserveAddressSpace(size_t size) {
		...
		if (s_reservation_address == nullptr) {
			void* mem = SystemAllocPages(nullptr, size, PageInaccessible, PageTag::kChromium, false);
			if (mem != nullptr) {
				// We guarantee this alignment when reserving address space.
				DCHECK(!(reinterpret_cast<uintptr_t>(mem) & kPageAllocationGranularityOffsetMask));
