...
namespace base {
	...
	BASE_I18N_EXPORT std::wstring TimeFormatTimeOfDay(const Time& time);

	// Returns the time of day in 24-hour clock format with millisecond accuracy,
	// e.g., "15:07:30.568"
	BASE_I18N_EXPORT std::wstring TimeFormatTimeOfDayWithMilliseconds(const Time& time);
	...
	BASE_I18N_EXPORT std::wstring TimeFormatTimeOfDayWithHourClockType(
		const Time& time,
		HourClockType type,
		AmPmClockType ampm);

	// Returns a shortened date, e.g. "Nov 7, 2007"
	BASE_I18N_EXPORT std::wstring TimeFormatShortDate(const Time& time);

	// Returns a numeric date such as 12/13/52.
	BASE_I18N_EXPORT std::wstring TimeFormatShortDateNumeric(const Time& time);

	// Returns a numeric date and time such as "12/13/52 2:44:30 PM".
	BASE_I18N_EXPORT std::wstring TimeFormatShortDateAndTime(const Time& time);

	// Returns a month and year, e.g. "November 2007"
	BASE_I18N_EXPORT std::wstring TimeFormatMonthAndYear(const Time& time);

	// Returns a numeric date and time with time zone such as
	// "12/13/52 2:44:30 PM PST".
	BASE_I18N_EXPORT std::wstring
	...
	BASE_I18N_EXPORT std::wstring TimeFormatFriendlyDateAndTime(const Time& time);

	// Formats a time in a friendly sentence format, e.g.
	// "Monday, March 6, 2008".
	BASE_I18N_EXPORT std::wstring TimeFormatFriendlyDate(const Time& time);
	...
	BASE_I18N_EXPORT std::wstring TimeFormatWithPattern(const Time& time,
														const char* pattern);
	...
	BASE_I18N_EXPORT bool TimeDurationFormat(const TimeDelta time,
											 ...
											 std::wstring* out) WARN_UNUSED_RESULT;
	...
	BASE_I18N_EXPORT std::wstring DateIntervalFormat(const Time& begin_time,
													 const Time& end_time,
													 DateFormat format);
