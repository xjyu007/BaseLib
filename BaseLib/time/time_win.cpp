...
namespace base {
	...
	namespace {
		...
		const Feature kSlowDCTimerInterruptsFeature{
		    "SlowDCTimerInterrups", FEATURE_DISABLED_BY_DEFAULT};
		...
		UINT MinTimerIntervalLowResMs() {
			...
			static const UINT s_interval =
				FeatureList::IsEnabled(kSlowDCTimerInterruptsFeature) ? 8 : 4;
			return s_interval;
		}
...
#undef max
		bool SafeConvertToWord(int in, WORD* out) {
			const CheckedNumeric<WORD> result = in;
	...
	namespace subtle {
		Time TimeNowIgnoringOverride() {
			...
			// clock.
			while (true) {
				auto ticks = TimeTicksNowIgnoringOverride();

				// Calculate the time elapsed since we started our timer
				auto elapsed = ticks - g_initial_ticks;
	...
	bool Time::ActivateHighResolutionTimer(bool activating) {
		...
		AutoLock lock(*GetHighResLock());
		const auto period = g_high_res_timer_enabled ? MinTimerIntervalHighResMs()
	...
	double Time::GetHighResolutionTimerUsage() {
		AutoLock lock(*GetHighResLock());
		const auto now = subtle::TimeTicksNowIgnoringOverride();
		const auto elapsed_time = now - g_high_res_timer_usage_start;
		...
		auto used_time = g_high_res_timer_usage;
		if (g_high_res_timer_count > 0) {
	...
	namespace {
		...
		DWORD(*g_tick_function)() = &timeGetTimeWrapper;
		...
		TimeTicks RolloverProtectedNow() {
			...
			while (true) {
				...
				const auto original = subtle::Acquire_Load(&g_last_time_and_rollovers);
				state.as_opaque_32 = original;
				now = g_tick_function();
				const auto now_8 = static_cast<uint8_t>(now >> 24);
				...
				const auto check = subtle::Release_CompareAndSwap(
...
#define ATOMIC_THREAD_FENCE(memory_order) _ReadWriteBarrier();
		...
		TimeDelta QPCValueToTimeDelta(LONGLONG qpc_value) {
			...
			const int64_t whole_seconds = qpc_value / g_qpc_ticks_per_second;
			const int64_t leftover_ticks = qpc_value - (whole_seconds * g_qpc_ticks_per_second);
			return TimeDelta::FromMicroseconds(
	...
	TimeTicks::TickFunctionType TimeTicks::SetMockTickFunction(TickFunctionType ticker) {
		const auto old = g_tick_function;
	...
	ThreadTicks ThreadTicks::GetForThread(const PlatformThreadHandle& thread_handle) {
		...
		const auto tsc_ticks_per_second = TSCTicksPerSecond();
		if (tsc_ticks_per_second == 0)
			return ThreadTicks();

		// Return the CPU time of the current thread.
		const auto thread_time_seconds = thread_cycle_time / tsc_ticks_per_second;
		return ThreadTicks(
	...
	double ThreadTicks::TSCTicksPerSecond() {
		...
		static const auto tsc_initial = __rdtsc();
		static const auto perf_counter_initial = QPCNowRaw();

		// Make a another reading of the TSC and the performance counter every time
		// that this function is called.
		const auto tsc_now = __rdtsc();
		const auto perf_counter_now = QPCNowRaw();
		...
		const auto perf_counter_ticks = perf_counter_now - perf_counter_initial;
		const auto elapsed_time_seconds =
			perf_counter_ticks / static_cast<double>(perf_counter_frequency.QuadPart);

		static constexpr auto kMinimumEvaluationPeriodSeconds = 0.05;
		if (elapsed_time_seconds < kMinimumEvaluationPeriodSeconds)
			return 0;

		// Compute the frequency of the TSC.
		DCHECK_GE(tsc_now, tsc_initial);
		const auto tsc_ticks = tsc_now - tsc_initial;
	...
	TimeDelta TimeDelta::FromFileTime(FILETIME ft) {
		return FromMicroseconds(FileTimeToMicroseconds(ft));
	}
	...
	TimeDelta TimeDelta::FromWinrtDateTime(ABI::Windows::Foundation::DateTime dt) {
		...
		return FromMicroseconds(dt.UniversalTime / 10);
	}
