...
namespace base::trace_event {
	...
	void TraceEvent::EstimateTraceMemoryOverhead(TraceEventMemoryOverhead* overhead) const {
	...
	void TraceEvent::AppendAsJSON(
		...
		const auto time_int64 = timestamp_.ToInternalValue();
		int process_id;
		int thread_id;
		...
		*out += ",\"args\":";
		...
		if (phase_ == TRACE_EVENT_PHASE_COMPLETE) {
			const auto duration = duration_.ToInternalValue();
			if (duration != -1)
				StringAppendF(out, ",\"dur\":%" PRId64, duration);
			if (!thread_timestamp_.is_null()) {
				const auto thread_duration = thread_duration_.ToInternalValue();
				if (thread_duration != -1)
					StringAppendF(out, ",\"tdur\":%" PRId64, thread_duration);
			}
			if (!thread_instruction_count_.is_null()) {
				const auto thread_instructions = thread_instruction_delta_.ToInternalValue();
		...
		if (!thread_timestamp_.is_null()) {
			const auto thread_time_int64 = thread_timestamp_.ToInternalValue();
			StringAppendF(out, ",\"tts\":%" PRId64, thread_time_int64);
		}

		// Output ticount if thread_instruction_count is valid.
		if (!thread_instruction_count_.is_null()) {
			const auto thread_instructions = thread_instruction_count_.ToInternalValue();
		...
		const auto id_flags_ = flags_ & (TRACE_EVENT_FLAG_HAS_ID |
			TRACE_EVENT_FLAG_HAS_LOCAL_ID |
			TRACE_EVENT_FLAG_HAS_GLOBAL_ID);
		if (id_flags_) {
			if (scope_ != trace_event_internal::kGlobalScope)
				StringAppendF(out, R"(,"scope":"%s")", scope_);
