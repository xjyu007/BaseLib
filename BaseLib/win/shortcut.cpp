...
namespace base::win {
	...
	bool CreateOrUpdateShortcutLink(const FilePath& shortcut_path,
		...
		const auto shortcut_existed = PathExists(shortcut_path);
		...
		const auto result = i_persist_file->Save(as_wcstr(shortcut_path.value()), TRUE);
	...
	bool PinShortcutToTaskbar(const FilePath& shortcut) {
		...
		const auto result = reinterpret_cast<intptr_t>(ShellExecute(
			nullptr, L"taskbarpin", as_wcstr(shortcut.value()), nullptr, nullptr, 0));
		return result > 32;
	}
	...
	bool UnpinShortcutFromTaskbar(const FilePath& shortcut) {
		...
		const auto result = reinterpret_cast<intptr_t>(ShellExecute(
			nullptr, L"taskbarunpin", as_wcstr(shortcut.value()), nullptr, nullptr, 0));
		return result > 32;
	}
} // namespace base
