...
#include "trace_event/heap_profiler_allocation_context_tracker.h"
...
namespace base::trace_event {
	...
	AllocationContextTracker* AllocationContextTracker::GetInstanceForCurrentThread() {
		auto tracker = static_cast<AllocationContextTracker*>(AllocationContextTrackerTLS().Get());
	...
	bool AllocationContextTracker::GetContextSnapshot(AllocationContext* ctx) {
		...
		const auto mode = static_cast<CaptureMode>(
...
/*#if defined(OS_ANDROID) && BUILDFLAG(CAN_UNWIND_WITH_CFI_TABLE)
	...
	size_t frame_count = debug::TraceStackFramePointers(
		...
		//#else
			...
			const debug::StackTrace stack_trace(Backtrace::kMaxFrameCount + 1);
			auto frame_count = 0u;
			const auto frames = stack_trace.Addresses(&frame_count);
			//#endif

					// If there are too many frames, keep the ones furthest from main().
			const size_t backtrace_capacity = backtrace_end - backtrace;
			...
			for (auto i = starting_frame_index - 1; i >= 0; --i) {
