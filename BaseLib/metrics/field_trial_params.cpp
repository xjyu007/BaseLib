...
namespace base {
	...
	bool AssociateFieldTrialParamsFromString(
		...
		for (auto experiment_group :
		SplitStringPiece(params_string, ",", TRIM_WHITESPACE, SPLIT_WANT_ALL)) {
			auto experiment = SplitStringPiece(experiment_group, ":", TRIM_WHITESPACE, SPLIT_WANT_ALL);
			if (experiment.size() != 2) {
				DLOG(ERROR) << "Experiment and params should be separated by ':'";
				return false;
			}

			auto group_parts = SplitString(experiment[0], ".", TRIM_WHITESPACE, SPLIT_WANT_ALL);
			if (group_parts.size() != 2) {
				DLOG(ERROR) << "Trial and group name should be separated by '.'";
				return false;
			}

			auto key_values = SplitString(experiment[1], "/", TRIM_WHITESPACE, SPLIT_WANT_ALL);
			if (key_values.size() % 2 != 0) {
				DLOG(ERROR) << "Param name and param value should be separated by '/'";
				return false;
			}
			auto trial = decode_data_func(group_parts[0]);
			auto group = decode_data_func(group_parts[1]);
			...
			for (size_t i = 0; i < key_values.size(); i += 2) {
				auto key = decode_data_func(key_values[i]);
				auto value = decode_data_func(key_values[i + 1]);
				params[key] = value;
			}
			auto result = AssociateFieldTrialParams(trial, group, params);
			if (!result) {
	...
	std::string GetFieldTrialParamValueByFeature(const Feature& feature,
		...
		const auto trial = FeatureList::GetFieldTrial(feature);
		if (!trial)
			return std::string();
	...
	double GetFieldTrialParamByFeatureAsDouble(const Feature& feature,
		...
		const auto value_as_string = GetFieldTrialParamValueByFeature(feature, param_name);
	...
	std::string FeatureParam<std::string>::Get() const {
		const auto value = GetFieldTrialParamValueByFeature(*feature, name);
