...
#include "win/scoped_propvariant.h"
...
namespace base::win
{
		...
		void ValidatePathsAreEqual(const FilePath& expected_path, const FilePath& actual_path) {
			...
			EXPECT_NE(0U, ::GetLongPathName(as_wcstr(expected_path.value()), as_writable_wcstr(long_expected_path_chars), MAX_PATH))
				<< "Failed to get LongPathName of " << expected_path.value();
			EXPECT_NE(0U, ::GetLongPathName(as_wcstr(actual_path.value()), as_writable_wcstr(long_actual_path_chars), MAX_PATH))
		...
		void ValidateShortcut(const FilePath& shortcut_path, const ShortcutProperties& properties) {
			...
			EXPECT_TRUE(SUCCEEDED(hr = i_shell_link.CopyTo(i_persist_file.GetAddressOf())));
			if (FAILED(hr))
				return;

			// Load the shortcut.
			EXPECT_TRUE(SUCCEEDED(hr = i_persist_file->Load(as_wcstr(shortcut_path.value()), 0))) << "Failed to load shortcut at " << shortcut_path.value();
			if (FAILED(hr))
				return;

			if (properties.options & ShortcutProperties::PROPERTIES_TARGET) {
				EXPECT_TRUE(SUCCEEDED(i_shell_link->GetPath(as_writable_wcstr(read_target), MAX_PATH, NULL, SLGP_SHORTPATH)));
			...
			if (properties.options & ShortcutProperties::PROPERTIES_WORKING_DIR) {
				EXPECT_TRUE(SUCCEEDED(i_shell_link->GetWorkingDirectory(as_writable_wcstr(read_working_dir), MAX_PATH)));
				ValidatePathsAreEqual(properties.working_dir, FilePath(read_working_dir));
			}

			if (properties.options & ShortcutProperties::PROPERTIES_ARGUMENTS) {
				EXPECT_TRUE(SUCCEEDED(i_shell_link->GetArguments(as_writable_wcstr(read_arguments), MAX_PATH)));
				EXPECT_EQ(properties.arguments, read_arguments);
			}

			if (properties.options & ShortcutProperties::PROPERTIES_DESCRIPTION) {
				EXPECT_TRUE(SUCCEEDED(i_shell_link->GetDescription(as_writable_wcstr(read_description), MAX_PATH)));
				EXPECT_EQ(properties.description, read_description);
			}

			if (properties.options & ShortcutProperties::PROPERTIES_ICON) {
				EXPECT_TRUE(SUCCEEDED(i_shell_link->GetIconLocation(as_writable_wcstr(read_icon), MAX_PATH, &read_icon_index)));
			...
			EXPECT_TRUE(SUCCEEDED(hr = i_shell_link.CopyTo(property_store.GetAddressOf())));
			if (FAILED(hr))
				return;
