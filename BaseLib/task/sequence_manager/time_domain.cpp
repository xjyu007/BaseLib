...
namespace base::sequence_manager {
	...
	void TimeDomain::UnregisterQueue(internal::TaskQueueImpl* queue) {
		...
		SetNextWakeUpForQueue(queue, std::nullopt, internal::WakeUpResolution::kLow,
			&lazy_now);
	}
	...
	void TimeDomain::SetNextWakeUpForQueue(
		...
		std::optional<TimeTicks> previous_wake_up;
		std::optional<internal::WakeUpResolution> previous_queue_resolution;
		...
		std::optional<TimeTicks> new_wake_up;
		if (!delayed_wake_up_queue_.empty())
	...
	void TimeDomain::MoveReadyDelayedTasksToWorkQueues(LazyNow* lazy_now) const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
	...
	std::optional<TimeTicks> TimeDomain::NextScheduledRunTime() const {
		DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
		if (delayed_wake_up_queue_.empty())
			return std::nullopt;
	...
	void TimeDomain::AsValueInto(trace_event::TracedValue* state) const {
		...
		if (!delayed_wake_up_queue_.empty()) {
			const auto delay = delayed_wake_up_queue_.Min().wake_up.time - Now();
