...
#include "logging.h"
#include "metrics/field_trial_param_associator.h"
...
namespace base {
	...
	namespace {
		...
		bool ParseFieldTrialsString(const std::string& trials_string,
			...
			while (next_item < trials_string.length()) {
				const auto name_end = trials_string.find(kPersistentStringSeparator, next_item);
				if (name_end == std::string::npos || next_item == name_end)
					return false;
				auto group_name_end =
	...
	PickleIterator FieldTrial::FieldTrialEntry::GetPickleIterator() const {
		...
		const Pickle pickle(src, pickle_size);
	...
	FieldTrial::FieldTrial(const std::string& trial_name,
		...
		default_group_name_(std::move(default_group_name)),
	...
	FieldTrialList::~FieldTrialList() {
		...
		while (!registered_.empty()) {
			const auto it = registered_.begin();
			it->second->Release();
	...
	FieldTrial* FieldTrialList::FactoryGetFieldTrialWithRandomizationSeed(
		...
		const auto field_trial = new FieldTrial(trial_name, total_probability,
			default_group_name, entropy_value);
		Register(field_trial);
		return field_trial;
	}
	...
	// static
	void FieldTrialList::AllStatesToString(std::string* output, bool include_disabled) {
		if (!global_)
			return;
		AutoLock auto_lock(global_->lock_);
	...
	void FieldTrialList::GetActiveFieldTrialGroupsFromString(
		...
		for (const auto& entry : entries) {
			if (entry.activated) {
				...
				group.trial_name = entry.trial_name.data();
				group.group_name = entry.group_name.data();
	...
	void FieldTrialList::GetInitiallyActiveFieldTrials(
		...
		while ((entry = mem_iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) !=
			...
			if (subtle::NoBarrier_Load(&entry->activated) &&
				...
				FieldTrial::ActiveGroup group;
				group.trial_name = trial_name.data();
				group.group_name = group_name.data();
				active_groups->push_back(group);
			}
		}
	}
	...
	bool FieldTrialList::CreateTrialsFromString(
		...
		for (const auto& entry : entries) {
			const std::string trial_name = entry.trial_name.data();
			const std::string group_name = entry.group_name.data();
    ...
    (defined(OS_MACOSX) && !defined(OS_IOS))
		if (cmd_line.HasSwitch(field_trial_handle_switch)) {
			...
			const auto result = CreateTrialsFromSwitchValue(switch_value);
		...
		if (cmd_line.HasSwitch(switches::kForceFieldTrials)) {
			const auto result = CreateTrialsFromString(
	...
	FieldTrial* FieldTrialList::CreateFieldTrial(
		...
		auto field_trial = FieldTrialList::Find(name);
		if (field_trial) {
		...
		const auto kTotalProbability = 100;
	...
	bool FieldTrialList::GetParamsFromSharedMemory(
		...
		const auto actual_size = sizeof(FieldTrial::FieldTrialEntry) + entry->pickle_size;
	...
	void FieldTrialList::ClearParamsFromSharedMemoryForTesting() {
		...
		// the existing item.
		auto allocator = global_->field_trial_allocator_.get();
		FieldTrialAllocator::Iterator mem_iter(allocator);
		...
		while ((prev_ref = mem_iter.GetNextOfType<FieldTrial::FieldTrialEntry>()) !=
			...
			const auto total_size = sizeof(FieldTrial::FieldTrialEntry) + pickle.size();
			auto new_entry =
			...
			FieldTrial* trial = global_->PreLockedFind(trial_name.data());
			trial->ref_ = new_ref;
...
#error Unsupported OS
	...
	ReadOnlySharedMemoryRegion
		...
		auto tokens = SplitStringPiece(
			switch_value, ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);

		if (tokens.size() != 4)
			return ReadOnlySharedMemoryRegion();

		auto field_trial_handle = 0;
...
#elif defined(OS_WIN)
		auto handle = reinterpret_cast<HANDLE>(field_trial_handle);
		if (IsCurrentProcessElevated()) {
			...
			const auto parent_pid = GetParentProcessId(GetCurrentProcess());
			const auto parent_handle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, parent_pid);
	...
	bool FieldTrialList::CreateTrialsFromSwitchValue(
		...
		if (!shm.IsValid())
			return false;
		return CreateTrialsFromSharedMemoryRegion(shm);
	}
#elif defined(OS_POSIX) && !defined(OS_NACL)
	// static
	...
	bool FieldTrialList::CreateTrialsFromSharedMemoryMapping(
		...
		const auto shalloc = global_->field_trial_allocator_.get();
		...
		while ((entry = mem_iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) !=
			...
			// StringPieces.
			auto trial =
				CreateFieldTrial(trial_name.data(), group_name.data());
	...
	void FieldTrialList::AddToAllocatorWhileLocked(
		...
		const auto total_size = sizeof(FieldTrial::FieldTrialEntry) + pickle.size();
		const auto ref = allocator->Allocate(
	...
	void FieldTrialList::ActivateFieldTrialEntryWhileLocked(
		...
		const auto ref = field_trial->ref_;
		if (ref == FieldTrialAllocator::kReferenceNull) {
