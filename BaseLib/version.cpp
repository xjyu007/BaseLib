...
namespace base {
	...
	namespace {
		...
		int CompareVersionComponents(const std::vector<uint32_t>& components1,
			...
			const auto count = std::min(components1.size(), components2.size());
	...
	bool Version::IsValidWildcardString(std::string_view wildcard_string) {
		auto version_string = wildcard_string;
	...
	int Version::CompareToWildcardString(std::string_view wildcard_string) const {
		...
		if (!EndsWith(wildcard_string, ".*", CompareCase::SENSITIVE)) {
			const Version version(wildcard_string);
			DCHECK(version.IsValid());
			return CompareTo(version);
		}

		std::vector<uint32_t> parsed;
		const auto success = ParseVersionNumbers(
			wildcard_string.substr(0, wildcard_string.length() - 2), &parsed);
		DCHECK(success);
		const auto comparison = CompareVersionComponents(components_, parsed);
		...
		const auto min_num_comp = std::min(components_.size(), parsed.size());
	...
	const std::string Version::GetString() const {
		...
		const auto count = components_.size();
