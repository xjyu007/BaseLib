...
namespace base {
	...
	namespace {
		...
		void ValidateJsonList(const std::string& json) {
			std::optional<Value> list = JSONReader::Read(json);
			ASSERT_TRUE(list);
		...
		TEST(JSONValueDeserializerTest, ReadProperJSONFromStringPiece) {
			// Create a std::string_view for the substring of kProperJSONPadded that matches
			// kProperJSON.
		...
		TEST(JSONValueDeserializerTest, ReadProperJSONFromFile) {
			...
			const auto temp_file(tempdir.GetPath().AppendASCII("test.json"));
		...
		TEST(JSONValueDeserializerTest, AllowTrailingComma) {
			...
			const auto root = deserializer.Deserialize(nullptr, nullptr);
			ASSERT_TRUE(root);
		...
		TEST(JSONValueSerializerTest, Roundtrip) {
			static const char kOriginalSerialization[] =
				R"({"bool":true,"double":3.14,"int":42,"list":[1,2],"null":null})";
			JSONStringValueDeserializer deserializer(kOriginalSerialization);
			auto root_dict = deserializer.Deserialize(nullptr, nullptr);
			ASSERT_TRUE(root_dict);
			ASSERT_TRUE(root_dict->is_dict());

			const auto null_value = root_dict->FindKey("null");
...
#undef JSON_NEWLINE
		...
		TEST(JSONValueSerializerTest, StringEscape) {
			...
			std::string expected_output = R"({"all_chars":")" + all_chars_expected +
				"\"}";
		...
		TEST(JSONValueSerializerTest, UnicodeStrings) {
			...
			const std::wstring test(L"\x7F51\x9875");
		...
		TEST(JSONValueSerializerTest, HexStrings) {
			...
			static const char kExpected[] = R"({"test":"\u0001\u0002"})";
			...
			static const char kEscapedChars[] = R"({"test":"\u0067\u006f"})";
		...
		TEST(JSONValueSerializerTest, JSONReaderComments) {
			...
			auto list = JSONReader::Read(R"(["// ok\n /* foo */ "])");
			ASSERT_TRUE(list);
			ASSERT_TRUE(list->is_list());
			ASSERT_EQ(1U, list->GetList().size());
			const auto& elt = list->GetList()[0];
		...
		TEST_F(JSONFileValueSerializerTest, Roundtrip) {
			...
			auto null_value = root_dict->FindKey("null");
			ASSERT_TRUE(null_value);
			ASSERT_TRUE(null_value->is_none());
			...
			auto string_value = root_dict->FindStringKey("string");
			ASSERT_TRUE(string_value);
			ASSERT_EQ("hello", *string_value);

			// Now try writing.
			const auto written_file_path =
