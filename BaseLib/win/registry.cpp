...
namespace base::win {
	...
	class RegKey::Watcher final : public ObjectWatcher::Delegate {
	public:
		Watcher() = default;
	...
	bool RegKey::Watcher::StartWatching(HKEY key, ChangeCallback callback) {
		...
		const auto result = RegNotifyChangeKeyValue(key, TRUE, filter, watch_event_.Get(),
		                                      TRUE);
	...
	LONG RegKey::CreateWithDisposition(HKEY rootkey,
		...
		HKEY subhkey = NULL;
		const auto result = RegCreateKeyEx(rootkey, subkey, 0, NULL, REG_OPTION_NON_VOLATILE, access, 
	...
	LONG RegKey::CreateKey(const wchar_t* name, REGSAM access) {
		...
		HKEY subkey = NULL;
		const auto result =	RegCreateKeyEx(key_, name, 0, NULL, REG_OPTION_NON_VOLATILE,
	...
	LONG RegKey::Open(HKEY rootkey, const wchar_t* subkey, REGSAM access) {
		...
		const auto result = RegOpenKeyEx(rootkey, subkey, 0, access, &subhkey);
		if (result == ERROR_SUCCESS) {
			Close();
			key_ = subhkey;
			wow64access_ = access & kWow64AccessMask;
	...
	LONG RegKey::OpenKey(const wchar_t* relative_key_name, REGSAM access) {
		...
		HKEY subkey = NULL;
		const auto result =	RegOpenKeyEx(key_, relative_key_name, 0, access, &subkey);
	...
	DWORD RegKey::GetValueCount() const {
		DWORD count = 0;
		const auto result = RegQueryInfoKey(key_, NULL, 0, NULL, NULL, NULL, NULL, &count,
											NULL, NULL, NULL, NULL);
	...
	LONG RegKey::GetValueNameAt(int index, std::wstring* name) const {
		...
		const auto r = ::RegEnumValue(key_, index, buf, &bufsize, NULL,	NULL, NULL, NULL);
		if (r == ERROR_SUCCESS)
	...
	LONG RegKey::DeleteKey(const wchar_t* name) const {
		DCHECK(key_);
		DCHECK(name);
		HKEY subkey = NULL;
		...
		// behavior.
		const auto result = RegOpenKeyEx(key_, as_wcstr(name), 0,
	...
	LONG RegKey::DeleteEmptyKey(const wchar_t* name) const {
		DCHECK(key_);
		DCHECK(name);

		HKEY target_key = NULL;
		auto result = RegOpenKeyEx(key_, name, 0, KEY_READ | wow64access_, &target_key);
	...
	LONG RegKey::DeleteValue(const wchar_t* value_name) const {
		DCHECK(key_);
		const auto result = RegDeleteValue(key_, value_name);
		return result;
	}
	...
	LONG RegKey::ReadValueDW(const wchar_t* name, DWORD* out_value) const {
		...
		DWORD local_value = 0;
		auto result = ReadValue(name, &local_value, &size, &type);
		if (result == ERROR_SUCCESS) {
	...
	LONG RegKey::ReadValue(const wchar_t* name,
						   ...
						   DWORD* dsize,
						   DWORD* dtype) const {
		const auto result = RegQueryValueEx(key_, name, 0, dtype,
	...
	LONG RegKey::ReadValues(const wchar_t* name, 
		...
		DWORD size = 0;
		auto result = ReadValue(name, NULL, &size, &type);
		...
		const auto buffer_end = buffer.cend();
	...
	LONG RegKey::WriteValue(const wchar_t* name, DWORD in_value) const {
		return WriteValue(
			name, &in_value, static_cast<DWORD>(sizeof(in_value)), REG_DWORD);
	}

	LONG RegKey::WriteValue(const wchar_t* name, const wchar_t* in_value) const	{
		return WriteValue(
			name, in_value,
	...
	LONG RegKey::WriteValue(const wchar_t* name,
		...
		return RegSetValueEx(key_, name, 0, dtype,
	...
	bool RegKey::StartWatching(ChangeCallback callback) {
		...
		return key_watcher_->StartWatching(key_, std::move(callback));
	}
	...
	LONG RegKey::RegDelRecurse(HKEY root_key, const wchar_t* name, REGSAM access) {
		...
		auto result = RegDeleteKeyEx(root_key, name, access, 0);
		if (result == ERROR_SUCCESS)
			return result;
		...
		const auto base_key_length = subkey_name.length();
		std::wstring key_name;
		while (result == ERROR_SUCCESS) {
			auto key_size = kMaxKeyNameLength;
			result =
	...
	void RegistryValueIterator::Initialize(HKEY root_key,
		...
		auto result = RegOpenKeyEx(root_key, folder_key, 0,	KEY_READ | wow64access, &key_);
		if (result != ERROR_SUCCESS) {
			key_ = NULL;
		} else {
			DWORD count = 0;
	...
	DWORD RegistryValueIterator::ValueCount() const {
		DWORD count = 0;
		const auto result = ::RegQueryInfoKey(key_, NULL, 0, NULL, NULL, NULL, NULL,
	...
	bool RegistryValueIterator::Read() {
		if (Valid()) {
			...
			if (result == ERROR_MORE_DATA) {
				...
				const auto value_size_in_wchars = to_wchar_size(value_size_);
	...
	DWORD RegistryKeyIterator::SubkeyCount() const {
		DWORD count = 0;
		const auto result = ::RegQueryInfoKey(key_, NULL, 0, NULL, &count, NULL, NULL,
	...
	bool RegistryKeyIterator::Read() {
		if (Valid()) {
			auto ncount = static_cast<DWORD>(size(name_));
			FILETIME written;
			const auto r = ::RegEnumKeyEx(key_, index_, name_, &ncount,	NULL, NULL, NULL, 
										  &written);
			if (ERROR_SUCCESS == r)
				return true;
		}
