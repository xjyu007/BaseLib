...
namespace base::trace_event {
	...
	namespace {
		...
		public:
			...
			[[nodiscard]] bool IsFull() const override { return false; }
			...
			TraceEvent* GetEventByHandle(TraceEventHandle handle) override {
				if (handle.chunk_index >= chunks_.size())
					return nullptr;
				auto chunk = chunks_[handle.chunk_index].get();
			...
			const TraceBufferChunk* NextChunk() override {
				...
				while (current_iteration_index_ != queue_tail_) {
					const auto chunk_index = recyclable_chunks_queue_[current_iteration_index_];
			...
			void EstimateTraceMemoryOverhead(TraceEventMemoryOverhead* overhead) override {
				...
				for (auto queue_index = queue_head_; queue_index != queue_tail_;
					queue_index = NextQueueIndex(queue_index)) {
					const auto chunk_index = recyclable_chunks_queue_[queue_index];
					if (chunk_index >= chunks_.size())  // Skip uninitialized chunks.
						continue;
		...
		private:
			[[nodiscard]] bool QueueIsEmpty() const { return queue_head_ == queue_tail_; }
			...
			[[nodiscard]] bool QueueIsFull() const { return QueueSize() == queue_capacity() - 1; }
			...
			[[nodiscard]] size_t NextQueueIndex(size_t index) const {
				index++;
		...
		public:
			...
			[[nodiscard]] bool IsFull() const override { return chunks_.size() >= max_chunks_; }
	...
	void TraceResultBuffer::Finish() const {
		output_callback_.Run("]");
	}
